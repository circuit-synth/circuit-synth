...
class ComprehensiveRegressionSuite:
    ...
    def clear_all_caches(self) -> bool:
        ...
        if pycache_count > 0:
            self.log(f"Cleared {pycache_count} bytecode files/directories", "SUCCESS", 1)
...
"""
    ...
    def rebuild_rust_modules(self) -> bool:
        ...
        if test_script.exists():
            self.log("Using test_rust_modules.sh to build and test Rust modules...", "INFO")
            ...
            if success:
                ...
                for rust_module in self.project_root.glob("rust_modules/*/Cargo.toml"):
                    ...
                    if (module_dir / "pyproject.toml").exists():
                        self.log(f"Building Python bindings for {module_name}...", "INFO", 1)
                        success, _, _ = self.run_command(
                        ...
                        if success:
                            self.log(f"✅ Python bindings built for {module_name}", "SUCCESS", 2)
                        else:
                            self.log(f"⚠️ Failed to build Python bindings for {module_name}", "WARNING", 2)
    ...
    def run_python_test(self, code: str, name: str, category: str, 
                       severity: str = "MEDIUM",
    ...
    def test_core_functionality(self):
        ...
        tests.append(self.run_python_test("""
from circuit_synth import *
print('✅ Basic imports work')
""", "Basic Imports", "Core", "CRITICAL"))
...
test_circuit()
""", "Component Creation", "Core", "CRITICAL"))
...
""", "Reference Auto-Assignment", "Core", "HIGH"))
...
""", "JSON Export", "KiCad", "HIGH", self.test_output_dir))
...
""", "Symbol Library Access", "KiCad", "HIGH"))
                ...
                for file_path in expected_files:
                    ...
                    if not full_path.exists():
                        ...
                        if size < 100:  # Files should have content
                            missing_files.append(f"{file_path} (too small: {size} bytes)")
    ...
    def run_all_tests(self) -> bool:
        ...
        # Step 7: Run example project test
        example_passed = self.test_example_project()
        
        # Step 8: Run additional tests (if not quick mode)
    ...
    def run_additional_tests(self):
        ...
        self.results.append(self.run_python_test("""
...
""", "JLCPCB Integration", "External", "LOW"))
...
""", "STM32 Search", "Components", "MEDIUM"))
    ...
    def generate_summary(self) -> bool:
        ...
        # Overall summary
        self.log("\n" + "-" * 60, "INFO")
        self.log(f"Total: {total_passed}/{total_passed + total_failed} tests passed", "INFO")
