...
impl RustSchematicWriter {
    ...
    pub fn generate_hierarchical_labels(
        &mut self,
        ...
        info!(
            "    - Is hierarchical: {}",
            self.circuit_data.is_hierarchical()
        );
        ...
        info!(
            "    - Local components: {}",
            self.circuit_data.components.len()
        );
        info!("    - Local nets: {}", self.circuit_data.nets.len());
        info!(
            "    - Total components (all levels): {}",
            hierarchy_stats.total_components
        );
        info!(
            "    - Total nets (all levels): {}",
        ...
        if self.circuit_data.is_hierarchical() {
            ...
            for (i, subcircuit) in self.circuit_data.subcircuits.iter().enumerate() {
                info!(
                    "  Subcircuit #{}: '{}' with {} components, {} nets",
                    i + 1,
                    subcircuit.name,
                    subcircuit.components.len(),
                    subcircuit.nets.len()
                );
    ...
    fn calculate_label_position(
        ...
        info!(
            "🦀 FIXED LABEL POSITION CALCULATION for {}.{}",
            component.reference, pin_id
        );
        info!(
        ...
        info!(
            "  🎯 FINAL FIXED LABEL POSITION: ({:.2}, {:.2})",
    ...
    fn determine_label_orientation(
        ...
        debug!(
            "🧮 Orientation calculation: pin={}°, label={}°, component_rotation={}°, global={}°",
    ...
    pub fn generate_schematic_sexp(&self) -> Result<String, SchematicError> {
        ...
        let result =
            generate_schematic_sexp(&self.circuit_data, &self.hierarchical_labels, &self.config)?;
    ...
    pub fn write_to_file(&self, path: &str) -> Result<(), SchematicError> {
        ...
        std::fs::write(path, content).map_err(|e| {
            SchematicError::IoError(format!("Failed to write file {}: {}", path, e))
        })?;
...
mod tests {
    ...
    fn test_six_hierarchical_labels_like_reference_design() {
        ...
        assert_eq!(
            labels.len(),
            6,
            "Should generate exactly 6 hierarchical labels like the reference design"
        );
        ...
        println!(
            "✅ Successfully generated {} hierarchical labels:",
            labels.len()
        );
        for label in &labels {
            println!(
                "  - {} at ({:.2}, {:.2}) orientation {:.0}°",
                label.name, label.position.x, label.position.y, label.orientation
            );
            println!(
                "    Grid alignment check: x % 1.27 = {:.6}, y % 1.27 = {:.6}",
                label.position.x % 1.27,
                label.position.y % 1.27
            );
        }
