...
use rust_netlist_processor::{NetlistProcessor, Circuit, Component, Net, NetNode, PinInfo, PinType};
...
fn create_test_circuit(num_components: usize, nets_per_component: usize) -> Circuit {
    ...
    for i in 0..num_components {
        ...
        // Add properties
        component.properties.insert("ki_keywords".to_string(), "resistor".to_string());
        component.properties.insert("ki_fp_filters".to_string(), "R_*".to_string());
...
fn create_hierarchical_circuit(depth: usize, components_per_level: usize) -> Circuit {
    fn create_subcircuit(name: String, level: usize, max_depth: usize, components_per_level: usize) -> Circuit {
        ...
        for i in 0..components_per_level {
            ...
            for j in 0..components_per_level.min(3) {
                let node = NetNode::new(
                    ...
                    PinInfo::new((i + 1).to_string(), format!("PIN_{}", i + 1), PinType::Bidirectional),
                );
                net.add_node(node);
            }
...
fn bench_complete_netlist_generation(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(
            ...
            |b, circuit| {
                b.iter(|| {
                    processor.generate_kicad_netlist(black_box(circuit)).unwrap()
                });
            },
        );
    }
...
fn bench_processing_stages(c: &mut Criterion) {
    ...
    c.bench_function("nets_only", |b| {
        b.iter(|| {
            processor.generate_nets_only(black_box(&circuit)).unwrap()
        });
    });
    
    c.bench_function("components_only", |b| {
        b.iter(|| {
            processor.generate_components_only(black_box(&circuit)).unwrap()
        });
    });
}
    ...
    for size in sizes {
        ...
        group.bench_with_input(
            ...
            |b, circuit| {
                b.iter(|| {
                    let mut processor = NetlistProcessor::new();
                    let _result = processor.generate_kicad_netlist(black_box(circuit)).unwrap();
...
fn bench_performance_regression(c: &mut Criterion) {
    ...
    c.bench_function("performance_baseline", |b| {
        b.iter(|| {
            let result = processor.generate_kicad_netlist(black_box(&circuit)).unwrap();
