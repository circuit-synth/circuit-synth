...
impl SExprFormatter {
    ...
    pub fn generate_design_section(&mut self, circuit: &Circuit) -> Result<String> {
        ...
        writeln!(
            section,
            "    (tool \"Circuit-Synth Rust Netlist Processor v0.1.0\")"
        )?;
    ...
    fn generate_hierarchical_sheets(
        &self,
        section: &mut String,
        circuit: &Circuit,
        source_file: &str,
    ...
    fn generate_subcircuit_sheets(
        &self,
        section: &mut String,
        circuit: &Circuit,
        parent_path: &str,
        sheet_number: &mut i32,
        source_file: &str,
    ) -> std::fmt::Result {
        for subcircuit in &circuit.subcircuits {
            ...
            *sheet_number += 1;
            ...
            self.generate_subcircuit_sheets(
                section,
                subcircuit,
                &sheet_name,
                sheet_number,
    ...
    pub fn format_component_entry(&mut self, component: &Component) -> Result<String> {
        ...
        // Libsource
        let (lib, part) = component
            .library()
            .and_then(|lib| component.part().map(|part| (lib, part)))
            .map_err(|e| {
                NetlistError::formatting_error(format!("Invalid component symbol: {}", e))
            })?;
        ...
        write!(
            entry,
            "\n      (property (name \"Sheetfile\") (value \"{}.kicad_sch\"))",
            "circuit"
        )?;
        ...
        write!(
            ...
            component.timestamp.as_deref().unwrap_or("default")
        )?;
    ...
    pub fn format_libpart_entry(
        ...
        // Docs (datasheet)
        let datasheet = if component.datasheet.is_empty() {
            "~"
        } else {
            &component.datasheet
        };
