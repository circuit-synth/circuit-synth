...
fn bench_component_addition(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        group.bench_with_input(
            BenchmarkId::new("final_references", size),
            size,
            |b, &size| {
                b.iter(|| {
                    let mut circuit =
                        Circuit::with_capacity("BenchCircuit".to_string(), size, size / 2);
        ...
        group.bench_with_input(
            BenchmarkId::new("prefix_references", size),
            size,
            |b, &size| {
                b.iter(|| {
                    let mut circuit =
    ...
    for size in [10, 100, 1000].iter() {
        group.bench_with_input(BenchmarkId::new("text_netlist", size), size, |b, &size| {
            b.iter_batched(
                ...
                |circuit| black_box(circuit.generate_text_netlist().unwrap()),
                criterion::BatchSize::SmallInput,
            )
        });
    }
...
fn bench_reference_manager(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        group.bench_with_input(
            BenchmarkId::new("reference_generation", size),
            size,
            |b, &size| {
                b.iter_batched(
        ...
        group.bench_with_input(
            BenchmarkId::new("reference_validation", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
...
fn bench_net_operations(c: &mut Criterion) {
    ...
    group.bench_function("net_creation", |b| {
        b.iter(|| Net::new(Some(black_box("VCC".to_string()))).unwrap())
    });

    group.bench_function("net_with_auto_name", |b| b.iter(|| Net::new(None).unwrap()));
...
fn bench_bulk_operations(c: &mut Criterion) {
    ...
    for size in [100, 1000].iter() {
        group.bench_with_input(
            BenchmarkId::new("bulk_component_addition", size),
            size,
            |b, &size| {
                b.iter_batched(
                    || {
