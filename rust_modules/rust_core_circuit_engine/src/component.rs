...
impl Component {
    ...
    pub fn new(
        ...
        if symbol.trim().is_empty() {
            return Err(
                ValidationError::InvalidSymbol("Symbol cannot be empty".to_string()).into(),
            );
        }
        ...
        component.load_symbol_data().map_err(|e| {
            ComponentError::InvalidSymbol(format!("Failed to load symbol data: {}", e))
        })?;

        log::debug!(
            "Created component {} with symbol {}",
    ...
    pub fn to_dict(&self) -> HashMap<String, PyObject> {
        Python::with_gil(|py| {
            ...
            dict.insert(
                "reference".to_string(),
                self.reference.clone().unwrap_or_default().to_object(py),
            );
            dict.insert(
                "value".to_string(),
                self.value.clone().unwrap_or_default().to_object(py),
            );
            dict.insert(
                "footprint".to_string(),
                self.footprint.clone().unwrap_or_default().to_object(py),
            );
            dict.insert(
                "datasheet".to_string(),
                self.datasheet.clone().unwrap_or_default().to_object(py),
            );
            dict.insert(
                "description".to_string(),
                self.description.clone().unwrap_or_default().to_object(py),
            );
    ...
    fn __repr__(&self) -> String {
        format!(
            "Component(symbol='{}', reference={:?}, pins={})",
            self.symbol,
            self.reference,
            self.pins.len()
        )
    ...
    fn __getattr__(&self, name: &str) -> PyResult<String> {
        if let Some(value) = self.extra_fields.get(name) {
            ...
            Err(PyAttributeError::new_err(format!(
                "'Component' object has no attribute '{}'",
                name
            )))
        }
    }
...
impl Component {
    ...
    fn load_symbol_data(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let pins_data = match self.get_pins_from_cache(&library, &name) {
            Ok(pins) if !pins.is_empty() => pins,
            Ok(_) => {
                return Err(ComponentError::InvalidSymbol(format!(
                    "Symbol '{}' not found in cache or has no pins",
                    self.symbol
                ))
                .into());
            }
            Err(e) => {
                return Err(ComponentError::InvalidSymbol(format!(
                    "Failed to load symbol data for {}: {}",
                    self.symbol, e
                ))
                .into());
            }
    ...
    fn get_pins_from_cache(
        &self,
        library: &str,
        name: &str,
    ) -> Result<Vec<(String, String, PinType)>, Box<dyn std::error::Error>> {
        ...
        Python::with_gil(|py| {
            ...
            match cache.call_method0("get_all_libraries") {
                Ok(all_libraries_py) => {
                    let library_count =
                        if let Ok(dict) = all_libraries_py.downcast::<pyo3::types::PyDict>() {
                            dict.len()
                        } else {
                            0
                        };
            ...
            match cache.call_method1("search_symbols", (name,)) {
                Ok(search_results_py) => {
                    let search_results: Vec<String> =
                        search_results_py.extract().unwrap_or_default();
                    ...
                    for result in &search_results {
                        if result == &symbol_name {
                            match cache.call_method1("get_symbol_data", (result,)) {
                                Ok(symbol_data_py) => {
                                    match self
                                        .extract_pins_from_python_symbol_data(py, symbol_data_py)
                                    {
                                        Ok(pins) => return Ok(pins),
                                        Err(e) => {
    ...
    fn extract_pins_from_python_symbol_data(
        &self,
        py: Python,
        symbol_data_py: &PyAny,
    ) -> Result<Vec<(String, String, PinType)>, Box<dyn std::error::Error>> {
        ...
        if let Ok(pins_dict) = pins_py.downcast::<pyo3::types::PyDict>() {
            for (pin_num_py, pin_data_py) in pins_dict.iter() {
                ...
                let pin_name = match pin_data_py.getattr("name") {
                    Ok(name_py) => name_py
                        .extract::<String>()
                        .unwrap_or_else(|_| pin_number.clone()),
                ...
                let pin_type = match pin_data_py.getattr("pin_type") {
                    Ok(type_py) => {
                        let type_str: String =
                            type_py.extract().unwrap_or_else(|_| "passive".to_string());
            ...
            for (i, pin_data_py) in pins_list.iter().enumerate() {
                ...
                let pin_number = if let Ok(dict) = pin_data_py.downcast::<pyo3::types::PyDict>() {
                    ...
                    for (key, value) in dict.iter() {
                        ...
                        let value_str = value
                            .extract::<String>()
                            .unwrap_or_else(|_| format!("non-string:{:?}", value));
                    }
                    ...
                    match dict.get_item("number") {
                        Ok(Some(num_py)) => {
                            let num_str = num_py
                                .extract::<String>()
                                .unwrap_or_else(|_| format!("extract_err_{}", i));
                    ...
                    match pin_data_py.getattr("number") {
                        Ok(num_py) => {
                            let num_str = num_py
                                .extract::<String>()
                                .unwrap_or_else(|_| format!("attr_extract_err_{}", i));
                ...
                let pin_name = if let Ok(dict) = pin_data_py.downcast::<pyo3::types::PyDict>() {
                    ...
                    match dict.get_item("name") {
                        Ok(Some(name_py)) => {
                            let name_str = name_py
                                .extract::<String>()
                                .unwrap_or_else(|_| pin_number.clone());

                            name_str
                        }
                        Ok(None) => pin_number.clone(),
                        Err(e) => pin_number.clone(),
                    }
                } else {
                    // Attribute access
