...
impl PinType {
    ...
    fn from_string(value: &str) -> PyResult<Self> {
        match value.to_lowercase().as_str() {
            ...
            _ => {
                Err(ValidationError::TypeValidation(format!("Invalid pin type: {}", value)).into())
            }
        }
    }
    ...
    fn connect_to_net(&mut self, net: &mut Net) -> PyResult<()> {
        ...
        if let Some(existing_pins) = net.get_pins() {
            if !existing_pins.is_empty() {
                ...
                if !self.func.can_connect_to(&first_pin_type)
                    && !first_pin_type.can_connect_to(&self.func)
                {
    ...
    fn __iadd__(&mut self, other: &PyAny) -> PyResult<()> {
        Python::with_gil(|py| {
            ...
            if let Ok(other_pin) = other.extract::<PyRef<Pin>>() {
                ...
                if !self.func.can_connect_to(&other_pin.func)
                    && !other_pin.func.can_connect_to(&self.func)
                {
                    return Err(PinError::IncompatibleTypes(
                        self.func.to_string(),
            ...
            Err(
                PinError::InvalidType(format!("Cannot add {} to Pin", other.get_type().name()?))
                    .into(),
            )
        })
    }
