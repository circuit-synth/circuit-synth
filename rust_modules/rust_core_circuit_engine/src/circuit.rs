...
impl Circuit {
    ...
    fn add_component(&mut self, mut component: Component) -> PyResult<()> {
        ...
        if has_trailing_digits {
            ...
            self.reference_manager.register_reference(user_ref.clone())?;
            ...
            self.prefix_components.insert(placeholder_key.clone(), component);
    ...
    fn finalize_references(&mut self) -> PyResult<()> {
        log::debug!("Starting reference finalization for circuit '{}'", self.name);
        ...
        for (final_ref, component) in newly_assigned {
            ...
            if let Some(pos) = self.component_list.iter().position(|k| to_remove.contains(&k)) {
        ...
        log::debug!("Finished reference finalization for circuit '{}', total components: {}", 
                   self.name, self.components.len());
    ...
    fn add_subcircuit(&mut self, mut subcircuit: Circuit) -> PyResult<()> {
        ...
        log::debug!("Added subcircuit '{}' to circuit '{}'", 
                   self.subcircuits.last().unwrap().name, self.name);
        Ok(())
    }
    ...
    fn generate_text_netlist(&self) -> PyResult<String> {
        ...
        if self.component_count() > 0 {
            for component in self.get_components() {
                lines.push(format!("  {} {}", component.symbol, component.get_display_reference()));
            }
        } else {
        ...
        if !self.nets.is_empty() {
            for net in self.nets.values() {
                lines.push(format!("  Net {} (pins={})", net.get_name(), net.pin_count()));
            }
        } else {
            lines.push("  (none)".to_string());
        }
    ...
    fn to_dict(&self) -> HashMap<String, PyObject> {
        Python::with_gil(|py| {
            ...
            // Components
            let components: Vec<HashMap<String, PyObject>> = self.get_components()
                .iter()
            ...
            // Nets
            let nets: Vec<HashMap<String, PyObject>> = self.nets.values()
                .map(|net| net.to_dict().unwrap_or_default())
                .collect();
            dict.insert("nets".to_string(), nets.to_object(py));
            
            // Subcircuits
            let subcircuits: Vec<HashMap<String, PyObject>> = self.subcircuits
                .iter()
                .map(|sub| sub.to_dict())
                .collect();
            ...
            // Statistics
            dict.insert("component_count".to_string(), self.component_count().to_object(py));
            dict.insert("net_count".to_string(), self.net_count().to_object(py));
            dict.insert("subcircuit_count".to_string(), self.subcircuit_count().to_object(py));
    ...
    fn get_stats(&self) -> HashMap<String, PyObject> {
        Python::with_gil(|py| {
            ...
            stats.insert("component_count".to_string(), self.component_count().to_object(py));
            stats.insert("net_count".to_string(), self.net_count().to_object(py));
            stats.insert("subcircuit_count".to_string(), self.subcircuit_count().to_object(py));
            stats.insert("prefix_components".to_string(), self.prefix_components.len().to_object(py));
            stats.insert("final_components".to_string(), self.components.len().to_object(py));
    ...
    fn __str__(&self) -> String {
        format!("Circuit('{}', components={}, nets={})", 
                self.name, self.component_count(), self.net_count())
    }
