...
impl ReferenceManager {
    ...
    fn set_initial_counters(&mut self, counters: HashMap<String, u32>) -> PyResult<()> {
        for (prefix, start_num) in counters {
            ...
            if start_num > current {
                ...
                log::debug!("Set initial counter for prefix '{}' to {}", prefix, start_num);
            }
        }
        Ok(())
    }
    ...
    fn get_prefix_counters(&self) -> HashMap<String, u32> {
        self.prefix_counters.iter().map(|(k, v)| (k.clone(), *v)).collect()
    }
    ...
    pub fn get_stats(&self) -> HashMap<String, PyObject> {
        Python::with_gil(|py| {
            ...
            stats.insert("total_references".to_string(), self.used_references.len().to_object(py));
            stats.insert("prefix_count".to_string(), self.prefix_counters.len().to_object(py));
            stats.insert("unnamed_net_counter".to_string(), self.unnamed_net_counter.to_object(py));
            stats.insert("has_parent".to_string(), self.parent.is_some().to_object(py));
    ...
    fn to_dict(&self) -> HashMap<String, PyObject> {
        Python::with_gil(|py| {
            ...
            dict.insert("used_references".to_string(), 
                       self.used_references.iter().cloned().collect::<Vec<_>>().to_object(py));
            dict.insert("prefix_counters".to_string(), 
                       self.get_prefix_counters().to_object(py));
            dict.insert("unnamed_net_counter".to_string(), 
...
impl ReferenceManager {
    ...
    pub fn generate_reference_fast(&mut self, prefix: &str) -> Option<String> {
        ...
        for _ in 0..1000 {  // Limit attempts
            let candidate = format!("{}{}", prefix, counter_value);
            counter_value += 1;
            
            if self.validate_reference_fast(&candidate) {
                self.prefix_counters.insert(prefix.to_string(), counter_value);
