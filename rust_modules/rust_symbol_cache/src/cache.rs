...
impl crate::SymbolLibCache {
            ...
            match self.load_from_disk_cache(&lib_path) {
                Ok(cached_data) => {
                    if self.is_cache_valid(&cached_data.metadata)? {
                        info!(
                            "✓ Loaded library from valid disk cache: {} ({} symbols)",
                            lib_name,
                            cached_data.symbols.len()
                        );
                        let arc_data = Arc::new(cached_data);
                        self.inner
                            .library_cache
                            .insert(lib_name.to_string(), arc_data.clone());
                        return Ok(arc_data);
                    } else {
    ...
    fn parse_library_fresh(&self, lib_path: &Path, lib_name: &str) -> Result<Arc<LibraryData>> {
        ...
        if self.inner.config.enabled {
            ...
            match self.save_to_disk_cache(&arc_data) {
                Ok(()) => {
                    info!(
                        "✓ Successfully saved library cache: {} ({} symbols)",
                        lib_name,
                        arc_data.symbols.len()
                    );
                }
                Err(e) => {
    ...
    fn parse_symbols_parallel(&self, lib_path: &Path) -> Result<HashMap<String, SymbolData>> {
        ...
        let all_symbols: HashMap<String, String> = symbol_texts
            .iter()
    ...
    pub(crate) fn get_symbol_directories(&self) -> Vec<PathBuf> {
        ...
        if let Ok(kicad_dir) = std::env::var("KICAD_SYMBOL_DIR") {
            ...
            for dir_str in kicad_dir.split(separator) {
                ...
                info!(
                    "🔍 [GET_SYMBOL_DIRS] Checking env dir: {}",
                    dir_path.display()
                );
                if dir_path.exists() && dir_path.is_dir() {
                    info!(
                        "✅ [GET_SYMBOL_DIRS] Found valid env dir: {}",
                        dir_path.display()
                    );
                    dirs.push(dir_path);
                } else {
                    info!(
                        "❌ [GET_SYMBOL_DIRS] Env dir not found/invalid: {}",
                        dir_path.display()
                    );
                }
            }
        ...
        if dirs.is_empty() {
            ...
            for dir_str in &default_dirs {
                ...
                info!(
                    "🔍 [GET_SYMBOL_DIRS] Checking default dir: {}",
                    dir_path.display()
                );
                if dir_path.exists() && dir_path.is_dir() {
                    info!(
                        "✅ [GET_SYMBOL_DIRS] Found valid default dir: {}",
                        dir_path.display()
                    );
                    dirs.push(dir_path);
                    break;
                    ...
                    info!(
                        "❌ [GET_SYMBOL_DIRS] Default dir not found/invalid: {}",
                        dir_path.display()
                    );
                }
            }
        ...
        if dirs.is_empty() {
            ...
            for dir_str in &additional_dirs {
                let dir_path = PathBuf::from(dir_str);
                info!(
                    "🔍 [GET_SYMBOL_DIRS] Checking additional dir: {}",
                    dir_path.display()
                );
                if dir_path.exists() && dir_path.is_dir() {
                    info!(
                        "✅ [GET_SYMBOL_DIRS] Found valid additional dir: {}",
                        dir_path.display()
                    );
                    dirs.push(dir_path);
                    break;
                    ...
                    info!(
                        "❌ [GET_SYMBOL_DIRS] Additional dir not found/invalid: {}",
                        dir_path.display()
                    );
                }
            }
        ...
        if dirs.is_empty() {
            ...
            warn!(
                "⚠️  [GET_SYMBOL_DIRS] This means the cache cannot be populated with real symbols"
            );
            warn!("⚠️  [GET_SYMBOL_DIRS] The system will rely on embedded fallback symbols");
        } else {
            info!(
                "✅ [GET_SYMBOL_DIRS] Found {} symbol directories:",
                dirs.len()
            );
            for dir in &dirs {
