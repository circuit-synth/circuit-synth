...
impl crate::SymbolLibCache {
    ...
    fn get_all_library_names(&self) -> Vec<String> {
        self.inner.library_index.iter().map(|entry| entry.key().clone()).collect()
    }
    ...
    pub fn search_symbols_regex(
        ...
        let regex = Regex::new(pattern).map_err(|e| {
            crate::SymbolCacheError::Validation {
        ...
        info!("Regex search found {} matches for pattern '{}'", matches.len(), pattern);
        Ok(matches)
    }
    ...
    pub fn get_symbol_suggestions(
        ...
        debug!("Generated {} suggestions for '{}'", suggestions.len(), partial_name);
        Ok(suggestions)
    }
}
