...
impl SymbolLibCache {
    ...
    pub fn with_config(config: CacheConfig) -> Self {
        let symbol_lru = Arc::new(RwLock::new(LruCache::new(
            NonZeroUsize::new(config.max_memory_cache_size).unwrap(),
        )));
    ...
    pub fn validate_cache_directory(&self) -> Result<bool> {
        ...
        if !metadata_file.exists() {
            debug!(
                "Cache metadata file does not exist: {}",
    ...
    pub fn get_symbol_data(&self, symbol_id: &str) -> Result<Arc<SymbolData>> {
        ...
        let symbol = library_data.symbols.get(&sym_name).ok_or_else(|| {
            SymbolCacheError::SymbolNotFound {
    ...
    pub fn get_symbol_data_by_name(&self, symbol_name: &str) -> Result<Arc<SymbolData>> {
        ...
        let entry = self.inner.symbol_index.get(symbol_name).ok_or_else(|| {
            SymbolCacheError::SymbolNotFound {
    ...
    pub fn check_cache_health(&self) -> Result<HashMap<String, serde_json::Value>> {
        ...
        health.insert(
            "directory_valid".to_string(),
            serde_json::Value::Bool(dir_valid),
        );
        ...
        health.insert(
            "cache_files_count".to_string(),
            serde_json::Value::Number(cache_files.into()),
        );
        ...
        health.insert(
            ...
            serde_json::Value::Bool(index_built),
        );
        ...
        let metadata_file = self.inner.config.cache_path.join("cache_metadata.json");
        health.insert(
            "metadata_exists".to_string(),
            serde_json::Value::Bool(metadata_file.exists()),
        );
    ...
    pub fn get_cache_stats(&self) -> HashMap<String, usize> {
        ...
        stats.insert(
            "library_cache_size".to_string(),
            self.inner.library_cache.len(),
        );
        stats.insert(
            "symbol_lru_size".to_string(),
            self.inner.symbol_lru.read().len(),
        );
        stats.insert(
            "symbol_index_size".to_string(),
            self.inner.symbol_index.len(),
        );
        stats.insert(
            "library_index_size".to_string(),
            self.inner.library_index.len(),
        );
        stats.insert(
            "categories_count".to_string(),
            self.inner.category_libraries.len(),
        );
        stats
    }
}
...
pub fn get_global_cache() -> &'static SymbolLibCache {
    GLOBAL_CACHE.get_or_init(|| {
        ...
        if let Err(e) = cache.ensure_index_built() {
            warn!(
                "Failed to build index on global cache initialization: {}",
                e
            );
        }
        cache
...
pub fn force_reinit_global_cache(config: CacheConfig) -> &'static SymbolLibCache {
    ...
    if let Some(existing_cache) = GLOBAL_CACHE.get() {
        ...
        if let Err(e) = existing_cache.rebuild_cache() {
            warn!(
                "Failed to rebuild cache during force reinitialization: {}",
                e
            );
        } else {
        ...
        if let Err(e) = existing_cache.ensure_index_built() {
            warn!(
                "Failed to build index on global cache reinitialization: {}",
                e
            );
        } else {
    ...
    let cache = GLOBAL_CACHE.get_or_init(|| {
        ...
        if let Err(e) = new_cache.ensure_index_built() {
            warn!(
                "Failed to build index on new global cache initialization: {}",
                e
            );
        } else {
