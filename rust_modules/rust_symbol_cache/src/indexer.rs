...
impl crate::SymbolLibCache {
    ...
    pub(crate) fn ensure_index_built(&self) -> Result<()> {
        ...
        if *index_built {
            info!(
                "🔍 [ENSURE_INDEX] Index already built - {} libraries, {} symbols",
        ...
        if self.inner.config.enabled && !self.inner.config.force_rebuild {
            ...
            if self.load_index_from_cache()? {
                info!(
                    "✅ [ENSURE_INDEX] Loaded symbol index from cache: {} libraries, {} symbols",
                    self.inner.library_index.len(),
                    self.inner.symbol_index.len()
                );
        ...
        *index_built = true;
        ...
        info!(
            "✅ [ENSURE_INDEX] Index build complete - {} libraries, {} symbols",
            self.inner.library_index.len(),
            self.inner.symbol_index.len()
        );
    ...
    fn build_complete_index_fresh(&self) -> Result<()> {
        ...
        info!(
            "Scanning symbol libraries in {} directories:",
            symbol_dirs.len()
        );
        ...
        for (lib_name, lib_path, symbols) in library_data {
            ...
            self.inner
                .library_index
                .insert(lib_name.clone(), lib_path.clone());
        ...
        info!(
            "Symbol index built: {} libraries, {} symbols",
            self.inner.library_index.len(),
            self.inner.symbol_index.len()
        );
    ...
    pub(crate) fn build_library_categorization(&self) {
        ...
        for entry in self.inner.library_index.iter() {
            ...
            self.inner
                .library_categories
                .insert(lib_name.clone(), category.clone());
        ...
        let categorized_libs: usize = self
            .inner
            .category_libraries
            .iter()
        ...
        info!(
            "Total: {}/{} libraries categorized",
            categorized_libs, total_libs
        );
    }
    ...
    fn load_index_from_cache(&self) -> Result<bool> {
        ...
        if !cache_file.exists() {
            info!(
                "No symbol index cache file found at: {}",
                cache_file.display()
            );
            return Ok(false);
        }
        ...
        if let Some(lib_index) = cache_data["library_index"].as_object() {
            for (name, path_value) in lib_index {
                if let Some(path_str) = path_value.as_str() {
                    self.inner
                        .library_index
                        .insert(name.clone(), PathBuf::from(path_str));
                }
            }
        }
    ...
    fn save_index_to_cache(&self) -> Result<()> {
        ...
        cache_data.insert(
            ...
            serde_json::Value::Number(self.inner.library_index.len().into()),
        );
        cache_data.insert(
            "symbol_count".to_string(),
            serde_json::Value::Number(self.inner.symbol_index.len().into()),
        );
        ...
        cache_data.insert(
            "library_index".to_string(),
            serde_json::Value::Object(lib_index),
        );
        ...
        cache_data.insert(
            ...
            serde_json::Value::Object(sym_index),
        );
        ...
        info!(
            "✓ Saved symbol index to cache: {} libraries, {} symbols",
            self.inner.library_index.len(),
            self.inner.symbol_index.len()
        );
