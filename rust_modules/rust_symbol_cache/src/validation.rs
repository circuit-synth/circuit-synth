...
impl crate::SymbolLibCache {
    ...
    pub fn validate_symbol_data(&self, symbol: &SymbolData) -> Result<()> {
        ...
        for (idx, pin) in symbol.pins.iter().enumerate() {
            ...
            let pin_count = symbol.pins.iter()
    ...
    pub fn validate_library_file(&self, lib_path: &Path) -> Result<()> {
        ...
        if !content.contains("(kicad_symbol_lib") {
            return Err(SymbolCacheError::Validation {
                message: format!("Invalid KiCad symbol library format: {}", lib_path.display()),
            });
        }
    ...
    pub fn validate_cache_consistency(&self) -> Result<Vec<String>> {
        ...
        for entry in self.inner.library_index.iter() {
            ...
            if !lib_path.exists() {
                issues.push(format!("Library file missing: {} -> {}", lib_name, lib_path.display()));
                continue;
            }
        ...
        for entry in self.inner.symbol_index.iter() {
            ...
            if !self.inner.library_index.contains_key(&index_entry.library_name) {
                orphaned_symbols += 1;
                if orphaned_symbols <= 10 { // Limit reporting to avoid spam
                    issues.push(format!(
        ...
        if orphaned_symbols > 10 {
            issues.push(format!("... and {} more orphaned symbols", orphaned_symbols - 10));
        }
    ...
    pub fn validate_environment(&self) -> Result<Vec<String>> {
        ...
        if let Ok(kicad_dir) = std::env::var("KICAD_SYMBOL_DIR") {
            ...
            for dir_str in dirs {
                ...
                if !dir_path.exists() {
                    warnings.push(format!("KICAD_SYMBOL_DIR contains non-existent path: {}", dir_str));
                } else if !dir_path.is_dir() {
                    warnings.push(format!("KICAD_SYMBOL_DIR contains non-directory path: {}", dir_str));
                }
            }
        } else {
        ...
        if !self.inner.config.cache_path.exists() {
            if let Err(e) = std::fs::create_dir_all(&self.inner.config.cache_path) {
                warnings.push(format!("Cannot create cache directory {}: {}", self.inner.config.cache_path.display(), e));
            }
        } else if !self.inner.config.cache_path.is_dir() {
            warnings.push(format!("Cache path is not a directory: {}", self.inner.config.cache_path.display()));
        }
    ...
    pub fn health_check(&self) -> Result<HealthReport> {
        ...
        report.cache_hit_ratio = stats.get("cache_hits")
        ...
        // Overall health
        report.overall_health = if report.cache_issues.is_empty() && report.environment_warnings.len() <= 1 {
