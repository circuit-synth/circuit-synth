...
use crate::types::{MatchDetails, MatchType, SearchResult as RustSearchResult};
...
impl PyRustSymbolSearcher {
    ...
    fn build_index(&mut self, symbols: &PyDict) -> PyResult<()> {
        ...
        self.searcher.build_index(symbol_map).map_err(|e| {
...
pub fn benchmark_rust_search(
    ...
    searcher.build_index(symbol_map).map_err(|e| {
        PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Failed to build index: {}", e))
    })?;
    ...
    Python::with_gil(|py| {
        ...
        result_dict.set_item(
            "avg_search_time_ms",
            search_time.as_millis() as f64 / (iterations * query_list.len()) as f64,
        )?;
        result_dict.set_item("total_searches", iterations * query_list.len())?;
        result_dict.set_item("total_results", total_results)?;
        result_dict.set_item(
            "searches_per_second",
            (iterations * query_list.len()) as f64 / search_time.as_secs_f64(),
        )?;
...
fn _rust_symbol_search(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add(
        "__description__",
        "High-performance Rust-based symbol search engine",
    )?;
...
mod tests {
    ...
    fn test_benchmark_function() {
        Python::with_gil(|py| {
            let symbols = [("R", "Device"), ("C", "Device"), ("L", "Device")].into_py_dict(py);
