...
fn test_basic_placement() {
    ...
    let connections = vec![Connection::new("R1".to_string(), "R2".to_string())];
...
fn test_boundary_constraints() {
    ...
    let result = placer
        .place(components, connections, board_width, board_height)
        .unwrap();

    // All components should be within board boundaries
    for (reference, position) in &result.positions {
        assert!(
            position.x >= 0.0 && position.x <= board_width,
            "Component {} X position {} outside board width {}",
            reference,
            position.x,
            board_width
        );
        assert!(
            position.y >= 0.0 && position.y <= board_height,
            "Component {} Y position {} outside board height {}",
            reference,
            position.y,
            board_height
        );
...
fn test_force_calculation_accuracy() {
    ...
    assert!(
        attraction.fx > 0.0,
        "Attraction force should be positive in X direction"
    );
    ...
    assert!(
        repulsion.fx < 0.0,
        "Repulsion force should be negative in X direction"
    );
...
fn test_performance_scaling() {
    ...
    for component_count in [10, 25, 50] {
        let components: Vec<Component> = (0..component_count)
            .map(|i| {
                Component::new(format!("R{}", i), "R_0805".to_string(), "10k".to_string())
                    .with_position(Point::new((i as f64 % 10.0) * 3.0, (i as f64 / 10.0) * 3.0))
            })
            .collect();
...
fn test_energy_minimization() {
    ...
    let initial_energy =
        force_calculator.calculate_system_energy(&components, &connections, &connection_graph);
    ...
    assert!(
        final_distance < 50.0,
        "Connected components should be closer: {} mm",
        final_distance
    );
}
...
fn test_rotation_optimization() {
    ...
    let connections = vec![Connection::new("U1".to_string(), "R1".to_string())];
    ...
    assert!(
        *u1_rotation % 360.0 == 0.0 || *u1_rotation % 90.0 == 0.0,
        "Rotation should be multiple of 90 degrees: {}",
        u1_rotation
    );
    assert!(
        *r1_rotation % 360.0 == 0.0 || *r1_rotation % 90.0 == 0.0,
        "Rotation should be multiple of 90 degrees: {}",
        r1_rotation
    );
}
