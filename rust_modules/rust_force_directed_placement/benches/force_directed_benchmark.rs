    ...
    (0..count)
        .map(|i| {
            Component::new(format!("R{}", i), "R_0805".to_string(), "10k".to_string())
                .with_position(Point::new((i as f64 % 10.0) * 5.0, (i as f64 / 10.0) * 5.0))
                .with_size(2.0, 1.0)
        })
        .collect()
}
...
fn generate_test_connections(component_count: usize) -> Vec<Connection> {
    ...
    for i in 0..(component_count - 1) {
        connections.push(Connection::new(format!("R{}", i), format!("R{}", i + 1)));
    }

    // 2. Some random cross-connections
    for i in (0..component_count).step_by(5) {
        if i + 10 < component_count {
            connections.push(Connection::new(format!("R{}", i), format!("R{}", i + 10)));
        }
    }
...
fn bench_collision_detection(c: &mut Criterion) {
    ...
    for component_count in [10, 25, 50, 100, 200, 500].iter() {
        ...
        group.bench_with_input(
            ...
            component_count,
            |b, _| {
                b.iter(|| detector.detect_collisions(black_box(&components)));
            },
        );
        ...
        group.bench_with_input(
            ...
            component_count,
            |b, _| {
                b.iter(|| detector.detect_collisions_parallel(black_box(&components)));
            },
        );
    }
...
fn bench_complete_placement(c: &mut Criterion) {
    ...
    for component_count in [10, 25, 50, 100, 200].iter() {
        ...
        for (config_name, config) in configs {
            group.bench_with_input(
                BenchmarkId::new(
                    format!("{}_{}", config_name, component_count),
                    component_count,
                ),
                component_count,
                |b, _| {
