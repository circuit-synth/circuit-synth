...
use rust_io_processor::{
    JsonLoader, JsonSerializer, KiCadParser, ValidationEngine, AsyncFileOps,
...
fn benchmark_json_processing(c: &mut Criterion) {
    ...
    for &size in &[SMALL_CIRCUIT_COMPONENTS, MEDIUM_CIRCUIT_COMPONENTS, LARGE_CIRCUIT_COMPONENTS] {
        ...
        group.bench_with_input(
            ...
            |b, (circuit, path)| {
                let serializer = JsonSerializer::new();
                b.to_async(&rt).iter(|| async {
                    black_box(serializer.save_circuit_to_json_file(circuit, path).await.unwrap())
                });
            },
        );
        ...
        rt.block_on(async {
            let serializer = JsonSerializer::new();
            serializer.save_circuit_to_json_file(&circuit_data, &file_path).await.unwrap();
        });
...
fn benchmark_kicad_parsing(c: &mut Criterion) {
    ...
    for &size in &[SMALL_CIRCUIT_COMPONENTS, MEDIUM_CIRCUIT_COMPONENTS, LARGE_CIRCUIT_COMPONENTS] {
        ...
        let dir = tempdir().unwrap();
        let sch_path = dir.path().join(format!("test_schematic_{}.kicad_sch", size));
        ...
        group.bench_with_input(
            ...
            |b, path| {
                ...
                b.to_async(&rt).iter(|| async {
                    black_box(parser.parse_schematic(path).await.unwrap())
                });
            },
        );
...
fn benchmark_file_io(c: &mut Criterion) {
    ...
    for (size_name, size_bytes) in file_sizes {
        ...
        rt.block_on(async {
            let file_ops = AsyncFileOps::new();
            file_ops.writer().write_file(&file_path, &test_data).await.unwrap();
        });
...
fn benchmark_batch_operations(c: &mut Criterion) {
    ...
    for &batch_size in &batch_sizes {
        ...
        let dir = tempdir().unwrap();
        let file_paths: Vec<_> = circuits.iter().enumerate().map(|(i, circuit)| {
            let path = dir.path().join(format!("batch_circuit_{}.json", i));
            rt.block_on(async {
                let serializer = JsonSerializer::new();
                serializer.save_circuit_to_json_file(circuit, &path).await.unwrap();
            });
            path
        }).collect();
        ...
        group.bench_with_input(
            ...
            |b, paths| {
                let loader = JsonLoader::new();
                b.to_async(&rt).iter(|| async {
                    black_box(loader.load_circuits_batch(paths.clone()).await)
                });
            },
        );
        
        // Benchmark batch saving
        let save_data: Vec<_> = circuits.iter().enumerate().map(|(i, circuit)| {
        ...
        group.bench_with_input(
            ...
            |b, (circuit, path)| {
                b.to_async(&rt).iter(|| async {
                    ...
                    let validation_result = validator.validate_circuit_data(&loaded_circuit).await.unwrap();
                    
                    // 4. Convert to JSON and back (simulating processing)
                    let json_value = serde_json::to_value(&loaded_circuit).unwrap();
                    let _processed_circuit: CircuitData = serde_json::from_value(json_value).unwrap();
