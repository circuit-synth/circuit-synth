...
pub use file_io::{AsyncFileOps, FileReader, FileWriter};
pub use json_processor::{CircuitData, JsonLoader};
pub use kicad_parser::{KiCadParser, PcbData, SchematicData};
...
fn rust_io_processor(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(
        python_bindings::load_circuit_from_dict,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        python_bindings::load_circuit_from_json_file,
        m
    )?)?;
