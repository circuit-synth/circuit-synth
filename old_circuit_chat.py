...
"""
    ...
    def _analyze_user_request(self, message: str) -> Dict[str, Any]:
        """Analyze user request to determine circuit type and requirements"""
        message_lower = message.lower()
        ...
        if "esp32" in message_lower:
            if "s3" in message_lower:
                ...
                analysis["questions"].append("Which ESP32 variant? (ESP32-S3, ESP32-C6, or classic ESP32)")
        elif "stm32" in message_lower:
            if "f4" in message_lower:
                analysis["mcu_type"] = "STM32F4"
            elif "f411" in message_lower:
                analysis["mcu_type"] = "STM32F411"
            else:
                analysis["mcu_type"] = "STM32"
                analysis["questions"].append("Which STM32 family? (STM32F4, STM32G4, STM32H7)")
        
        # Detect USB requirement
        if any(term in message_lower for term in ["usb", "usb-c", "programming", "debug"]):
            analysis["needs_usb"] = True
            analysis["needs_power_reg"] = True  # USB is 5V, MCUs are 3.3V
        
        # Detect peripherals
        peripheral_map = {
            "imu": "IMU sensor (MPU-6050)",
            "mpu": "IMU sensor (MPU-6050)", 
            "accelerometer": "IMU sensor (MPU-6050)",
            "gyroscope": "IMU sensor (MPU-6050)",
        ...
        for keyword, description in peripheral_map.items():
            if keyword in message_lower:
                analysis["peripherals"].append(description)
        
        # Determine project type for hierarchical generation
        if analysis["mcu_type"] and ("esp32" in analysis["mcu_type"].lower()):
            if analysis["peripherals"]:
                analysis["circuit_type"] = "esp32_complete_board"
            else:
                analysis["circuit_type"] = "esp32_complete_board"
        elif analysis["mcu_type"] and ("stm32" in analysis["mcu_type"].lower()):
            analysis["circuit_type"] = "stm32_complete_board"
        
        return analysis
    
    def _ask_clarifying_questions(self, analysis: Dict[str, Any], user_message: str) -> str:
        """Generate clarifying questions based on analysis"""
        questions = analysis["questions"].copy()
        
        # Add power questions for USB circuits
        if analysis["needs_usb"] and not any("power" in q.lower() for q in questions):
            questions.append("Power input: USB-C only, or also external DC jack?")
        
        # Add peripheral specification questions
        if "Sensors" in analysis["peripherals"]:
            questions.append("What type of sensors? (IMU, temperature, pressure, etc.)")
        
        if "Motor control" in analysis["peripherals"]:
            questions.append("What motor type? (Stepper, DC brushed, servo)")
        
        if not questions:
            return None
        
        # Format questions nicely
        question_text = "I need a few details to design the best circuit for you:\n\n"
        for i, question in enumerate(questions, 1):
            question_text += f"{i}. {question}\n"
        
        question_text += f"\nOr just say 'use defaults' and I'll make good engineering choices for you!"
