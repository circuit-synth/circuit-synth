...
def main():
    ...
    sorted_libs = sorted(
        library_stats.items(), key=lambda x: x[1]["footprint_count"], reverse=True
    )
    ...
    print(
        f"Mixed footprints: {total_mixed:,} ({total_mixed/total_footprints*100:.1f}%)"
    )
    ...
    smd_only_libs = [
        ...
        if stats["smd_count"] == stats["footprint_count"]
        and stats["footprint_count"] > 0
    ]
    if smd_only_libs:
    ...
    tht_only_libs = [
        (lib, stats)
        for lib, stats in sorted_libs
        if stats["tht_count"] == stats["footprint_count"]
        and stats["footprint_count"] > 0
    ]
    if tht_only_libs:
