...
def example_1_quick_search():
    ...
    capacitors = search_digikey_components(
        "100nF 0603 capacitor", max_results=5, in_stock_only=True
    )
...
def example_2_detailed_search():
    ...
    stm32_parts = searcher.search_components(
        keyword="STM32F407", max_results=5, in_stock_only=True
    )
...
def example_3_find_alternatives():
    ...
    resistors = searcher.search_components(
        keyword="10k 0603 resistor", max_results=1, in_stock_only=True
    )
    ...
    if resistors:
        ...
        alternatives = searcher.find_alternatives(
            reference_component=reference, max_results=5
        )
...
def example_5_cache_management():
    ...
    print("\nCache Statistics:")
    print(
        f"  Search cache: {stats['search_cache']['valid_files']} valid, "
        f"{stats['search_cache']['expired_files']} expired"
    )
    print(
        f"  Product cache: {stats['product_cache']['valid_files']} valid, "
