...
def main():
    ...
    for comp in components:
        try:
            ...
            print(f"   ✓ Added {comp['reference']}: {comp['value']} at ({comp['x']}, {comp['y']})")
    ...
    for label in labels:
        try:
            ...
            print(f"   ✓ Added label '{label['name']}' ({label['shape']}) at ({label['x']}, {label['y']})")
        except Exception as e:
    ...
    try:
        result = subprocess.run(
            ["kicad-cli", "sch", "export", "pdf", output_file, "--output", "voltage_divider_circuit.pdf"],
            capture_output=True,
            text=True,
            timeout=10
        )
    ...
    print(f"  • Components: {len(components)} ({', '.join([c['reference'] for c in components])})")
