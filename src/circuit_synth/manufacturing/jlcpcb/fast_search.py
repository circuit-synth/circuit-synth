...
class FastJLCSearch:
    ...
    def search(
        ...
        logger.info(f"Search completed in {elapsed:.2f}s: {len(results)} results for '{query}'")
    ...
    def find_cheapest(self, query: str, min_stock: int = 100) -> Optional[FastSearchResult]:
    ...
    def _perform_search(self, query: str, category: Optional[str]) -> List[FastSearchResult]:
    ...
    def _convert_to_fast_result(self, raw_data: Dict, query: str) -> Optional[FastSearchResult]:
    ...
    def _calculate_match_score(self, description: str, part_number: str, query: str) -> float:
    ...
    def _sort_results(self, results: List[FastSearchResult], sort_by: str) -> List[FastSearchResult]:
...
def fast_jlc_search(
    ...
    **kwargs
    ...
    return searcher.search(query, min_stock=min_stock, max_results=max_results, **kwargs)
