...
class DigiKeyComponentSearch:
    ...
    def _parse_product(self, product_data: Dict[str, Any]) -> DigiKeyComponent:
        ...
        digikey_pn = product_data.get("DigiKeyPartNumber", product_data.get("DigiKeyProductNumber", ""))
        manufacturer_pn = product_data.get("ManufacturerPartNumber", product_data.get("ManufacturerProductNumber", ""))
        ...
        if isinstance(manufacturer_field, dict):
            manufacturer = manufacturer_field.get("Value", manufacturer_field.get("Name", "Unknown"))
        else:
        ...
        if isinstance(description_field, dict):
            description = description_field.get("Value", description_field.get("ProductDescription", ""))
        else:
            description = str(description_field) if description_field else ""
        
        # Extract availability
        qty_available = product_data.get("QuantityAvailable", 0)
        qty_on_hand = product_data.get("QuantityOnHand", qty_available)  # Use qty_available as fallback
        min_qty = product_data.get("MinimumOrderQuantity", product_data.get("MinimumQuantity", 1))
        ...
        price_breaks = []
        for pb in product_data.get("PriceBreaks", product_data.get("StandardPricing", [])):
            price_breaks.append({
        ...
        category_field = product_data.get("Category", product_data.get("ProductCategory", {}))
        if isinstance(category_field, dict):
            category = category_field.get("Value", category_field.get("Name", "Unknown"))
        else:
        ...
        parameters = {}
        for param in product_data.get("Parameters", product_data.get("ProductParameters", [])):
    ...
    def get_component_details(self, digikey_part_number: str) -> Optional[DigiKeyComponent]:
        """
    ...
    def find_alternatives(
        self,
        reference_component: DigiKeyComponent,
        max_results: int = 10
    ) -> List[DigiKeyComponent]:
        """
