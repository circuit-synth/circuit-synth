...
def test_digikey_connection():
    ...
    try:
        ...
        print(f"\nConfiguration:")
        print(f"  Client ID: {config_dict['client_id'][:10]}..." if len(config_dict['client_id']) > 10 else f"  Client ID: {config_dict['client_id']}")
    ...
    try:
        ...
        if results and "Products" in results:
            ...
            if product_count > 0:
                ...
                print(f"  Description: {product.get('Description', {}).get('Value', 'N/A')}")
