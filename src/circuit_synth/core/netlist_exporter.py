...
class NetlistExporter:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        print(f"\n🔍 HIERARCHICAL STRUCTURE DEBUG - Starting to_dict() for circuit: {self.circuit.name}")
        print(f"🔍 Circuit has {len(self.circuit._components)} components and {len(self.circuit._nets)} nets")
        print(f"🔍 Circuit has {len(self.circuit._subcircuits)} subcircuits")
        
        # Log subcircuit hierarchy
        if self.circuit._subcircuits:
            print(f"🔍 Subcircuits found:")
            for i, subcirc in enumerate(self.circuit._subcircuits):
                print(f"🔍   {i+1}. {subcirc.name} (components: {len(subcirc._components)}, nets: {len(subcirc._nets)})")
        else:
            print(f"🔍 No subcircuits in this circuit")
        ...
        print(f"🔍 HIERARCHICAL STRUCTURE - Creating sheet structure:")
        print(f"🔍   Sheet tstamps: {sheet_tstamps}")
        ...
        print(f"🔍 HIERARCHICAL STRUCTURE - Processing {len(self.circuit._subcircuits)} subcircuits:")
        for i, sc in enumerate(self.circuit._subcircuits):
            print(f"🔍   Processing subcircuit {i+1}: {sc.name}")
            exporter = NetlistExporter(sc)
            subcircuit_data = exporter.to_dict()
            data["subcircuits"].append(subcircuit_data)
            print(f"🔍   Subcircuit {sc.name} added to hierarchy")
            
        print(f"🔍 HIERARCHICAL STRUCTURE - Final data structure for {self.circuit.name}:")
        print(f"🔍   Components: {len(data['components'])} - {list(data['components'].keys())}")
        print(f"🔍   Nets: {len(data['nets'])} - {list(data['nets'].keys())}")
        print(f"🔍   Subcircuits: {len(data['subcircuits'])} - {[sub['name'] for sub in data['subcircuits']]}")
        print(f"🔍   Sheet info: {data['tstamps']} -> {data['source_file']}")
        print(f"🔍 HIERARCHICAL STRUCTURE DEBUG COMPLETE for {self.circuit.name}\n")
