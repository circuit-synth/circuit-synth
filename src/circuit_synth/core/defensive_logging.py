...
DEFENSIVE_LOG_FORMAT = '%(asctime)s | %(levelname)-8s | DEFENSIVE-%(name)s | %(message)s'
...
class ComponentMetrics:
    ...
    def avg_rust_time(self) -> float:
        ...
        return self.total_rust_time / self.rust_successes if self.rust_successes > 0 else 0.0
        
    @property
    def avg_python_time(self) -> float:
        """Average Python operation time"""
        return self.total_python_time / self.python_operations if self.python_operations > 0 else 0.0
...
class DefensiveLogger:
    ...
    def __init__(self, component_name: str):
        ...
        # Safety thresholds
        self.max_failure_rate = float(os.environ.get('CIRCUIT_SYNTH_MAX_RUST_FAILURE_RATE', '0.1'))  # 10%
        self.min_operations_for_disable = int(os.environ.get('CIRCUIT_SYNTH_MIN_OPS_FOR_DISABLE', '10'))
        
        # Rust availability check
        self.rust_globally_disabled = os.environ.get('CIRCUIT_SYNTH_DISABLE_RUST', 'false').lower() == 'true'
        ...
        self.logger.info(f"   📊 Min operations for auto-disable: {self.min_operations_for_disable}")
    ...
    def is_rust_available(self) -> bool:
        ...
        if (self.metrics.total_operations >= self.min_operations_for_disable and 
            self.metrics.rust_failure_rate > self.max_failure_rate):
            
            self.rust_auto_disabled = True
            self.logger.warning(f"🚨 AUTO-DISABLING RUST [{self.component_name}]")
            self.logger.warning(f"   📈 Failure rate: {self.metrics.rust_failure_rate:.1%} > {self.max_failure_rate:.1%}")
    ...
    def log_python_success(self, metrics: OperationMetrics, result: Any, **kwargs) -> None:
        ...
        self.logger.info(f"✅ PYTHON SUCCESS [{self.component_name}] {metrics.operation_name}")
    ...
    def log_rust_success(self, metrics: OperationMetrics, result: Any, python_result: Any = None, **kwargs) -> None:
        ...
        if python_result is not None:
            ...
            if checksum == python_checksum:
                ...
                self.logger.info(f"🔍 VALIDATION PASSED: Rust output identical to Python")
            else:
        ...
        self.logger.info(f"✅ RUST SUCCESS [{self.component_name}] {metrics.operation_name}")
        self.logger.info(f"   ⏱️  Duration: {duration:.4f}s")
        self.logger.info(f"   📏 Output size: {result_size} bytes")
        self.logger.info(f"   🔐 Checksum: {checksum[:16]}...")
    ...
    def log_rust_fallback(self, metrics: OperationMetrics, error: Exception, **kwargs) -> None:
        ...
        self.logger.warning(f"⚠️  RUST FALLBACK [{self.component_name}] {metrics.operation_name}")
    ...
    def log_file_validation(self, filepath: Union[str, Path], expected_checksum: Optional[str] = None) -> bool:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'rust_success_rate': 1.0 - self.metrics.rust_failure_rate if self.metrics.rust_successes + self.metrics.rust_failures > 0 else 0.0,
    ...
    def log_performance_summary(self) -> None:
        ...
        if summary['rust_operations'] > 0:
            self.logger.info(f"   📈 Rust success rate: {summary['rust_success_rate']:.1%}")
            self.logger.info(f"   ⚡ Performance improvement: {summary['performance_improvement']:.1f}x")
            self.logger.info(f"   ⏱️  Avg Rust time: {summary['avg_rust_time']:.4f}s")
        
        if summary['python_operations'] > 0:
            self.logger.info(f"   ⏱️  Avg Python time: {summary['avg_python_time']:.4f}s")
