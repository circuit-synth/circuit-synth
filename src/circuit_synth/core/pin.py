...
class PinType(Enum):
    ...
    def from_string(cls, value: str) -> 'PinType':
        ...
        try:
            return next(
                pin_type for pin_type in cls
                if pin_type.value.lower() == value.lower()
            )
...
class Pin:
    ...
    def connect_to_net(self, net: Net):
        ...
        if net._pins:
            ...
            if not (self.func.can_connect_to(other_pin.func) or other_pin.func.can_connect_to(self.func)):
                raise ValueError(
    ...
    def __iadd__(self, other):
        ...
        if isinstance(other, Net):
            ...
            if not (self.func.can_connect_to(other.func) or other.func.can_connect_to(self.func)):
                raise ValueError(
                    f"Cannot connect {self.func} pin to {other.func} pin"
                )
            ...
            if self.net is None and other.net is None:
                ...
                if self.net != other.net:
                    ...
                    for p1 in all_pins:
                        for p2 in all_pins:
                            if p1 != p2 and not (p1.func.can_connect_to(p2.func) or p2.func.can_connect_to(p1.func)):
                                raise ValueError(
