...
class RustAccelerator:
    ...
    def _detect_rust_modules(self):
        ...
        if self.rust_modules:
            logger.info(f"🚀 Rust acceleration active for {len(self.rust_modules)} modules")
        else:
            logger.info("🐍 Using Python implementations (Rust not available)")
    
    def get_rust_function(self, module_name: str, function_name: str) -> Optional[Callable]:
...
def generate_component_sexp(component_data: dict) -> str:
    ...
    rust_func = _accelerator.get_rust_function('kicad_writer', 'generate_component_sexp')
    if not rust_func:
        rust_func = _accelerator.get_rust_function('kicad_writer_standalone', 'generate_component_sexp')
    ...
    if rust_func:
        ...
        from circuit_synth.kicad.sch_gen.s_expression_generator import generate_component_sexp_python
...
def create_symbol_cache():
    ...
    rust_cache_class = _accelerator.get_rust_function('symbol_cache', 'RustSymbolLibCache')
    if not rust_cache_class:
        rust_cache_class = _accelerator.get_rust_function('symbol_cache_standalone', 'RustSymbolLibCache')
...
def create_placement_engine():
    ...
    rust_placer_class = _accelerator.get_rust_function('placement', 'ForceDirectedPlacer')
    if not rust_placer_class:
        rust_placer_class = _accelerator.get_rust_function('placement_standalone', 'ForceDirectedPlacer')
...
if status['rust_available']:
    logger.info(f"🚀 Circuit-synth acceleration: {status['total_modules']} Rust modules active")
else:
