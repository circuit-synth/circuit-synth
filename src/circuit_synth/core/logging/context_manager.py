...
class UserContext:
    ...
    def set_request(self, request_id: Optional[str] = None) -> str:
        ...
        SessionManager.get_instance().add_request_to_session(self.session_id, request_id)
...
class RequestContext:
    ...
    def __enter__(self):
        ...
        if session_id:
            SessionManager.get_instance().add_request_to_session(session_id, self.request_id)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        session_id = current_session.get()
        if session_id:
            SessionManager.get_instance().remove_request_from_session(session_id, self.request_id)
