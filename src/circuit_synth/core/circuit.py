...
class Circuit:
    ...
    def to_spice(self, include_analysis=True) -> str:
        ...
        This method converts the circuit-synth circuit definition into a SPICE netlist
        that can be used with any SPICE-compatible simulator (ngspice, Xyce, etc.).
        
        Args:
            include_analysis: If True, include basic .DC and .AC analysis commands
            
        Returns:
            str: Complete SPICE netlist ready for simulation
            
        Example:
            >>> circuit = my_voltage_divider()
            >>> spice_netlist = circuit.to_spice()
            >>> print(spice_netlist)
            * my_voltage_divider
            * Generated by circuit-synth SPICE exporter
            
            .title my_voltage_divider
        ...
        try:
            ...
            context_logger.error(
                "SPICE export functionality not available", 
                component="CIRCUIT", 
                error=str(e)
            )
            raise ImportError(
                "SPICE export requires the spice module. "
                "Check circuit-synth installation."
            ) from e
        
        context_logger.info(
            "Exporting circuit to SPICE netlist",
            component="CIRCUIT", 
            circuit_name=self.name,
        ...
        netlist = exporter.generate_netlist(include_analysis=include_analysis)
        
        # Validate the generated netlist
        if not exporter.validate_netlist(netlist):
            context_logger.warning(
                "Generated SPICE netlist may have issues",
                component="CIRCUIT",
                circuit_name=self.name
            )
