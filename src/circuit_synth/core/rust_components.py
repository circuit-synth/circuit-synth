...
try:
    ...
    rust_core_path = os.path.join(
        ...
        '../../rust_modules/rust_core_circuit_engine/python'
    )
    if rust_core_path not in sys.path:
        sys.path.insert(0, rust_core_path)
    
    from rust_core_circuit_engine import create_resistor, create_capacitor
    
    _RUST_COMPONENTS_AVAILABLE = True
    context_logger.info("🦀 RUST_COMPONENTS: ✅ Ultra-high-performance Rust component creation loaded", component="RUST_COMPONENTS")
    context_logger.info("🚀 RUST_COMPONENTS: Expected 3-5x component creation speedup", component="RUST_COMPONENTS")
    
except ImportError as e:
    _RUST_COMPONENTS_AVAILABLE = False
    context_logger.info(f"🐍 RUST_COMPONENTS: Not available ({e}), using Python fallback", component="RUST_COMPONENTS")
...
def create_rust_resistor(ref=None, value=None, **kwargs):
    ...
    if _RUST_COMPONENTS_AVAILABLE:
        try:
            ...
            context_logger.debug(f"🦀 Creating Rust resistor {ref}={value}", component="RUST_COMPONENTS")
            ...
            context_logger.warning(f"🔄 Rust resistor creation failed, using Python fallback: {e}", component="RUST_COMPONENTS")
    
    # Python fallback
    from .component import Component
    context_logger.debug(f"🐍 Creating Python resistor {ref}={value}", component="RUST_COMPONENTS")
...
def create_rust_capacitor(ref=None, value=None, **kwargs):
    ...
    if _RUST_COMPONENTS_AVAILABLE:
        try:
            # Use Rust-accelerated component creation
            context_logger.debug(f"🦀 Creating Rust capacitor {ref}={value}", component="RUST_COMPONENTS")
            ...
            context_logger.warning(f"🔄 Rust capacitor creation failed, using Python fallback: {e}", component="RUST_COMPONENTS")
    
    # Python fallback
    from .component import Component
    context_logger.debug(f"🐍 Creating Python capacitor {ref}={value}", component="RUST_COMPONENTS")
...
def get_rust_component_status():
    ...
    return {
        ...
        'acceleration_type': 'Rust PyO3' if _RUST_COMPONENTS_AVAILABLE else 'Python fallback',
