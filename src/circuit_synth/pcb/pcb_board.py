...
class PCBBoard:
    ...
    def auto_place_components(self, algorithm: str = "hierarchical", **kwargs) -> None:
        ...
        if algorithm == "hierarchical":
            ...
            logger.error(
                f"🔥 Starting external placement with {len(self.footprints)} components"
            )
            ...
            for footprint in self.footprints.values():
                ...
                logger.error(
                    f"🔥 BEFORE: {footprint.reference} at ({old_pos.x}, {old_pos.y})"
                )
                footprint.position = Point(0.0, 0.0)
                footprint.rotation = 0.0  # Reset rotation to 0
                logger.error(
                    f"🔥 AFTER: {footprint.reference} at ({footprint.position.x}, {footprint.position.y})"
                )
                updated_count += 1

            logger.error(
                f"🔥 All components positioned at origin: {updated_count} components updated"
            )
            ...
            logger.error(
                f"🔥 Creating cutout rectangle: ({cutout_start_x}, {cutout_start_y}) to ({cutout_end_x}, {cutout_end_y})"
            )
            ...
            for footprint in self.footprints.values():
                logger.error(
                    f"🔥   {footprint.reference}: ({footprint.position.x}, {footprint.position.y})"
                )

            logger.info(
                f"External placement completed: {updated_count} components at origin"
            )
            logger.info(
