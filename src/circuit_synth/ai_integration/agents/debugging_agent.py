...
class CircuitDebuggingAgent:
    ...
    def __init__(self):
        ...
        self.knowledge_base_path = Path(__file__).parent.parent.parent.parent.parent / "debugging_knowledge_base"
    ...
    def analyze_circuit_code(self, circuit_code: str, symptoms: List[str]) -> Dict[str, Any]:
        """
    ...
    def _find_relevant_failures(self, symptom: str) -> List[Dict]:
        ...
        for component_type, data in self.component_failures["component_failure_modes"].items():
                        ...
                        if isinstance(issue_symptoms, list):
                            if any(s.lower() in symptom for s in issue_symptoms):
                                relevant.append({
                                    ...
                                    "fixes": issue_data.get("fixes") or issue_data.get("repair"),
                ...
                if "no_output" in power_debug:
                    ...
                    for step in steps:
                        if isinstance(step, dict):
                            techniques.append(f"Step {step.get('step')}: {step.get('action')} - {step.get('details')}")
                ...
                if "no_ack" in i2c_debug:
                    ...
                    for check in checks:
                        if isinstance(check, dict):
                            techniques.append(f"{check.get('test')}: {check.get('method')}")
    ...
    def generate_test_plan(self, symptoms: List[str], measurements: Dict[str, Any]) -> List[Dict]:
        """
    ...
    def search_web_for_solution(self, symptoms: List[str], board_info: Dict[str, str]) -> str:
        """
        ...
        for symptom in symptoms:
            if "i2c" in symptom.lower():
                search_terms.extend(["I2C debugging", "I2C no ACK", "I2C troubleshooting"])
            elif "power" in symptom.lower():
                search_terms.extend(["power supply debugging", "voltage regulator troubleshooting"])
    ...
    def generate_debug_report(self, 
        ...
        board_info = {"mcu": "extracted_from_circuit"}  # Would extract from circuit analysis
        report["web_search_guidance"] = self.search_web_for_solution(symptoms, board_info)
        
        # Priority recommendations
        report["priority_actions"] = self._generate_priority_actions(symptoms, measurements, all_problems)
        
        return report
    
    def _generate_priority_actions(self, symptoms: List[str], measurements: Dict, problems: List) -> List[str]:
        """Generate prioritized action items"""
        actions = []
        
        # Critical actions first
        if any("not turning on" in s.lower() or "no power" in s.lower() for s in symptoms):
            actions.append("CRITICAL: Verify input power and check for shorts on power rails")
