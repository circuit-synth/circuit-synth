...
class DebugPattern:
    ...
    def matches_symptoms(self, symptoms: List[str], threshold: float = 0.5) -> float:
        ...
        pattern_set = set(word.lower() for symptom in self.symptoms for word in symptom.split())
        symptom_set = set(word.lower() for symptom in symptoms for word in symptom.split())
...
class DebugKnowledgeBase:
    ...
    def _load_default_patterns(self):
        ...
        default_patterns = [
            DebugPattern(
                ...
                category="power",
                symptoms=["3.3V rail reading low", "Board draws excessive current", "Regulator hot"],
                ...
                typical_measurements={"3.3V_rail": 2.8, "regulator_temp_c": 85, "current_draw_ma": 1200}
            ),
            DebugPattern(
                pattern_id=self._generate_pattern_id(["USB", "enumeration", "fail"]),
                category="digital",
                symptoms=["USB device not recognized", "Enumeration fails", "Device descriptor error"],
            ...
            DebugPattern(
                ...
                typical_measurements={"SDA_high": 3.3, "SCL_high": 3.3, "pullup_resistance": 4700}
            ),
            DebugPattern(
                pattern_id=self._generate_pattern_id(["oscillation", "power", "unstable"]),
                category="power",
                symptoms=["Power rail oscillating", "Unstable output voltage", "Audible noise from regulator"],
            ...
            DebugPattern(
                ...
                category="power",
                symptoms=["Component fails after handling", "Intermittent failures", "Input protection damaged"],
                ...
                typical_measurements={"input_impedance": "open", "leakage_current_ua": 1000}
            )
        ]
    ...
    def add_pattern(self, pattern: DebugPattern) -> bool:
        ...
        try:
            ...
            """, (
                ...
                json.dumps(pattern.typical_measurements) if pattern.typical_measurements else None,
    ...
    def search_patterns(self, symptoms: List[str], category: Optional[str] = None, 
                       min_similarity: float = 0.3) -> List[Tuple[DebugPattern, float]]:
        ...
        for row in cursor:
            pattern = DebugPattern(
                ...
                typical_measurements=json.loads(row["typical_measurements"]) if row["typical_measurements"] else None,
                references=json.loads(row["reference_docs"]) if row["reference_docs"] else None
            )
    ...
    def get_similar_sessions(self, board_name: str, symptoms: List[str], limit: int = 5) -> List[Dict[str, Any]]:
