...
class DebugReportGenerator:
    ...
    def _create_title_page(self) -> List[Flowable]:
        ...
        # Session info
        info_data = [
            ['Board:', f"{self.session.get('board_name', 'Unknown')} {self.session.get('board_version', '')}"],
            ['Session ID:', self.session.get('session_id', 'N/A')[:8] + '...'],
            ['Date:', self.session.get('started_at', 'Unknown')],
            ['Status:', 'Resolved' if self.session.get('resolution') else 'In Progress']
        ]
    ...
    def _create_executive_summary(self) -> List[Flowable]:
        ...
        metrics_data = [
            ...
            ['Resolution Status:', 'Complete' if self.session.get('resolution') else 'Ongoing']
        ]
    ...
    def _create_symptoms_section(self) -> List[Flowable]:
        ...
        if symptoms:
            ...
            elements.append(Paragraph("No symptoms reported", self.styles['CustomBody']))
        ...
        if measurements:
            ...
            for name, data in measurements.items():
                table_data.append([
                    ...
                    str(data.get('value', '')),
                    data.get('unit', ''),
                    data.get('notes', '')[:30] + '...' if len(data.get('notes', '')) > 30 else data.get('notes', '')
                ])
            
            # Create table
            table = Table(table_data, colWidths=[2*inch, 1*inch, 0.7*inch, 2.8*inch])
            ...
            elements.append(Paragraph("No measurements recorded", self.styles['CustomBody']))
    ...
    def _create_issues_section(self) -> List[Flowable]:
        ...
        for i, issue in enumerate(issues, 1):
            ...
            if issue.get('test_suggestions'):
                elements.append(Paragraph("Recommended Tests:", self.styles['BodyText']))
        ...
        if not issues:
            elements.append(Paragraph("No issues identified", self.styles['CustomBody']))
    ...
    def _create_resolution_section(self) -> List[Flowable]:
        ...
        if self.session.get('resolution'):
            ...
            elements.append(Paragraph("Session still in progress", self.styles['CustomBody']))
...
def generate_pdf_from_session(session_file: str, output_file: str = "debug_report.pdf") -> str:
