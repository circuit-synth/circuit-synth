...
class TestMeasurement:
    ...
    def evaluate(self) -> bool:
        ...
        if isinstance(self.value, (int, float)) and isinstance(self.expected_value, (int, float)):
    ...
    def analyze_waveform(self) -> Dict[str, Any]:
        ...
        for i in range(1, len(v_array)):
            if v_array[i-1] <= v_10 and v_array[i] >= v_90:
                ...
                t_10 = np.interp(v_10, [v_array[i-1], v_array[i]], [t_array[i-1], t_array[i]])
                t_90 = np.interp(v_90, [v_array[i-1], v_array[i]], [t_array[i-1], t_array[i]])
...
class SymptomAnalyzer:
    ...
    POWER_SYMPTOMS = [
        "not turning on", "no power", "dead", "won't start", "no voltage",
        "burning smell", "hot", "overheating", "smoke", "melted",
        "regulator hot", "fuse blown", "short circuit", "overcurrent"
    ]
    
    DIGITAL_SYMPTOMS = [
        "i2c", "spi", "uart", "usb", "can", "communication",
        "no ack", "nack", "timeout", "bus error", "not responding",
        "enumeration", "not detected", "unknown device"
    ]
    
    ANALOG_SYMPTOMS = [
        "noise", "distortion", "offset", "drift", "unstable",
        "oscillating", "saturated", "clipping", "no output"
    ]
    
    RF_SYMPTOMS = [
        "no signal", "weak signal", "interference", "poor range",
        "antenna", "vswr", "reflection", "impedance mismatch"
    ]
    
    THERMAL_SYMPTOMS = [
        "overheating", "thermal shutdown", "hot", "cold",
        "temperature", "thermal cycling", "condensation"
    ]
    
    MECHANICAL_SYMPTOMS = [
        "broken", "cracked", "loose", "vibration", "shock",
        "bent", "physical damage", "connector", "intermittent"
    ]
    ...
    def analyze_voltage_measurement(self, measurement: TestMeasurement) -> Dict[str, Any]:
        ...
        if deviation < 0.05:  # Within 5%
            ...
            analysis["issues"].append(f"Voltage slightly out of spec: {actual}V vs {expected}V expected")
            analysis["recommendations"].append("Check load conditions and regulator feedback")
        else:  # More than 10% deviation
            analysis["status"] = "failed"
            
            if actual < expected * 0.9:  # More than 10% low
                analysis["issues"].append(f"Voltage too low: {actual}V (expected {expected}V)")
                ...
                analysis["issues"].append(f"Voltage too high: {actual}V (expected {expected}V)")
                analysis["recommendations"].extend([
    ...
    def analyze_i2c_signals(self, sda_trace: OscilloscopeTrace, scl_trace: OscilloscopeTrace) -> Dict[str, Any]:
        ...
        if voh < 2.97:  # 90% of 3.3V
            ...
            analysis["recommendations"].append("Check pull-up resistor value (try 2.2kΩ - 4.7kΩ)")
        
        if vol > 0.4:
            analysis["issues"].append(f"SDA low level too high: {vol}V")
            analysis["recommendations"].append("Check for weak driver or bus contention")
        ...
        if "rise_time" in sda_analysis:
            ...
            if rise_time_us > 1.0:
                analysis["issues"].append(f"SDA rise time too slow: {rise_time_us:.2f}μs")
                analysis["recommendations"].extend([
        ...
        if "frequency" in scl_analysis:
            ...
            if freq_khz > 400:
                analysis["recommendations"].append(f"Clock frequency {freq_khz:.1f}kHz exceeds I2C Fast Mode")
    ...
    def analyze_power_rail_stability(self, rail_trace: OscilloscopeTrace, load_current: Optional[float] = None) -> Dict[str, Any]:
        ...
        if ripple_percent > 5:
            ...
            analysis["recommendations"].append("Consider adding ceramic bypass capacitors")
        
        # Check for oscillation
        if "frequency" in waveform and waveform["frequency"] > 1000:
            analysis["issues"].append(f"Possible oscillation detected at {waveform['frequency']:.1f}Hz")
            analysis["recommendations"].extend([
        ...
        if load_current is not None:
            ...
            if ripple_percent > 1 and load_current > 0.5:
                analysis["recommendations"].append("High ripple under load - check thermal derating")
