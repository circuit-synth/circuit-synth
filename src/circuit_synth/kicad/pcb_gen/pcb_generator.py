...
from circuit_synth.kicad_api.pcb.routing.freerouting_docker import (
    route_pcb_docker
)
...
class PCBGenerator:
    ...
    def _calculate_initial_board_size(self, pcb: 'PCBBoard', component_spacing: float = 5.0,
        ...
        logger.debug(f"Calculated initial board size: {width}x{height}mm based on {component_count} components")
        return width, height
    ...
    def generate_pcb(self,
        ...
        try:
            ...
            for comp_info in components:
                ...
                if footprint:
                    ...
                    if fp and comp_info.get('hierarchical_path'):
                        ...
                        logger.debug(f"Added {comp_info['reference']} with footprint: {footprint} (from {'component' if comp_info.get('footprint') else 'symbol mapping'})")
                else:
                    logger.warning(f"No footprint found for {comp_info['reference']} ({comp_info['lib_id']})")
            
            # Calculate initial board size if not provided
            if board_width is None or board_height is None:
                current_width, current_height = self._calculate_initial_board_size(pcb, component_spacing)
            else:
            ...
            if connections and placement_algorithm == "hierarchical":
                logger.info("Found connections, switching to spiral placement for better results")
                placement_algorithm = "spiral"
            
            logger.debug(f"Applying {placement_algorithm} placement algorithm")
            logger.debug(f"Component spacing: {component_spacing}mm, Group spacing: {group_spacing}mm")
            ...
            while retry_count < max_retries and not placement_successful:
                try:
                    ...
                    logger.debug(f"Attempting placement with board size: {current_width}x{current_height}mm (attempt {retry_count + 1})")
                    ...
                    if retry_count == 0:
                        ...
                        logger.debug(f"Components before placement: {len(footprints_before)}")
                        for ref, footprint_lib, x, y in footprints_before[:5]:  # Show first 5
                    ...
                    if placement_algorithm == "force_directed":
                        ...
                        result = pcb.auto_place_components(
                            ...
                            connections=connections if placement_algorithm in ["spiral", "force_directed", "connection_centric"] else None
                        )
                    
                    # If we get here, placement was successful
                    placement_successful = True
                    logger.debug(f"✓ Placement successful with board size: {current_width}x{current_height}mm")
                    
                    # Calculate actual board size needed based on placement
                    from circuit_synth.kicad_api.pcb.placement.utils import calculate_placement_bbox
                    footprints = list(pcb.footprints.values())
                    min_x, min_y, max_x, max_y = calculate_placement_bbox(footprints, margin=10.0)
                    ...
                    logger.debug(f"✓ Adjusted board size to actual needs: {actual_width}x{actual_height}mm")
                    ...
                    for ref, footprint_lib, x, y in footprints_after[:5]:  # Show first 5
                        logger.debug(f"  {ref} at ({x}, {y})")
                    ...
                    if "Could not find valid position" in str(e):
                        ...
                        if current_width > max_board_size or current_height > max_board_size:
                            logger.error(f"Maximum board size ({max_board_size}mm) exceeded")
                            raise ValueError(f"Cannot place all components even with {max_board_size}x{max_board_size}mm board")
                        
                        logger.warning(f"Placement failed: {e}")
                        logger.debug(f"Increasing board size to {current_width}x{current_height}mm for next attempt")
                    else:
    ...
    def _extract_components_from_schematics(self) -> List[Dict[str, Any]]:
        ...
        for sch_file in sch_files:
            try:
                ...
                schematic = reader.read_file(str(sch_file))
                
                # Get components from this schematic
                for comp in schematic.components:
                    # Extract hierarchical path from schematic file name
                    # For sub-sheets, use the sheet name as the hierarchical path
                    if sch_file.stem == self.project_name:
                        # This is the root schematic
                        hierarchical_path = "/"
                    else:
                        # This is a sub-sheet, use its name as the path
                    ...
                    logger.debug(f"Component {comp.reference} from {sch_file.stem} has hierarchical path: {hierarchical_path}")
                    ...
                    if comp_info['reference'] and not comp_info['reference'].startswith('#'):
                        ...
                        if comp_info['reference'] in seen_references:
                            logger.warning(f"Duplicate component reference found: {comp_info['reference']} in {sch_file.stem}")
                            logger.warning(f"  Previous hierarchical path may be overwritten")
                        seen_references.add(comp_info['reference'])
                        
                        components.append(comp_info)
                        logger.debug(f"Found component: {comp_info['reference']} ({comp_info['lib_id']}) from {sch_file.stem}")
                        logger.debug(f"  Hierarchical path: {comp_info['hierarchical_path']}")
                        logger.debug(f"  Footprint: {comp_info.get('footprint', 'None')}")
        ...
        if netlist_path.exists():
            ...
            try:
                ...
                for net_name, net_content in net_matches:
                    ...
                    if base_name in ['GND', '3V3', '5V', 'VCC', 'VDD', 'VSS', ''] or net_name.startswith('Net-('):
                        continue
        ...
        for net_name, connected_refs in nets.items():
            ...
            for i in range(len(connected_list)):
                for j in range(i + 1, len(connected_list)):
                    ...
                    logger.debug(f"Connection: {connected_list[i]} <-> {connected_list[j]} (net: {net_name})")
        ...
        try:
            ...
            for base_name, net_list in net_groups.items():
                if len(net_list) > 1:
                    ...
                    logger.info(f"Found {len(net_list)} nets with base name '{base_name}' to merge:")
            ...
            for net_name, net_content in net_matches:
                ...
                if existing_net:
                    ...
                    logger.debug(f"Created net {net_num}: '{flattened_net_name}'" +
                               (f" (flattened from '{net_name}')" if net_name != flattened_net_name else ""))
                ...
                for ref, pad_num in clean_nodes:
                    ...
                    if not footprint:
                        logger.debug(f"Footprint {ref} not found, skipping pad assignment")
                        continue
                    ...
                    for pad in footprint.pads:
                        if pad.number == pad_num:
                            ...
                            pads_found += 1
                            logger.debug(f"Assigned {ref} pad {pad_num} (instance {pads_found}) to net {net_num} ('{flattened_net_name}')")
                    
                    if pads_found > 0:
                        assigned_pads.add((ref, pad_num))
                        if pads_found > 1:
                            logger.info(f"Note: {ref} has {pads_found} pads numbered '{pad_num}', assigned all to net {net_num}")
            
            logger.info(f"Created {nets_created} nets, assigned {len(assigned_pads)} pads to nets")
                ...
                for pad in footprint.pads:
                    ...
                    if (footprint.reference, pad.number) not in assigned_pads:
                        ...
                        if pad.net is None:
                            ...
                            logger.debug(f"Created net {net_num} for unconnected pad: {footprint.reference} pad {pad.number}")
            
            if unconnected_count > 0:
                logger.info(f"Created {unconnected_count} unique nets for unconnected pads")
    ...
    def _auto_route_pcb(self, pcb: PCBBoard, passes: int = 4, effort: float = 1.0) -> bool:
        """
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                logger.info(f"  - Footprints: {len(pcb.pcb_data.get('footprints', []))}")
                ...
                logger.info(f"✓ Temp PCB saved, size: {temp_pcb_file.stat().st_size} bytes")
                ...
                logger.info(f"✓ DSN export complete, size: {dsn_file.stat().st_size} bytes")
                ...
                with open(dsn_file, 'r') as f:
                    ...
                    logger.info(f"  - Components: {dsn_content.count('(component ')} components")
                ...
                logger.info(f"Freerouting result: success={success}, result_file={result_file}")
                ...
                logger.info(f"✓ Freerouting completed, SES size: {ses_file.stat().st_size} bytes")
                ...
                try:
                    import_ses_to_pcb(str(temp_pcb_file), str(ses_file), str(temp_pcb_file))
                except Exception as e:
                ...
                with open(temp_pcb_file, 'r') as f:
                    ...
                    logger.info(f"Routed PCB has {track_count} tracks and {via_count} vias")
