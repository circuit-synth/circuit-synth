...
from ...interfaces.kicad_interface import (
    IKiCadIntegration, ISchematicGenerator, IPCBGenerator,
    ISymbolLibrary, IFootprintLibrary, KiCadGenerationConfig
)
from ...core.dependency_injection import DependencyContainer, ServiceLocator, IDependencyContainer

# Import existing implementation modules
from .circuit_loader import load_circuit_hierarchy, assign_subcircuit_instance_labels
from .schematic_writer import SchematicWriter, write_schematic_file
from .collision_manager import CollisionManager, SHEET_MARGIN
...
class SchematicGeneratorImpl(ISchematicGenerator):
    ...
    def generate_from_circuit_data(self,
                                  circuit_data: Dict[str, Any],
                                  config: Optional[KiCadGenerationConfig] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            legacy_generator = SchematicGenerator(str(self.output_dir), self.project_name)
...
class SchematicGenerator(IKiCadIntegration):
    ...
    def __init__(self, output_dir: str, project_name: str, container: Optional[IDependencyContainer] = None):
        self.output_dir = Path(output_dir).resolve()
    ...
    def get_schematic_generator(self) -> ISchematicGenerator:
        ...
        if self._schematic_generator is None:
            self._schematic_generator = SchematicGeneratorImpl(str(self.output_dir), self.project_name)
    ...
    def get_pcb_generator(self) -> IPCBGenerator:
        ...
        if self._pcb_generator is None:
            ...
            class PCBGeneratorAdapter(IPCBGenerator):
                ...
                def generate_from_schematic(self,
                                           schematic_path: Path,
                                           config: Optional[KiCadGenerationConfig] = None) -> Dict[str, Any]:
                    try:
            ...
            self._pcb_generator = PCBGeneratorAdapter(str(self.output_dir), self.project_name)
    ...
    def get_footprint_library(self) -> IFootprintLibrary:
        ...
        if self._footprint_library is None:
            class FootprintLibraryAdapter(IFootprintLibrary):
                ...
                def get_footprint_info(self, footprint_name: str) -> Optional[Dict[str, Any]]:
    ...
    def _collect_all_references(self, json_file: str) -> set:
        ...
        logger.info(f"Found {len(all_refs)} pre-assigned references: {sorted(all_refs)}")
        return all_refs
    ...
    def generate_project(self, json_file: str, force_regenerate: bool = False, generate_pcb: bool = True,
                        placement_algorithm: str = "connection_centric",
                        schematic_placement: str = "connection_aware", draw_bounding_boxes: bool = False, **pcb_kwargs):
        """
        ...
        if project_exists and not force_regenerate:
            ...
            logger.warning(f"⚠️  WARNING: Force regenerating project at: {self.project_dir}")
            logger.warning("   This will LOSE all manual work (component positions, wires, etc.)")
        ...
        if generate_pcb:
            logger.info(f"Generating KiCad project '{self.project_name}' with schematics and PCB from '{json_file}'")
        else:
            logger.info(f"Generating KiCad project '{self.project_name}' with schematics only from '{json_file}'")
        ...
        self._collision_place_all_circuits(sub_dict, placement_algorithm=schematic_placement)
        ...
        if all_assigned_refs:
            shared_ref_manager.api_ref_manager.add_existing_references(list(all_assigned_refs))
            logger.info(f"Pre-populated reference manager with {len(all_assigned_refs)} existing references")
        ...
        while len(generated_circuits) < len(sub_dict):
                    ...
                    if parent_generated:
                        ...
                        if parent_name == top_name:
                            ...
                            if c_name in main_writer.sheet_symbol_map:
                                ...
                                hierarchical_path = [cover_uuid, sheet_uuid, sheet_symbol_uuid]
                            else:
                                logger.error(f"No sheet symbol found for {c_name} in main circuit!")
                                continue
                        else:
                            # Nested subcircuit - need to find its sheet symbol in parent
                            parent_writer = sheet_writers.get(parent_name)
                            if parent_writer and c_name in parent_writer.sheet_symbol_map:
                                sheet_symbol_uuid = parent_writer.sheet_symbol_map[c_name]
                                parent_path = circuit_parent_info.get(parent_name, {}).get('full_path', [])
                                hierarchical_path = parent_path + [sheet_symbol_uuid]
                            else:
                                logger.error(f"No sheet symbol found for {c_name} in parent {parent_name}!")
                                continue
                        ...
                        logger.debug(f"  Hierarchical path: {'/'.join(hierarchical_path)}")
                        ...
                        sheet_writers[c_name] = writer  # Store writer for nested subcircuits
            ...
            if not made_progress:
                ...
                logger.error(f"Could not generate circuits due to dependency issues: {remaining}")
                break
        ...
        if generate_pcb:
            ...
            success = pcb_gen.generate_pcb(
                ...
                board_width=pcb_kwargs.get('board_width', None),  # Auto-calculate if not specified
                ...
                **{k: v for k, v in pcb_kwargs.items() if k not in ['board_width', 'board_height', 'component_spacing', 'group_spacing']}
            )
    ...
    def _determine_paper_size(self, components, sheets):
        ...
        logger.debug(f"Determined paper size: {paper_size} ({current_size[0]}x{current_size[1]}mm) for max coordinates: ({max_x}, {max_y})") # Changed from INFO
        return paper_size

    def _collision_place_all_circuits(self, sub_dict: dict, placement_algorithm: str = "sequential"):
        """
        ...
        for c_name, circ in sub_dict.items():
            ...
            if placement_algorithm == "connection_aware":
                logger.info(f"Using connection-aware placement algorithm for circuit '{c_name}'")
                ...
                logger.info(f"Using LLM-based placement algorithm for circuit '{c_name}'")
                # LLM placement will use its own manager
                cm = CollisionManager(sheet_size=sheet_size)
            else:
                logger.info(f"Using sequential placement algorithm for circuit '{c_name}'")
                cm = CollisionManager(sheet_size=sheet_size)
            
            logger.debug(f"Using paper size: {self.paper_size} with dimensions: {sheet_size}")
            ...
            if placement_algorithm == "llm":
                ...
                import asyncio
                try:
                    success = asyncio.run(llm_manager.apply_llm_placement(circ, circ.components))
                    if success:
                        logger.info("LLM placement completed successfully")
                        # Skip the normal placement loop
                        continue
                    else:
                        logger.warning("LLM placement failed, falling back to sequential")
            ...
            if placement_algorithm == "connection_aware" and isinstance(cm, ConnectionAwareCollisionManager):
                # Get connection-based placement order
                placement_order = cm.connection_analyzer.get_placement_order(circ.components)
                logger.info(f"Connection-based placement order: {placement_order[:10]}...")  # Show first 10
                
                # Create a mapping for quick lookup
                comp_map = {comp.reference: comp for comp in circ.components}
                components_to_place = [comp_map[ref] for ref in placement_order if ref in comp_map]
            else:
                # Use original order
            ...
            for comp in components_to_place:
                if comp.reference in existing_positions:
                    ...
                    comp.position.x = x
                    comp.position.y = y
                    logger.debug(f"Using preserved position for {comp.reference}: ({x}, {y})")
                else:
                    ...
                    try:
                        ...
                        comp_width, comp_height = SymbolBoundingBoxCalculator.get_symbol_dimensions(symbol_data)
                        logger.debug(f"Component {comp.reference} ({comp.lib_id}) dimensions: {comp_width:.2f}x{comp_height:.2f}mm")
                    except Exception as e:
                        logger.error(f"Failed to get dimensions for {comp.reference} ({comp.lib_id}): {e}")
                        raise ValueError(f"Cannot calculate bounding box for component {comp.reference} ({comp.lib_id}): {e}")
                    
                    # Use appropriate placement method
                    if placement_algorithm == "connection_aware" and isinstance(cm, ConnectionAwareCollisionManager):
                        x, y = cm.place_component_connection_aware(comp.reference, comp_width, comp_height)
                    else:
                        x, y = cm.place_symbol(comp_width, comp_height)
                    
                    comp.position.x = x
                    comp.position.y = y
                    logger.debug(f"Placed new component {comp.reference} at: ({x}, {y})")
            ...
            for child in circ.child_instances:
                ...
                if sub_name in sub_dict:
                    ...
                    logger.debug(f"Sheet {sub_name}: name_width={name_width:.1f}mm, "
                ...
                child['x'] = x
                child['y'] = y
                logger.debug(f"Placed sheet {child['sub_name']} ({sheet_width}x{sheet_height}mm) at: ({x}, {y})")
            
            # Determine appropriate paper size based on placement
            self.paper_size = self._determine_paper_size(circ.components, circ.child_instances)
            ...
            if placement_algorithm == "connection_aware" and isinstance(cm, ConnectionAwareCollisionManager):
                ...
                logger.info(f"  Total wire length: {metrics['total_wire_length']:.2f}mm")
                logger.info(f"  Average wire length: {metrics['average_wire_length']:.2f}mm")
...
'''
    ...
    def generate_pcb_from_schematics(self,
                                    ...
                                    group_spacing: float = 10.0) -> bool:  # Increased for better separation
        """
        ...
        if success:
            logger.info(f"✅ PCB generated successfully at: {self.project_dir / f'{self.project_name}.kicad_pcb'}")
        else:
            logger.error("❌ PCB generation failed")
    ...
    def create_pcb_generator(self) -> 'IPCBGenerator':
        ...
        class PCBGeneratorAdapter(IPCBGenerator):
            ...
            def generate_from_circuit_data(self, circuit_data: Dict[str, Any], 
                                         config: Optional[KiCadGenerationConfig] = None) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if config:
                        placement_algorithm = config.placement_algorithm or placement_algorithm
                    ...
                    return {
                        'success': success,
                        'message': 'PCB generated successfully' if success else 'PCB generation failed'
                    }
                except Exception as e:
                    logger.error(f"PCB generation failed: {e}")
                    return {
