...
class KiCadFormatterNew:
    ...
    def format(self, expr: Any, level: int = 0, parent_context: str = None) -> str:
        ...
        if isinstance(expr, list):
            ...
            current_elem = str(expr[0]) if expr and isinstance(expr[0], Symbol) else None
    ...
    def _format_inline_construct(self, expr: List, level: int, current_elem: str) -> str:
        ...
        if first_elem == 'property' and len(expr) >= 3:
            ...
            prop_value_formatted = self.format(prop_value, 0, first_elem) if isinstance(prop_value, str) else f'"{prop_value}"'
        ...
        elif first_elem == 'symbol' and len(expr) >= 2 and not isinstance(expr[1], list):
        ...
        elif first_elem in ['hide', 'bold', 'italic', 'plot', 'mirror', 'unit', 
                           'exclude_from_sim', 'in_bom', 'on_board', 'dnp', 
                           'fields_autoplaced', 'page']:
    ...
    def _format_standard_list(self, expr: List, level: int, current_elem: str) -> str:
        ...
        if len(parts) <= 3 and all(not '\n' in p for p in parts) and sum(len(p) for p in parts) < 60:
            return f"({' '.join(parts)})"
