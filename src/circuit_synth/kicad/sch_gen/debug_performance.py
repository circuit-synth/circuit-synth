...
@contextmanager
def timed_operation(name: str, threshold_ms: float = 10.0, details: Optional[Dict] = None):
    """
    ...
    try:
        ...
        if elapsed_ms > threshold_ms:
            logger.warning(f"🐌 SLOW OPERATION: {name} took {elapsed_ms:.2f}ms (threshold: {threshold_ms}ms)")
            if details:
...
def log_symbol_lookup(lib_id: str, found: bool, time_ms: float, source: str = "unknown"):
    ...
    # Warn if slow
    if time_ms > 50:
        logger.warning(f"🐌 SLOW SYMBOL LOOKUP: {lib_id} took {time_ms:.2f}ms from {source}")
...
def log_component_processing(component_ref: str, lib_id: str, operation: str, time_ms: float, details: Optional[Dict] = None):
    ...
    # Warn if slow
    if time_ms > 20:
        logger.warning(f"🐌 SLOW COMPONENT OPERATION: {component_ref} - {operation} took {time_ms:.2f}ms")
...
def print_performance_summary():
    ...
    if PERF_DATA['slow_operations']:
        ...
        print("-"*60)
        sorted_ops = sorted(PERF_DATA['slow_operations'], key=lambda x: x['time_ms'], reverse=True)
    ...
    if PERF_DATA['symbol_lookups']:
        ...
        if slow_symbols:
            ...
            for lib_id, time_ms, count in sorted(slow_symbols, key=lambda x: x[1], reverse=True):
                avg_time = time_ms / count
                print(f"    {lib_id:50s}: {time_ms:8.2f}ms total ({count} lookups, {avg_time:.2f}ms avg)")
    ...
    if PERF_DATA['component_details']:
        ...
        # Sort by total time
        sorted_comps = sorted(component_totals.items(), key=lambda x: x[1], reverse=True)
        ...
        for comp_ref, total_time in sorted_comps[:10]:  # Top 10
            if total_time > 10:
                ...
                if comp_ref in PERF_DATA['component_details']:
                    for op_name, op_list in PERF_DATA['component_details'][comp_ref].items():
                        op_time = sum(o['time_ms'] for o in op_list)
                        if op_time > 5:
