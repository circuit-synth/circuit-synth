...
class SymbolBoundingBoxCalculator:
    ...
    DEFAULT_PIN_TEXT_WIDTH_RATIO = 2.0  # Width to height ratio for pin text (increased from 1.5)
    
    @classmethod
    def calculate_bounding_box(cls, symbol_data: Dict[str, Any],
                             include_properties: bool = True,
                             hierarchical_labels: Optional[List[Dict[str, Any]]] = None) -> Tuple[float, float, float, float]:
        """
        Calculate the actual bounding box of a symbol from its graphical elements.
        ...
        logger.debug(f"Calculated bounding box: ({min_x:.2f}, {min_y:.2f}) to ({max_x:.2f}, {max_y:.2f})")
        
        return (min_x, min_y, max_x, max_y)
    
    @classmethod
    def get_symbol_dimensions(cls, symbol_data: Dict[str, Any],
                            include_properties: bool = True) -> Tuple[float, float]:
        """
        ...
        """
        min_x, min_y, max_x, max_y = cls.calculate_bounding_box(symbol_data, include_properties)
        width = max_x - min_x
        height = max_y - min_y
        return (width, height)
    
    @classmethod
    def _get_shape_bounds(cls, shape: Dict[str, Any]) -> Optional[Tuple[float, float, float, float]]:
    ...
    @classmethod
    def _get_pin_bounds(cls, pin: Dict[str, Any]) -> Optional[Tuple[float, float, float, float]]:
        ...
        if pin_number:
            num_width = len(pin_number) * cls.DEFAULT_PIN_NUMBER_SIZE * cls.DEFAULT_PIN_TEXT_WIDTH_RATIO
