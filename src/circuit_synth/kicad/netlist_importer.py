...
class Subcircuit:
    ...
    def to_dict(self) -> dict:
        ...
        result = {
            "name": self.name,
            "components": {ref: comp.to_dict() for ref, comp in self.components.items()},
            # Structure A format: Map net name directly to list of nodes
            "nets": {
                nname: net.to_dict()["nodes"]
                for nname, net in self.nets.items()
            },
...
class CircuitTemplate:
    ...
    representative_circuit: Optional[Subcircuit] = None  # One instance to use as template
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "representative_circuit_name": self.representative_circuit.name if self.representative_circuit else None
        }

@dataclass
class DuplicateDetectionResult:
    """Results of duplicate detection analysis."""
    templates: Dict[str, CircuitTemplate] = field(default_factory=dict)  # template_id -> CircuitTemplate
    unique_circuits: List[str] = field(default_factory=list)  # Circuit names that are unique
    duplicate_groups: Dict[str, str] = field(default_factory=dict)  # circuit_name -> template_id
...
class Circuit:
    ...
    def to_dict(self) -> dict:
        ...
        result = {
            ...
            "nets": root_dict["nets"], # Uses the 'nets' from root_dict, which is already in Structure A (Net Name -> List of Nodes).
    ...
    def _clean_artifact_subcircuits(self, subcircuit: Subcircuit) -> bool:
        ...
        return (bool(subcircuit.components) or 
                bool(subcircuit.nets) or 
                bool(subcircuit.children) or 
                self._is_human_readable_name(subcircuit.name))
    ...
    def _is_human_readable_name(self, name: str) -> bool:
        ...
        common_words = ['usb', 'power', 'cpu', 'interface', 'sensor', 'led', 'filter', 'card', 'memory', 'output', 'input']
...
class ParsedNetlist:
    ...
    def sheets(self) -> List[List[Any]]:
        ...
        if not sheets_found:
            logger.debug("No direct sheet entries found in design section. Design structure:")
            for item in des:
...
class CircuitSynthParser:
    ...
    def __init__(self):
        ...
        self.subcircuits: Dict[str, Subcircuit] = {} # Map path string to Subcircuit object
        # Component reference to sheetpath lookup for faster access
        self.component_sheetpaths: Dict[str, str] = {} # Map component ref to its sheetpath string
        ...
        self.net_name_map: Dict[str, str] = {} # Map original net name to standardized name
        ...
        self.all_components: Dict[str, Component] = {} # Map component ref to Component object
    ...
    def parse_kicad_netlist(self, netlist_path: Path) -> Circuit:
        ...
        libpart_blocks = netlist.libparts() # Assuming ParsedNetlist has libparts() method
    ...
    def _parse_libparts(self, libpart_blocks: List[List[Any]]) -> None:
        ...
        for block in libpart_blocks:
            ...
            if not lib_name or not part_name:
                logger.warning(f"Skipping libpart block due to missing lib or part name: {block}")
                continue
            ...
            for item in block[1:]:
                ...
                for pin_item in item[1:]:
                    ...
                    if not pin_data['num']:
                        logger.warning(f"Skipping pin with missing number in {libpart_key}: {pin_item}")
                        continue
                    ...
                    logger.debug(f"Added pin {pin_data['num']} ({pin_data['name']}) of type {pin_type.value} to {libpart_key}")
    ...
    def _determine_pin_type(self, type_str: str, direction: str, electrical_type: str, pin_name: str) -> PinType:
        """
        ...
        if any(power_term in pin_name for power_term in ['vcc', 'vdd', 'v+', 'pwr', 'power']):
        ...
        logger.debug(f"Could not determine specific pin type for: type={type_str}, direction={direction}, name={pin_name}")
    ...
    def _create_subcircuit_hierarchy(self, sheet_paths, sheet_data):
        ...
        for path in sorted(sheet_paths, key=lambda p: p.count("/")):
            if path not in self.subcircuits:
                ...
                logger.info(f"Created subcircuit for path: {path} with name: {subcircuit_name}")
        ...
        for path in sorted(sheet_paths, key=lambda p: p.count("/")):
            ...
            if parent_path in self.subcircuits:
                ...
                if child_name not in parent.children:
                    ...
                    logger.info(f"Added subcircuit '{child_name}' to parent '{parent_path}'")
            else:
                logger.warning(f"Parent path '{parent_path}' not found for subcircuit '{path}', attaching to root")
    ...
    def _register_component(self, component: Component, sheet_path: str) -> None:
        ...
        logger.debug(f"Registered component '{component.reference}' in subcircuit '{sheet_path}'")
    ...
    def _parse_components(self, comp_blocks: List[List[Any]]) -> None:
        ...
        for comp_block in comp_blocks:
                    ...
                    if key == "ref":
                        ...
                        for sub_item in val:
                            if isinstance(sub_item, list) and len(sub_item) == 2:
                                ...
                                if sub_key == "lib": lib_name = sub_val
                                if sub_key == "part": part_name = sub_val
                    elif key == "sheetpath":
                        ...
                        for field_item in val:
                            if isinstance(field_item, list) and field_item[0] == "field" and len(field_item) >= 3:
                                field_parts = field_item[1:]
                                field_name = ""
                                field_value = ""
                                # Extract field name and value
                                if isinstance(field_parts[0], list) and field_parts[0][0] == "name":
                                    field_name = field_parts[0][1]
                                if len(field_parts) >= 2 and isinstance(field_parts[1], str):
                                    field_value = field_parts[1]
                                
                                if field_name:
                                    found_fields.add(field_name)
                                    comp_info["properties"][field_name] = field_value
                                    logger.debug(f"Added field '{field_name}' = '{field_value}' to component {comp_info['ref']}")
                    elif key == "property":
                        p_name, p_val = "", ""
                        if isinstance(val, list):
                            for sub2 in val:
                                if isinstance(sub2, list) and len(sub2)==2:
                                    ...
                                    if sub2key == "name": p_name = sub2val
                                    elif sub2key == "value": p_val = sub2val
                        if p_name:
                            found_properties.add(p_name)
                            comp_info["properties"][p_name] = p_val
                            logger.debug(f"Added property '{p_name}' = '{p_val}' to component {comp_info['ref']}")
            ...
            if libpart_key in self._libparts_data:
                ...
                logger.debug(f"  Added {len(c.pins)} pins to {c.reference} from libpart {libpart_key}")
            else:
                logger.warning(f"  Libpart key {libpart_key} not found for component {c.reference}. Pins will be empty.")
        ...
        for sheet_path, components in components_by_sheet.items():
            logger.info(f"Subcircuit '{sheet_path}' has {len(components)} components: {components}")


    def _get_pin_details(self, component_ref: str, pin_number: str, net_name: str, node_index: int) -> Tuple[str, PinType]:
        """
        ...
        if not component:
            logger.warning(f"Node {node_index}: Component '{component_ref}' not found for net '{net_name}'")
            return pin_name, pin_type
            
        # Try to find the pin in this component
        pin = component.pins.get(pin_number)
        if not pin:
            logger.warning(f"Node {node_index}: Pin '{pin_number}' not found in component '{component_ref}' for net '{net_name}'")
            return pin_name, pin_type
            
        # Found both component and pin
        pin_name = pin.name or "~"  # Use ~ if name is empty
        pin_type = pin.pin_type
        logger.debug(f"Node {node_index}: Found pin '{pin_name}' of type '{pin_type.value}'")
    ...
    def _parse_nets(self, net_blocks: List[List[Any]]) -> None:
        ...
        for i, net_block in enumerate(net_blocks):
            ...
            # Skip empty nets
            if not nodes_data:
                logger.warning(f"Net '{net_name}' (code {net_code}) has no nodes. Skipping.")
                continue

            # Get standardized name once for the entire net
            standardized_name = self._standardize_net_name(net_name)
            logger.debug(f"Processing net '{net_name}' -> '{standardized_name}' with {len(nodes_data)} nodes")
        ...
        logger.info(f"Finished parsing nets. Processed {len(processed_nodes)} unique nodes.")
    ...
    def _extract_node_info(self, node_data: List[Any], net_name: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            logger.error(f"Error extracting node info for net '{net_name}': {e}", exc_info=True)
            return None
    ...
    def _get_component_sheet_path(self, component_ref: str, net_name: str) -> str:
        ...
        if not sheet_path:
            logger.warning(f"Component '{component_ref}' not found in sheetpaths for net '{net_name}'. Using root.")
            sheet_path = "/"
            
        return self._normalize_sheet_path(sheet_path)

    def _add_node_to_subcircuit(self, component_ref: str, pin_number: str,
                               net_name: str, net_code: str, sheet_path: str,
                               net_attributes: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not target_subcircuit:
                logger.error(f"Could not find/create subcircuit for '{sheet_path}'. Skipping node.")
                return

            # Get local net name
            local_name, hierarchical_name, is_hierarchical = self._get_local_net_name(net_name, sheet_path)
            ...
            pin_name, pin_type = self._get_pin_details(component_ref, pin_number, net_name, 0)
            node = Node(
                component_ref=component_ref,
                pin_number=pin_number,
                pin_name=pin_name,
            ...
            # Add node to net
            net.add_node(node)
            logger.debug(f"Added node {component_ref}:{pin_number} to net '{local_name}' in '{sheet_path}'")
            ...
            try:
                ...
                for item in net_block:
                    if isinstance(item, list) and len(item) == 2 and item[0] == 'name':
                        ...
                        break
                if net_name is None:
                    logger.warning(f"  Net Block {i+1}: Could not find name in block: {net_block}")
                    continue

                standardized = net_name # Default to original
                if net_name.startswith('/'):
                    standardized = net_name[1:]
                    logger.debug(f"  Net Block {i+1}: Standardizing '{net_name}' -> '{standardized}'")
                ...
                logger.warning(f"  Net Block {i+1}: Error processing block for mapping: {net_block}. Error: {e}")
    ...
    def _standardize_net_name(self, net_name: str) -> str:
        ...
        if net_name in self.net_name_map:
            ...
            if standardized != net_name:
                logger.debug(f"Standardizing net name via map: '{net_name}' -> '{standardized}'")
            return standardized
            
        # If not in map but has leading slash, remove it
        if net_name.startswith('/'):
            standardized = net_name[1:]
            logger.debug(f"Standardizing net name (removing leading '/'): '{net_name}' -> '{standardized}'")
            return standardized
    ...
    def _get_local_net_name(self, full_net_name: str, sheet_path: str) -> Tuple[str, str, bool]:
        """
        ...
        is_hierarchical = full_net_name != self.net_name_map.get(f"/{full_net_name}", full_net_name) or "/" in full_net_name # Check if original started with / or contains /

        if "/" in full_net_name and not full_net_name.startswith('/'): # Handles names like "Sheet1/NetLabel"
            ...
            logger.debug(f"Relative hierarchical net detected. Local: '{local_name}', Constructed Hierarchical: '{hierarchical_name}'")
            ...
            logger.debug(f"Absolute hierarchical net detected. Hierarchical: '{hierarchical_name}', Local: '{local_name}'")
        else:
        ...
        if not hierarchical_name.startswith('/'):
            logger.warning(f"Generated hierarchical name '{hierarchical_name}' does not start with '/'. Forcing.")
    ...
    def _normalize_sheet_path(self, sheet_path: str) -> str:
        ...
        while '//' in path:
            path = path.replace('//', '/')
        if path != "/" and path.endswith("//"): # Handle potential trailing double slash after replace
             path = path[:-1]
    ...
    def _find_or_create_subcircuit(self, path: str) -> Optional[Subcircuit]:
        ...
        for part in parts:
            if not part: continue # Skip empty parts resulting from slashes
            ...
            logger.debug(f"Processing path part: '{part}', Target child path: '{child_path}'")

            # Check if child exists by path in the global lookup first
            if child_path in self.subcircuits:
                logger.debug(f"Found existing child subcircuit for path '{child_path}' in global lookup.")
            ...
            # If not in global lookup, check the current parent's children by name
                ...
                logger.debug(f"Found existing child subcircuit by name '{part}' under parent '{current_subcircuit.name}'. Adding to global lookup.")
                ...
                logger.warning(f"Subcircuit for path '{child_path}' (name '{part}') not found under parent '{current_subcircuit.name}'. Creating it.")
                # Use the part name for the new subcircuit's name
                new_subcircuit = Subcircuit(name=part)
                current_subcircuit.children[part] = new_subcircuit # Add to parent's children dict
        ...
        logger.debug(f"Returning subcircuit '{current_subcircuit.name}' for path '{path}'")
    ...
    def _extract_sheetpath(self, sheetpath_item: List[Any]) -> str:
        ...
        if not isinstance(sheetpath_item, list) or len(sheetpath_item) < 3 or sheetpath_item[0] != 'sheetpath':
            logger.warning(f"Invalid sheetpath item format: {sheetpath_item}. Defaulting to '/'")
            return "/"
        try:
            # Find the 'names' sub-list
            for sub_item in sheetpath_item[1:]:
                 if isinstance(sub_item, list) and len(sub_item) == 2 and sub_item[0] == 'names':
                     path = sub_item[1]
                     normalized_path = self._normalize_sheet_path(path)
                     logger.debug(f"Extracted sheetpath: '{path}' -> Normalized: '{normalized_path}' from item: {sheetpath_item}")
                     return normalized_path
            logger.warning(f"Could not find 'names' in sheetpath item: {sheetpath_item}. Defaulting to '/'")
            return "/"
        except (IndexError, TypeError):
             logger.warning(f"Error parsing sheetpath item: {sheetpath_item}. Defaulting to '/'")
             return "/"
    ...
    def _verify_and_log_hierarchy(self) -> None:
        ...
        while nodes_to_visit:
             ...
             if current_node_id in visited_objs: continue # Avoid cycles if any exist
        ...
        paths_only_in_tree = all_paths_in_tree - all_paths_in_lookup # Should be empty if lookup is correct

        if paths_only_in_lookup:
             logger.warning(f"Found {len(paths_only_in_lookup)} paths in lookup table but not reachable in the final tree: {paths_only_in_lookup}")
        if paths_only_in_tree:
             logger.error(f"Found {len(paths_only_in_tree)} paths in the final tree but missing from the lookup table: {paths_only_in_tree}") # This indicates a bug
    ...
    def detect_duplicate_circuits(self) -> DuplicateDetectionResult:
        ...
        circuit_analysis = {}  # subcircuit_name -> (source_file, signature, subcircuit_obj)
        ...
        for path, subcircuit in self.subcircuits.items():
            ...
            logger.info(f"Circuit '{subcircuit.name}': source='{source_file}', signature='{signature[:16]}...'")
        
        # Step 2: Group circuits by source file and signature
        source_groups = defaultdict(list)  # source_file -> [(name, signature, subcircuit)]
        ...
        for source_file, circuits in source_groups.items():
            if len(circuits) == 1:
                ...
                logger.debug(f"Unique circuit: '{circuits[0][0]}' (source: {source_file})")
            else:
                ...
                for signature, circuit_group in signature_groups.items():
                    if len(circuit_group) == 1:
                        ...
                        logger.debug(f"Unique circuit: '{circuit_group[0][0]}' (unique signature)")
                    else:
                        # True duplicates found!
                        template_id = self._generate_template_id(source_file)
                        instance_names = [name for name, _ in circuit_group]
                        representative = circuit_group[0][1]  # Use first as representative
                        ...
                        logger.info(f"Detected duplicate template '{template_id}': {len(instance_names)} instances of '{source_file}'")
        ...
        logger.info(f"Duplicate detection complete: {len(templates)} templates, {len(unique_circuits)} unique circuits")
        return result
    ...
    def _extract_schematic_source(self, subcircuit: Subcircuit) -> Optional[str]:
        ...
        for component in subcircuit.components.values():
            if "Sheetfile" in component.properties:
                ...
                logger.debug(f"Found source file '{source_file}' in component '{component.reference}'")
                return source_file
    ...
    def _generate_component_signature(self, subcircuit: Subcircuit) -> str:
        ...
        for ref, component in subcircuit.components.items():
            ...
            components_by_timestamp.append((timestamp, component.value, component.symbol))
        ...
        for net_name, net in sorted_nets:
            ...
            for node in net.nodes:
                ...
                if node.component_ref in subcircuit.components:
                    ...
                    connection_types.append(f"{comp.value}:{comp.symbol}:{node.pin_number}")
                else:
            ...
            normalized_net_name = "net" if not net_name.startswith("unconnected-") else "unconnected"
            net_sig = f"{normalized_net_name}:{len(net.nodes)}:{':'.join(connection_types)}"
        ...
        logger.debug(f"Generated signature for '{subcircuit.name}': {signature_hash[:16]}... (from {len(signature_parts)} parts)")
    ...
    def _collect_attached_subcircuits(self, subcircuit: Subcircuit, attached: Set[str]) -> None:
    ...
    def _log_subcircuit_hierarchy(self, subcircuit: Subcircuit, level: int = 0) -> None:
        ...
        for p, s in self.subcircuits.items(): # Use self.subcircuits
             if s is subcircuit:
                 path_str = f"Path: '{p}'"
                 break
        logger.debug(f"{indent}- Subcircuit: '{subcircuit.name}' ({path_str}, {details})")
        ...
        if subcircuit.nets:
             ...
             net_info = [f"'{n.name}' (Hier: '{n.hierarchical_name}')" for n in subcircuit.nets.values()]
