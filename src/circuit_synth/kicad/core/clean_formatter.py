...
from typing import List, Tuple, Set, Optional, Callable, Any, Union, Dict
...
class FormattingRules:
    ...
    def _format_property(self, sexp: List, indent: int, formatter) -> str:
        ...
        lines.append(f"{formatter._get_indent(indent + 1)}{formatter._quote_if_needed(sexp[1])}")
        
        # Property value (quoted)
        lines.append(f"{formatter._get_indent(indent + 1)}{formatter._quote_if_needed(sexp[2])}")
    ...
    def _format_pin(self, sexp: List, indent: int, formatter) -> str:
        ...
        for elem in sexp[1:]:
            if isinstance(elem, list):
                lines.append(formatter.format(elem, indent + 1))
            elif elem in ["input", "output", "bidirectional", "power_in", "passive", "line", "inverted"]:
...
class CleanSExprFormatter:
    ...
    def _format_multiline(self, sexp: List, indent: int, rule: FormatRule) -> str:
        ...
        if sexp:
            ...
            for i, elem in enumerate(sexp[1:], 1):
                if i in rule.quote_indices:
                    lines.append(f"{self._get_indent(indent + 1)}{self._quote_if_needed(elem)}")
                elif isinstance(elem, list):
                    lines.append(self.format(elem, indent + 1))
                else:
                    lines.append(f"{self._get_indent(indent + 1)}{self._format_primitive(elem)}")
    ...
    def _build_component_sexp(self, comp: Dict) -> List:
        ...
        # Add properties
        sexp.append([
            "property", "Reference", comp.get("reference", "REF"),
            ["at", 0, 0, 0],
            ["effects", ["font", ["size", 1.27, 1.27]]]
        ])
        
        sexp.append([
            "property", "Value", comp.get("value", ""),
            ["at", 0, 5, 0],
            ["effects", ["font", ["size", 1.27, 1.27]]]
        ])
