...
class CanonicalCircuit:
    ...
    @classmethod
    def _process_subcircuit_recursive(cls, subcircuit, connections: List[CanonicalConnection],
                                    component_idx_ref: int) -> int:
        """
        ...
        for child in subcircuit.children.values():
            component_idx_ref = cls._process_subcircuit_recursive(child, connections, component_idx_ref)
    ...
    def from_kicad(cls, schematic_data: Dict[str, Any]) -> 'CanonicalCircuit':
        ...
        for symbol in symbols:
            ...
            if cls._is_power_symbol(symbol):
                logger.debug(f"Skipping power symbol: {symbol.get('lib_id', 'unknown')}")
                continue
            ...
            pin_connections = cls._get_pin_connections(symbol, comp_uuid, net_connections)
    ...
    @staticmethod
    def _get_pin_connections(symbol: Dict[str, Any], comp_uuid: str,
                           net_connections: Dict[str, str]) -> Dict[str, str]:
        """
...
class CircuitMatcher:
    ...
    def match(self, old_circuit: CanonicalCircuit, new_circuit: CanonicalCircuit) -> Dict[int, int]:
        """
        ...
        """
        self.logger.info(f"Starting component matching: {old_circuit.component_count} old components, "
        ...
        for component_type, old_indices in old_by_type.items():
            ...
            self.logger.debug(f"Processing type '{component_type}': "
                            f"{len(old_indices)} old, {len(new_indices)} new")
            
            if not new_indices:
                # No components of this type in new circuit
                self.logger.debug(f"No new components of type '{component_type}' - skipping")
                continue
            ...
            if len(old_indices) == 1 and len(new_indices) == 1:
                ...
                self.logger.debug(f"Direct match for single '{component_type}': "
                ...
                self._match_by_connectivity(
                    ...
                    matches, matched_new_components,
                    component_type
                )
        
        # Log summary of matching results
        matched_count = sum(1 for v in matches.values() if v != -1)
        self.logger.info(f"Matching complete: {matched_count}/{len(matches)} components matched")
        ...
        unmatched_new = [idx for idx in new_circuit.components.keys()
    ...
    def _group_components_by_type(self, circuit: CanonicalCircuit) -> Dict[str, List[int]]:
        """
    ...
    def _match_by_connectivity(self,
        ...
        """
        self.logger.debug(f"Matching {len(old_indices)} old '{component_type}' components "
        ...
        for old_idx in old_indices:
            for new_idx in new_indices:
                ...
                self.logger.debug(f"Connectivity score: old[{old_idx}] <-> new[{new_idx}] = {score:.3f}")
        ...
        for old_idx, new_idx, score in scores:
            ...
            if score > 0.0:  # Could adjust threshold if needed
                matches[old_idx] = new_idx
                matched_new_components.add(new_idx)
                self.logger.debug(f"Matched '{component_type}': old[{old_idx}] -> new[{new_idx}] "
                                f"(score: {score:.3f})")
