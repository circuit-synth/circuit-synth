...
class FormatterTestHarness:
    ...
    def _generate_schematic_sexp(self, circuit_data: Dict, use_new: bool = False) -> str:
    ...
    def _build_component_sexp(self, component: Dict) -> List:
        ...
        return [
            ...
            ["at", component.get("x", 0), component.get("y", 0), component.get("angle", 0)],
            ...
            ["property", "Reference", component.get("reference", "R1"),
                ["at", 0, 0, 0],
                ["effects", ["font", ["size", 1.27, 1.27]]]
            ],
            ["property", "Value", component.get("value", "10k"),
                ["at", 0, 5, 0],
                ["effects", ["font", ["size", 1.27, 1.27]]]
            ]
        ]
        ...
        report = {
            ...
            "summary": {
                ...
                "success_rate": (
                    ...
                    self.migration_status["tests_run"] * 100
                    if self.migration_status["tests_run"] > 0 else 0
                )
            }
        }
        
        # Save JSON report
        report_file = self.reports_dir / f"migration_report_{datetime.now():%Y%m%d_%H%M%S}.json"
        report_file.write_text(json.dumps(report, indent=2))
        
        # Generate human-readable summary
        summary_file = self.reports_dir / f"migration_summary_{datetime.now():%Y%m%d_%H%M%S}.md"
...
"""
    ...
    def _calculate_avg_metrics(self, report: Dict, metrics_key: str) -> str:
        ...
        avg_indent = sum(r[metrics_key]['indent_levels'] for r in report['test_results']) / len(report['test_results'])
        
        return f"Lines: {total_lines}, Chars: {total_chars}, Avg Indent: {avg_indent:.1f}"
...
def run_baseline_capture():
    ...
    hierarchical_circuit = {
        ...
        "sheets": [
            {"name": "child1", "file": "child1.kicad_sch"}
        ]
    }
