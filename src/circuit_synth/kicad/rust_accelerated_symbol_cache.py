...
try:
    ...
    rust_cache_path = Path(__file__).parent.parent.parent.parent / "rust_modules" / "rust_symbol_cache" / ".venv" / "lib" / "python3.12" / "site-packages"
    ...
    logger.info("🦀 Rust-accelerated SymbolLibCache available (55x performance improvement)")
...
class RustAcceleratedSymbolLibCache:
    ...
    def get_symbol_data(cls, symbol_id: str) -> Dict[str, Any]:
        ...
        if instance._using_rust:
            try:
                ...
                logger.warning(f"🔄 Rust symbol lookup failed for {symbol_id}, using Python fallback: {e}")
                ...
                if instance._python_cache is None:
                    if _PythonSymbolLibCache is not None:
                        ...
                        raise FileNotFoundError(f"Symbol '{symbol_id}' not found and no Python fallback available")
    ...
    def get_symbol_data_by_name(cls, symbol_name: str) -> Dict[str, Any]:
        ...
        if instance._using_rust:
            try:
                ...
                logger.warning(f"🔄 Rust symbol search failed for {symbol_name}, using Python fallback: {e}")
                if instance._python_cache is None:
                    if _PythonSymbolLibCache is not None:
                        instance._python_cache = _PythonSymbolLibCache()
                    else:
                        raise FileNotFoundError(f"Symbol '{symbol_name}' not found and no Python fallback available")
    ...
    def find_symbol_library(cls, symbol_name: str) -> Optional[str]:
        ...
        if instance._using_rust:
            try:
                ...
                logger.warning(f"🔄 Rust library search failed for {symbol_name}, using Python fallback: {e}")
                if instance._python_cache is None:
                    if _PythonSymbolLibCache is not None:
                        instance._python_cache = _PythonSymbolLibCache()
                    else:
    ...
    def get_all_libraries(cls) -> Dict[str, str]:
        ...
        if instance._using_rust:
            try:
                ...
                logger.warning(f"🔄 Rust library enumeration failed, using Python fallback: {e}")
                if instance._python_cache is None:
                    instance._python_cache = _PythonSymbolLibCache()
                return instance._python_cache.get_all_libraries()
        else:
    ...
    def get_all_symbols(cls) -> Dict[str, str]:
        ...
        if instance._using_rust:
            try:
                ...
                logger.warning(f"🔄 Rust symbol enumeration failed, using Python fallback: {e}")
                if instance._python_cache is None:
                    instance._python_cache = _PythonSymbolLibCache()
                return instance._python_cache.get_all_symbols()
        else:
    ...
    def get_performance_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expected_cache_time_reduction": "0.216s → 0.004s" if self._using_rust else "no change"
        }
