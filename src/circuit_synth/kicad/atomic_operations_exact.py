...
def remove_component_from_schematic_exact(
    ...
    try:
        ...
        while ref_pattern in remaining_content:
            ...
            remaining_content = remaining_content[:symbol_start] + remaining_content[symbol_end:]
            components_removed += 1
            
            # Look for any trailing newline after the removed component
            if symbol_start < len(remaining_content) and remaining_content[symbol_start] == '\n':
                remaining_content = remaining_content[:symbol_start] + remaining_content[symbol_start + 1:]
        ...
        if '\t(sheet_instances' not in remaining_content:
            ...
            if embedded_fonts_pos != -1:
                ...
                if final_paren != -1:
                    sheet_instances_section = '\t(sheet_instances\n\t\t(path "/"\n\t\t\t(page "1")\n\t\t)\n\t)\n\t(embedded_fonts\n\t\tno\n\t)\n'
                    remaining_content = remaining_content[:final_paren] + sheet_instances_section + ')'
        
        # Write back to file
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(remaining_content)
        
        logger.info(f"Removed {components_removed} instances of component {reference} from {file_path}")
        return True
