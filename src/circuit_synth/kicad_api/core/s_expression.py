...
from ..core.types import (
    Schematic, SchematicSymbol, SymbolInstance, Wire, Label, Junction,
    Point, SchematicPin, LabelType, Sheet, SheetPin, Net, Rectangle
)
...
class SExpressionParser:
    ...
    def _format_sexp(self, sexp: Any, indent: int = 0, parent_tag: str = None, in_number: bool = False, in_project: bool = False, in_instances: bool = False, in_page: bool = False, in_property_value: bool = False, in_property_name: bool = False, in_generator: bool = False, in_symbol: bool = False, in_lib_symbols: bool = False, in_name: bool = False) -> str:
        ...
        is_simple = (
            (len(sexp) <= 4 and
             all(not isinstance(item, list) for item in sexp) and
             sum(len(str(item)) for item in sexp) < 60) or
            (is_symbol_in_lib_symbols and len(sexp) == 2)  # Just symbol tag and ID
        )
        ...
        if is_simple:
            ...
            for i, item in enumerate(sexp):
                ...
                if is_number_expr and i == 1:
                    parts.append(self._format_sexp(item, indent, tag_name, in_number=True))
                # For name expressions, pass in_name=True for the value at index 1
                elif is_name_expr and i == 1:
                    parts.append(self._format_sexp(item, indent, tag_name, in_name=True))
                # For project expressions, pass in_project=True for the value at index 1
                elif is_project_expr and i == 1:
                    parts.append(self._format_sexp(item, indent, tag_name, in_project=True, in_instances=in_instances))
                elif is_instances_expr:
                    # Inside instances, pass the flag down
                    parts.append(self._format_sexp(item, indent, tag_name, in_instances=True))
                elif is_lib_symbols_expr:
                    # Inside lib_symbols, pass the flag down
                    parts.append(self._format_sexp(item, indent, tag_name, in_lib_symbols=True, in_instances=in_instances))
                elif is_page_expr and i == 1:
                    # For page expressions, pass in_page=True for the value at index 1
                    parts.append(self._format_sexp(item, indent, tag_name, in_page=True, in_instances=in_instances))
                    ...
                    parts.append(self._format_sexp(item, indent, tag_name, in_symbol=True, in_instances=in_instances))
            ...
            if is_symbol_expr and in_lib_symbols and len(sexp) >= 2:
                ...
                for i in range(2, len(sexp)):
                    result += "\n" + "\t" * (indent + 1) + self._format_sexp(sexp[i], indent + 1, tag_name, in_lib_symbols=in_lib_symbols, in_instances=in_instances)
                result += "\n" + "\t" * indent + ")"
                return result
            ...
            for i, item in enumerate(sexp):
                if i == 0:
                    ...
                    result += self._format_sexp(item, indent, tag_name, in_number=False, in_instances=in_instances, in_lib_symbols=in_lib_symbols)
                else:
                    # Other items on new lines with indentation
                    # For number expressions, pass in_number=True for the value at index 1
                    if is_number_expr and i == 1:
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_number=True, in_instances=in_instances)
                    # For name expressions, pass in_name=True for the value at index 1
                    elif is_name_expr and i == 1:
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_name=True, in_instances=in_instances)
                    elif is_project_expr and i == 1:
                        # For project expressions, pass in_project=True for the value at index 1
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_project=True, in_instances=in_instances)
                    elif is_instances_expr:
                        # Inside instances, pass the flag down
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_instances=True)
                    elif is_lib_symbols_expr:
                        # Inside lib_symbols, pass the flag down
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_lib_symbols=True, in_instances=in_instances)
                    elif is_page_expr and i == 1:
                        # For page expressions, pass in_page=True for the value at index 1
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_page=True, in_instances=in_instances)
                    elif is_property_expr and i == 1:
                        # For property expressions, pass in_property_name=True for the name at index 1
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_property_name=True, in_instances=in_instances)
                    elif is_property_expr and i == 2:
                        # For property expressions, pass in_property_value=True for the value at index 2
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_property_value=True, in_instances=in_instances)
                    elif is_symbol_expr and i == 1:
                        # For symbol expressions, the library ID at index 1 should be quoted
                        result += "\n" + "\t" * (indent + 1) + self._format_sexp(item, indent + 1, tag_name, in_symbol=True, in_instances=in_instances)
                    elif is_generator_expr and i == 1:
                        # For generator expressions, the value at index 1 should be quoted
    ...
    def to_schematic(self, sexp_data: Any) -> Schematic:
        ...
        if not self._is_sexp_list(sexp_data) or not self._get_symbol_name(sexp_data[0]) == "kicad_sch":
    ...
    def _parse_sheet(self, sexp: List) -> Optional[Sheet]:
        ...
        try:
            ...
            # Parse properties
            for elem in sexp:
                if self._is_sexp_list(elem) and self._get_symbol_name(elem[0]) == "property":
            ...
            sheet = Sheet(name=name, filename=filename, position=position, size=size, pins=pins)
            if uuid_val:
                sheet.uuid = uuid_val
    ...
    def _parse_sheet_pin(self, sexp: List) -> Optional[SheetPin]:
        ...
        try:
            ...
            pin = SheetPin(name=name, position=position, orientation=orientation, shape=shape)
            if uuid_val:
                pin.uuid = uuid_val
    ...
    def _symbol_to_sexp(self, symbol: SchematicSymbol) -> List:
        ...
        at_expr = [sexpdata.Symbol("at"), symbol.position.x, symbol.position.y, int(symbol.rotation)]
        sexp.append(at_expr)
        ...
        sexp.append([sexpdata.Symbol("in_bom"), sexpdata.Symbol("yes") if symbol.in_bom else sexpdata.Symbol("no")])
        sexp.append([sexpdata.Symbol("on_board"), sexpdata.Symbol("yes") if symbol.on_board else sexpdata.Symbol("no")])
        sexp.append([sexpdata.Symbol("dnp"), sexpdata.Symbol("yes") if symbol.dnp else sexpdata.Symbol("no")])
        ...
        if symbol.reference:
            ...
            prop.append([sexpdata.Symbol("at"), symbol.position.x, symbol.position.y - 5, 0])
            prop.append([sexpdata.Symbol("effects"),
        ...
        if symbol.footprint:
            ...
            # Position relative to symbol
            prop.append([sexpdata.Symbol("at"), symbol.position.x, symbol.position.y + 10, 0])
            prop.append([sexpdata.Symbol("effects"),
                        [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]],
        ...
        internal_properties = {'hierarchy_path', 'ki_keywords', 'ki_description', 'Reference', 'Value', 'Footprint', 'Datasheet', 'Description', 'project_name', 'hierarchical_path'}
        for name, value in symbol.properties.items():
            if name not in internal_properties:
                prop = [sexpdata.Symbol("property"), name, str(value)]
                # Position relative to symbol
                prop.append([sexpdata.Symbol("at"), symbol.position.x, symbol.position.y + 15, 0])
                prop.append([sexpdata.Symbol("effects"),
                            [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]],
                            [sexpdata.Symbol("hide"), sexpdata.Symbol("yes")]])
                sexp.append(prop)
        ...
        if hasattr(symbol, 'instances'):
            logger.debug(f"  Number of instances: {len(symbol.instances) if symbol.instances else 0}")
            logger.debug(f"  Instances: {symbol.instances}")
        
        if hasattr(symbol, 'instances') and symbol.instances:
            logger.debug(f"  Creating instances S-expression for {len(symbol.instances)} instance(s)")
            ...
            for project_name, project_inst_list in project_instances.items():
                ...
                for inst in project_inst_list:
                    ...
                    project_block = [
                        ...
                        [
                            sexpdata.Symbol("path"), inst.path,  # Path will be quoted by formatter
    ...
    def _wire_to_sexp(self, wire: Wire) -> List:
        ...
        stroke_type = sexpdata.Symbol(wire.stroke_type) if wire.stroke_type else sexpdata.Symbol("default")
    ...
    def _sheet_to_sexp(self, sheet: Sheet) -> List:
        ...
        if sheet.name:
            ...
            prop.append([sexpdata.Symbol("at"), sheet.position.x, sheet.position.y - 1.27, 0])
            prop.append([sexpdata.Symbol("effects"),
                        [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]],
        ...
        if sheet.filename:
            ...
            prop.append([sexpdata.Symbol("at"), sheet.position.x, sheet.position.y + sheet.size[1] + 1.27, 0])
            prop.append([sexpdata.Symbol("effects"),
                        [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]],
        ...
        for pin in sheet.pins:
            ...
            at_expr = [sexpdata.Symbol("at"), pin.position.x, pin.position.y, pin.orientation]
            logger.debug(f"Creating 'at' expression for pin '{pin.name}': {at_expr}")
            pin_sexp.append(at_expr)
            pin_sexp.append([sexpdata.Symbol("effects"),
                             [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]],
        ...
        project_instance = [
            sexpdata.Symbol("project"), "circuit_synth",
            [
    ...
    def _symbol_definition_to_sexp(self, symbol_def) -> List:
        ...
        sexp.append([sexpdata.Symbol("pin_numbers"), [sexpdata.Symbol("hide"), sexpdata.Symbol("no")]])
        ...
        if symbol_def.graphic_elements:
            ...
            symbol_name = symbol_def.lib_id.split(":")[-1] if ":" in symbol_def.lib_id else symbol_def.lib_id
        ...
        if symbol_def.pins:
            ...
            for pin in symbol_def.pins:
                ...
                pin_sexp = [
                    sexpdata.Symbol("pin"),
                    sexpdata.Symbol(pin.type),  # electrical type (passive, input, output, etc.)
                ...
                pin_sexp.extend([
                    [sexpdata.Symbol("at"), pin.position.x, pin.position.y, pin.orientation],
                    [sexpdata.Symbol("length"), pin.length],
                    [sexpdata.Symbol("name"), str(pin.name),
                     [sexpdata.Symbol("effects"),
                      [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]]]],
                    # Pin number MUST be a quoted string
                    [sexpdata.Symbol("number"), str(pin.number),
                     [sexpdata.Symbol("effects"),
                      [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]]]]
                ])
    ...
    def _graphic_element_to_sexp(self, element: Dict[str, Any]) -> List:
        ...
        if elem_type == "rectangle":
            return [
                ...
                [sexpdata.Symbol("start"), element["start"]["x"], element["start"]["y"]],
                ...
                [sexpdata.Symbol("stroke"),
                 ...
                 [sexpdata.Symbol("type"), sexpdata.Symbol(element.get("stroke_type", "default"))]],
                [sexpdata.Symbol("fill"),
                 [sexpdata.Symbol("type"), sexpdata.Symbol(element.get("fill_type", "none"))]]
            ]
            ...
            if "center" in element and element["center"]:
                circle_sexp.append([sexpdata.Symbol("center"), element["center"]["x"], element["center"]["y"]])
            else:
            ...
            if "mid" in element and element["mid"]:
                arc_sexp.append([sexpdata.Symbol("mid"), element["mid"]["x"], element["mid"]["y"]])
    ...
    def _label_to_sexp(self, label: Label) -> List:
        ...
        at_expr = [sexpdata.Symbol("at"), label.position.x, label.position.y, int(label.orientation)]
        sexp.append(at_expr)
    ...
    def _rectangle_to_sexp(self, rect: Rectangle) -> List:
        ...
        sexp = [
            ...
            [sexpdata.Symbol("fill"), [sexpdata.Symbol("type"), sexpdata.Symbol(rect.fill_type)]],
