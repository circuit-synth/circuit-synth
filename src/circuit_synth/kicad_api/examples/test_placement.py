        ...
        Footprint(
            ...
            pads=[
                Pad(
                    ...
                    type="smd",
                    shape="rect",
                    position=Point(x=(i % 8) * 0.8 - 2.8, y=2.8 if i < 8 else -2.8),
                    size=(0.4, 1.2),
                )
                for i in range(16)
            ]
            + [
                Pad(
                    ...
                    type="smd",
                    shape="rect",
                    position=Point(x=2.8 if i < 8 else -2.8, y=(i % 8) * 0.8 - 2.8),
                    size=(1.2, 0.4),
                )
                for i in range(16)
            ],
...
def test_hierarchical_placement():
    ...
    for fp in board.pcb_data["footprints"]:
        print(
            f"  {fp.reference} ({fp.value}) at ({fp.position.x:.2f}, {fp.position.y:.2f}) - path: {fp.path}"
        )
    ...
    if initial_bbox:
        ...
        print(
            f"Initial size: {initial_bbox[2]-initial_bbox[0]:.2f} x {initial_bbox[3]-initial_bbox[1]:.2f} mm"
        )
    ...
    if new_bbox:
        ...
        print(
            f"New size: {new_bbox[2]-new_bbox[0]:.2f} x {new_bbox[3]-new_bbox[1]:.2f} mm"
        )
    ...
    for path, components in groups.items():
        ...
        for comp in components:
            print(
                f"    {comp.reference} at ({comp.position.x:.2f}, {comp.position.y:.2f})"
            )
...
def test_custom_spacing():
    ...
    board.auto_place_components(
        algorithm="hierarchical", component_spacing=0.5, group_spacing=2.0
    )

    tight_bbox = board.get_placement_bbox()
    if tight_bbox:
        print(
            f"Tight spacing size: {tight_bbox[2]-tight_bbox[0]:.2f} x {tight_bbox[3]-tight_bbox[1]:.2f} mm"
        )
    ...
    board.auto_place_components(
        algorithm="hierarchical", component_spacing=2.0, group_spacing=10.0
    )

    loose_bbox = board.get_placement_bbox()
    if loose_bbox:
        print(
            f"Loose spacing size: {loose_bbox[2]-loose_bbox[0]:.2f} x {loose_bbox[3]-loose_bbox[1]:.2f} mm"
        )
