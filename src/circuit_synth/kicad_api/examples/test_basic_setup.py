...
def test_basic_types():
    ...
    symbol = SchematicSymbol(
        reference="R1", value="10k", lib_id="Device:R", position=Point(50.0, 50.0)
    )
    ...
    # Create a label
    label = Label(text="VCC", position=Point(5, 0), label_type=LabelType.GLOBAL)
...
def test_symbol_cache():
    ...
    if resistor:
        ...
        for pin in resistor.pins:
            print(
                f"    Pin {pin.number}: {pin.name} ({pin.type}) at ({pin.position.x}, {pin.position.y})"
            )
    else:
