...
def test_sheet_hierarchy():
    ...
    root_schematic = Schematic(title="Main Board", date="2025-06-08", revision="1.0")
    ...
    if power_sheet:
        print(
            f"   ✓ Created sheet '{power_sheet.name}' at ({power_sheet.position.x}, {power_sheet.position.y})"
        )
        print(f"   ✓ Size: {power_sheet.size[0]} x {power_sheet.size[1]} mm")

    # Create a microcontroller sheet
    mcu_sheet = sheet_manager.add_sheet(
        name="Microcontroller", filename="mcu.kicad_sch", position=(200.0, 50.0)
    )
    if mcu_sheet:
    ...
    suggested_size = calculate_sheet_size_from_content(
        components_count=10, pins_count=8
    )
    print(f"   ✓ Suggested sheet size for 10 components, 8 pins: {suggested_size}")

    # Suggest pin side
    suggested_side = suggest_pin_side("DATA_OUT", "output", power_sheet.pins)
    ...
    vcc_label = label_manager_power.add_label(
        "VCC", (76.2, 25.4), label_type=LabelType.HIERARCHICAL
    )
    gnd_label = label_manager_power.add_label(
        "GND", (76.2, 38.1), label_type=LabelType.HIERARCHICAL
    )
    enable_label = label_manager_power.add_label(
        "ENABLE", (0, 25.4), label_type=LabelType.HIERARCHICAL
    )
    ...
    vreg = comp_manager_power.add_component(
        library_id="Regulator_Linear:LM7805", reference="U1", value="LM7805"
    )
    ...
    # Update sheet
    success = sheet_manager.update_sheet(power_sheet.uuid, name="PowerSupply_v2")
    if success:
    ...
    suggested_pos = suggest_sheet_position(
        [power_sheet, mcu_sheet], (76.2, 50.8), "right"
    )
    ...
    print(
        f"   - Total sheet pins: {sum(len(sheet.pins) for sheet in root_schematic.sheets)}"
    )
