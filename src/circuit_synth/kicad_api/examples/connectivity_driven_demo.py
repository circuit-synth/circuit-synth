...
def main():
    ...
    pcb.add_footprint_from_library(
        "Package_TO_SOT_THT:TO-220-3_Vertical", "U1", 20, 20, value="LM7805"
    )
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0805_2012Metric", "C1", 30, 20, value="10uF"
    )
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0805_2012Metric", "C2", 40, 20, value="100nF"
    )
    pcb.add_footprint_from_library("Diode_SMD:D_SMA", "D1", 50, 20, value="1N4007")

    # Microcontroller section - central hub
    pcb.add_footprint_from_library(
        "Package_QFP:TQFP-32_7x7mm_P0.8mm", "U2", 60, 50, value="ATmega328"
    )
    pcb.add_footprint_from_library(
        "Crystal:Crystal_SMD_HC49-SD", "Y1", 70, 50, value="16MHz"
    )
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0603_1608Metric", "C3", 80, 50, value="22pF"
    )
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0603_1608Metric", "C4", 90, 50, value="22pF"
    )

    # Peripheral ICs
    pcb.add_footprint_from_library(
        "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm", "U3", 30, 70, value="MAX232"
    )
    pcb.add_footprint_from_library(
        "Package_SO:SOIC-16_3.9x9.9mm_P1.27mm", "U4", 50, 70, value="74HC595"
    )
    ...
    pcb.add_footprint_from_library(
        "Connector_USB:USB_B_OST_USB-B1HSxx_Horizontal", "J3", 10, 80, value="USB"
    )

    # Decoupling capacitors - need to be near ICs
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0603_1608Metric", "C5", 25, 30, value="100nF"
    )
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0603_1608Metric", "C6", 35, 30, value="100nF"
    )
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0603_1608Metric", "C7", 45, 30, value="100nF"
    )
    pcb.add_footprint_from_library(
        "Capacitor_SMD:C_0603_1608Metric", "C8", 55, 30, value="100nF"
    )

    # Pull-up resistors
    pcb.add_footprint_from_library(
        "Resistor_SMD:R_0603_1608Metric", "R1", 65, 30, value="10k"
    )
    pcb.add_footprint_from_library(
        "Resistor_SMD:R_0603_1608Metric", "R2", 75, 30, value="10k"
    )
    pcb.add_footprint_from_library(
        "Resistor_SMD:R_0603_1608Metric", "R3", 85, 30, value="10k"
    )
    pcb.add_footprint_from_library(
        "Resistor_SMD:R_0603_1608Metric", "R4", 95, 30, value="10k"
    )
    ...
    print("\n✓ Demo complete!")
    print(
        "  Compare 'connectivity_driven_before.kicad_pcb' and 'connectivity_driven_after.kicad_pcb'"
    )
