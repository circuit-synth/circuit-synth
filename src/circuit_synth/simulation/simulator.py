...
class CircuitSimulator:
    ...
    def dc_analysis(self, source: str, start: float, stop: float, step: float) -> SimulationResult:
    ...
    def ac_analysis(self, start_freq: float, stop_freq: float, points: int = 100) -> SimulationResult:
        """Run AC analysis."""
        if not self.spice_circuit:
            raise RuntimeError("SPICE circuit not initialized")
