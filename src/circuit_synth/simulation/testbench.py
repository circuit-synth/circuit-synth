    ...
    def generate_stimulus_code(self) -> str:
        ...
        for name, signal in self.test_signals.items():
            if signal.signal_type == SignalType.SINE:
                ...
                code.append(f'{name}_source = Component("Device:V", ref="V", value="{signal.amplitude}V")')
                code.append(f"# Configure as sine: {signal.frequency}Hz, {signal.amplitude}V amplitude")
            elif signal.signal_type == SignalType.PULSE:
                code.append(f"# Pulse source: {name}")
                code.append(f'{name}_source = Component("Device:V", ref="V", value="PULSE")')
