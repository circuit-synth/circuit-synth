...
class DCAnalysis:
    ...
    def is_sweep(self) -> bool:
        ...
        return all(x is not None for x in [self.source, self.start, self.stop, self.step])
...
class CommonAnalyses:
    ...
    @staticmethod
    def dc_sweep(source: str, start: float, stop: float, points: int = 100) -> DCAnalysis:
        """DC sweep analysis."""
    ...
    @staticmethod
    def transient_step_response(duration_ms: float = 1.0, resolution_us: float = 1.0) -> TransientAnalysis:
        """Step response transient analysis."""
        return TransientAnalysis(
            step_time=resolution_us * 1e-6,
            end_time=duration_ms * 1e-3,
            start_time=0.0
        )
    
    @staticmethod
    def transient_pulse_response(pulse_width_us: float = 100, 
                               resolution_ns: float = 10) -> TransientAnalysis:
        """Pulse response transient analysis."""
        duration = pulse_width_us * 5  # 5x pulse width for settling
        return TransientAnalysis(
            step_time=resolution_ns * 1e-9,
            end_time=duration * 1e-6,
            start_time=0.0
        )