...
class SimulationVisualizer:
    ...
    def plot_time_domain(self, nodes: List[str], 
            ...
            try:
                ...
                if isinstance(voltage, list):
                    if time_array is not None:
                        ax.plot(time_array * 1000, voltage, label=f"V({node})", linewidth=2)
                    else:
    ...
    def plot_bode(self, input_node: str, output_node: str,
    ...
    def generate_pdf_report(self, filepath: str, 
        ...
        with PdfPages(filepath) as pdf:
            ...
            fig.suptitle(f"SPICE Simulation Report\n{circuit_name}", 
...
- Simulation completed successfully
            ...
            ax.text(0.1, 0.7, report_text, fontsize=12, 
            ...
            if include_plots:
                if self.analysis_type == "transient":
                    ...
                    fig = self.plot_time_domain(
                        self.result.list_nodes()[:4],  # First 4 nodes
                        show=False
                    )
                    if fig:
                        pdf.savefig(fig, bbox_inches='tight')
    ...
    def create_interactive_plot(self, nodes: List[str], 
