...
class SpiceConverter:
    ...
    def _add_opamp(self, component, ref: str, value: str):
        ...
        nodes = self._get_component_nodes(component)
        if len(nodes) < 3:
            logger.warning(f"Op-amp {ref} needs at least 3 connections, got {len(nodes)}")
            return
        ...
        self.spice_circuit.VCVS(ref, nodes[0], self.spice_circuit.gnd, 
                                nodes[1], nodes[2], gain)
    ...
    def _get_component_nodes(self, component) -> List[str]:
        ...
        if not nodes:
            logger.warning(f"No connections found for component {getattr(component, 'ref', 'unknown')}")
    ...
    def _convert_value_to_spice(self, value: str, component_type: str) -> float:
        ...
        multipliers = {
            # Resistance
            'r': 1, 'ohm': 1, 'ohms': 1,
            'k': 1e3, 'kohm': 1e3, 'kohms': 1e3,
            'm': 1e6, 'meg': 1e6, 'mohm': 1e6, 'mohms': 1e6,
            # Capacitance  
            'f': 1, 'pf': 1e-12, 'nf': 1e-9, 'uf': 1e-6, 'mf': 1e-3,
            'p': 1e-12, 'n': 1e-9, 'u': 1e-6,
            # Inductance
            'h': 1, 'nh': 1e-9, 'uh': 1e-6, 'mh': 1e-3,
        }
    ...
    def _add_power_sources(self):
        ...
        for net in nets_to_process:
            ...
            if any(pattern in net_name for pattern in ['VCC', 'VDD', 'V+', '+5V', '+3V3', '+12V']):
        ...
        for i, (net_name, voltage) in enumerate(power_nets):
            ...
            spice_node = self.node_map.get(net_name, net_name)
            self.spice_circuit.V(source_name, spice_node, self.spice_circuit.gnd, voltage@u_V)
