...
from circuit_synth.kicad_api.pcb import KiCadCLIError, PCBBoard, get_kicad_cli
...
def create_example_pcb():
    ...
    pcb.add_footprint(
        "U1", "Package_TO_SOT_THT:TO-220-3_Vertical", 20, 20, value="LM7805"
    )
    pcb.add_footprint("C1", "Capacitor_SMD:C_0805_2012Metric", 35, 20, value="100uF")
    pcb.add_footprint("C2", "Capacitor_SMD:C_0603_1608Metric", 45, 20, value="100nF")

    # MCU section - placed too close to edge
    pcb.add_footprint(
        "U2", "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm", 95, 40, value="ATtiny85"
    )  # Too close to edge!

    # Resistors with one unconnected
    pcb.add_footprint("R1", "Resistor_SMD:R_0603_1608Metric", 50, 40, value="10k")
    pcb.add_footprint("R2", "Resistor_SMD:R_0603_1608Metric", 50, 50, value="10k")
    ...
    pcb.add_track(
        50, 40, 50, 45, width=0.1, layer="F.Cu", net=pcb.get_net_by_name("SIGNAL1")
    )

    # Normal track
    pcb.add_track(
        20, 25, 35, 25, width=0.5, layer="F.Cu", net=pcb.get_net_by_name("VIN")
    )
...
def run_basic_checks(pcb):
    ...
    for ref, lib, x, y in pcb.list_footprints():
        if (
            x < edge_clearance
            or x > board_width - edge_clearance
            or y < edge_clearance
            or y > board_height - edge_clearance
        ):
    ...
    for track in pcb.get_tracks():
        if track.width < min_track_width:
            print(
                f"  ⚠️  Track at ({track.start.x}, {track.start.y}) has width {track.width}mm (minimum: {min_track_width}mm)"
            )
    ...
    try:
        ...
        if result.warnings:
            ...
            for w in result.warnings:
                print(
                    f"  - {w.get('type', 'Unknown')}: {w.get('description', 'No description')}"
                )
