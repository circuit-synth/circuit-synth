...
def create_simple_circuit():
    ...
    led_results = pcb.search_footprints(
        "LED", filters={"footprint_type": "SMD", "pad_count": 2}
    )
    if led_results:
    ...
    conn_results = pcb.search_footprints(
        "PinHeader_1x03", filters={"footprint_type": "THT"}
    )
    if conn_results:
    ...
    pcb.auto_place_components(
        algorithm="hierarchical", component_spacing=2.0, group_spacing=5.0
    )
    ...
    if bbox:
        ...
        pcb.set_board_outline_rect(
            bbox[0] - margin, bbox[1] - margin, width + 2 * margin, height + 2 * margin
        )
    ...
    for conn in ratsnest[:5]:  # Show first 5
        print(
            f"     - {conn['from']['ref']}.{conn['from']['pad']} to "
