...
from typing import Union, Dict, Set, List, Optional, Tuple
from collections import defaultdict

from ..io.json_loader import load_circuit_from_json_file, load_circuit_from_dict
...
def circuit_synth_json_to_python_project(
    ...
    circuit_name_map = {}
    used_sanitized = set()
    _phase2_rename_circuits(root_circuit, circuit_name_map, used_sanitized, is_top_level=True)
    ...
    circuit_to_filename = {}
    _phase7_write_all_circuits_organized(root_circuit, out_path, common_map, circuit_to_filename)

    # 10) Write main.py
    _phase8_generate_main_py(root_circuit, out_path, circuit_to_filename, highest_refs=highest_refs)
...
def _phase2_rename_circuits(c: Circuit, name_map: Dict[Circuit,str], used: Set[str], is_top_level: bool):
    import re
...
def _phase3_write_common_parts(d: Dict[Tuple[str,str,str], str], outp: Path, highest_refs: Dict[str, int]):
    ...
    with open(fp,"w",encoding="utf-8") as f:
        ...
        for (sym,val,fpt),varnm in d.items():
            ...
            f.write(f"        ref='{ref_prefix}',  # prefix only - number assigned by circuit starting from {ref_prefix}{start_num}\n")
            f.write(f"        value={vs},\n")
...
def _phase4_compute_net_owners_lca(root: Circuit, usage: Dict[str,Set[Circuit]])->Dict[str,Circuit]:
    ancestry={}
    ...
    for net, used_by in usage.items():
        if not used_by:
            ...
            owners[net] = lca
        logger.debug("Net '%s' used by %s => owner: %s",
                     net,[c.name for c in used_by], owners[net].name)
    return owners
...
def _phase6_compute_imported_nets_with_descendants(root: Circuit, net_owner_map: Dict[str,Circuit]):
    """
...
def _phase7_write_all_circuits_organized(root: Circuit, out: Path,
...
def _write_all_circuits_standard(root: Circuit, circuits_dir: Path,
                               common_map: Dict[Tuple[str,str,str], str],
                               circuit_to_file: Dict[Circuit,str]):
    """
    ...
    def rec(c: Circuit, parent: Optional[Circuit]):
        ...
        logger.info("Writing circuit file: circuits/%s => circuit name: '%s'", fpy.name, c.name)
...
def _generate_circuit_py_for_circuits_folder(c: Circuit,
                                           parent: Optional[Circuit],
                                           common_map: Dict[Tuple[str,str,str], str]) -> str:
    """
    ...
    lines.append(f"    logger.info(\"Entering {build_func} with imported nets: %s\", {imported})")
...
def _phase8_generate_main_py(root: Circuit,
                             out_dir: Path,
                             circuit_to_file: Dict[Circuit,str],
                             main_fn="main.py",
    ...
    if highest_refs:
        ...
        lines.append("        logger.info('Set initial reference counters: %s', _initial_counters)")
