...
def main(circuit_path: str, output: str, verbose: bool, top: int, threshold: int):
    ...
    console.print(Panel.fit(
        Text("🔍 FMEA Circuit Analysis Tool", style="bold blue"),
        style="blue"
    ))
    ...
    try:
        ...
        stats_table.add_row("🔴 Critical (≥300)", str(critical), f"{critical/total*100:.1f}%")
        stats_table.add_row("🟠 High (≥{})".format(threshold), str(high), f"{high/total*100:.1f}%")
        stats_table.add_row("🟡 Medium (50-{})".format(threshold-1), str(medium), f"{medium/total*100:.1f}%")
        ...
        console.print(f"  Total components analyzed: {circuit_data.get('component_count', 'N/A')}")
        ...
        if report_path:
            ...
            console.print(f"✅ Report generated: {report_path} ({file_size:.1f} KB)", style="green")
