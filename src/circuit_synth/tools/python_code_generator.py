...
class PythonCodeGenerator:
    ...
    def _get_ancestors(self, circuit_name: str, child_to_parent: Dict[str, str]) -> List[str]:
        """
    ...
    def _get_depth(self, circuit_name: str, hierarchy_tree: Dict[str, List[str]]) -> int:
        """
    ...
    def _find_lowest_common_ancestor(self, net_users: List[str], hierarchy_tree: Dict[str, List[str]]) -> str:
        """
        ...
        deepest_ancestor = max(common_ancestors, 
    ...
    def _determine_net_scope(self, net_name: str, net_users: List[str], 
        ...
        global_nets = ['GND', 'GROUND', 'VSS', 'VSSA', 'VCC', 'VDD', 'VDDA', 'VBUS', 'VIN']
        if net_name in global_nets or net_name.startswith('VCC_') or net_name.startswith('VDD_'):
            return "main"
    ...
    def _analyze_hierarchical_nets(self, circuits: Dict[str, Circuit], hierarchical_tree: Optional[Dict] = None) -> Tuple[Dict[str, Dict[str, Net]], Dict[str, List[str]]]:
        ...
        """
        logger.info("🔍 NET_ANALYSIS: Starting hierarchical net analysis with LCA algorithm")
        ...
        if hierarchical_tree:
            ...
            logger.info(f"🔍 NET_ANALYSIS: Built fallback hierarchy tree: {hierarchy_tree}")
        ...
        for net_name, using_circuits in net_usage.items():
            ...
            net_scope = self._determine_net_scope(net_name, list(using_circuits), hierarchy_tree)
            ...
            logger.info(f"🔍 NET_ANALYSIS: {net_name} -> created at {net_scope}, shared with {[c for c in using_circuits if c != net_scope]}")
        
        logger.info("🔍 NET_ANALYSIS: Hierarchical net analysis complete using LCA algorithm")
    ...
    def _generate_multiple_files(
        ...
        try:
            ...
            top_level_circuits = self._identify_top_level_circuits(hierarchical_tree or {})
            logger.info(f"🗂️ MULTI_FILE: Top-level circuits (direct children of main): {top_level_circuits}")
            ...
            main_code = self._generate_main_file_with_imports(main_circuit, top_level_circuits, shared_nets_per_subcircuit, main_nets)
    ...
    def _get_child_interface_nets(self, child_circuit: str, circuits: Dict[str, Circuit], 
                                 net_assignments: Dict[str, str]) -> List[str]:
        """
    ...
    def _generate_hierarchical_circuit_recursive(self, circuit_name: str, circuits: Dict[str, Circuit],
        ...
        if circuit_name not in circuits:
            logger.warning(f"🔧 RECURSIVE_GEN: Circuit {circuit_name} not found in circuits")
            return code_lines
        
        circuit = circuits[circuit_name]
        
        # Generate nets that belong to this circuit scope
        local_nets = [(net_name, net_obj) for net_name, scope in net_assignments.items() 
        ...
        if children:
            ...
            for child_name in children:
                ...
                child_interface_nets = self._get_child_interface_nets(child_name, circuits, net_assignments)
                
                if child_interface_nets:
                    net_vars = [self._sanitize_variable_name(net_name) for net_name in child_interface_nets]
                    params = ", ".join(net_vars)
                    code_lines.append(f"{indent}{child_name.lower()}_circuit = {child_name.lower()}({params})")
                else:
                    code_lines.append(f"{indent}{child_name.lower()}_circuit = {child_name.lower()}()")
        ...
        if circuit.nets:
            ...
            for net in connected_nets:
                if net.connections and net.name in [name for name, scope in net_assignments.items() if scope == circuit_name]:
    ...
    def _generate_main_circuit_code_with_params(self, circuit: Circuit, subcircuit_names: List[str], shared_nets_per_subcircuit: Dict[str, List[str]], main_nets: Dict[str, Net] = None) -> List[str]:
        ...
        logger.info("🎯 MAIN_CIRCUIT_PARAMS: Generating main circuit code with hierarchical structure")
        ...
        if subcircuit_names:
            ...
            for subcircuit_name in subcircuit_names:
                ...
                if shared_nets:
                    ...
                    code_lines.append(f"    {subcircuit_name.lower()}_circuit = {subcircuit_name.lower()}({args_str})")
                else:
                    # No shared nets
                    code_lines.append(f"    {subcircuit_name.lower()}_circuit = {subcircuit_name.lower()}()")
    ...
    def update_python_file(
        self, python_file: Path, circuits: Dict[str, Circuit], preview_only: bool = True, hierarchical_tree: Optional[Dict] = None
    ) -> Optional[str]:
        ...
        if hierarchical_tree:
            logger.info(f"🔧 CODE_UPDATE_DEBUG: Using hierarchical tree: {hierarchical_tree}")
        else:
            logger.warning("🔧 CODE_UPDATE_DEBUG: No hierarchical tree provided, will use fallback")
        ...
        try:
            ...
            if is_hierarchical:
                ...
                return self._generate_multiple_files(python_file, circuits, preview_only, hierarchical_tree)
            else:
