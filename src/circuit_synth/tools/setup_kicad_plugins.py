...
def get_kicad_plugin_directories() -> Dict[str, Path]:
    ...
    if system == "Darwin":  # macOS
        return {
            "user": Path.home() / "Library" / "Application Support" / "kicad" / "scripting" / "plugins",
            "system": Path("/Applications/KiCad/KiCad.app/Contents/SharedSupport/scripting/plugins")
        }
    elif system == "Windows":
        return {
            "user": Path.home() / "AppData" / "Roaming" / "kicad" / "scripting" / "plugins",
            "system": Path("C:/Program Files/KiCad/share/kicad/scripting/plugins")
        }
    else:  # Linux
        return {
            "user": Path.home() / ".local" / "share" / "kicad" / "8.0" / "3rdparty" / "plugins",
...
def show_manual_instructions(plugin_dirs: Dict[str, Path], source_dir: Path):
    ...
    if system == "Darwin":  # macOS
        ...
        console.print(f"copy \"{source_dir}\\*.py\" \"{plugin_dirs['user']}\"", style="dim")
...
@click.command()
@click.option("--manual", is_flag=True, help="Show manual installation instructions only")
@click.option("--system", is_flag=True, help="Install to system-wide directory (requires admin)")
def main(manual: bool, system: bool):
    """Setup KiCad plugins for circuit-synth AI integration"""
    
    console.print(
        Panel.fit(
            Text("🔌 Circuit-Synth KiCad Plugin Setup", style="bold blue"), 
            style="blue"
        )
    )
    ...
    if system:
        console.print("⚠️  System installation requires administrator privileges", style="yellow")
