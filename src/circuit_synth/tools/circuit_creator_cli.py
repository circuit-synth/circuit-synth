...
from ..agents.circuit_creator_agent import circuit_creator, create_custom_circuit, register_circuit, list_circuits, use_circuit
...
def create_circuit_interactive():
    ...
    if result["success"]:
        ...
        for comp in result["components"]:
            ...
            print(f"  {stock_status} {comp['part_number']}: {comp['description']} (Stock: {comp['stock']})")
        ...
        if register_choice in ['y', 'yes']:
            ...
            if registration["success"]:
                ...
                print(f"❌ Registration failed: {registration.get('error', 'Unknown error')}")
        else:
            print(f"ℹ️  Circuit created but not registered. You can register it later if needed.")
...
Examples:
  ...
  circuit-creator --example              # Create example requirements file
    ...
    action_group.add_argument("--interactive", "-i", action="store_true",
                            help="Interactive circuit creation")
    action_group.add_argument("--file", "-f", type=str,
                            help="Create circuit from JSON requirements file")
    action_group.add_argument("--list", "-l", action="store_true",
                            help="List registered circuits")
    action_group.add_argument("--use", "-u", type=str,
                            help="Use a registered circuit (provide circuit ID)")
    action_group.add_argument("--example", "-e", action="store_true",
                            help="Create example requirements file")
    
    # Optional arguments
    parser.add_argument("--category", "-c", type=str,
                       help="Filter by category (for --list)")
    parser.add_argument("--circuits-dir", type=str,
    ...
    if args.circuits_dir:
        ...
        circuit_creator.registry_file = circuit_creator.circuits_dir / "circuit_registry.json"
