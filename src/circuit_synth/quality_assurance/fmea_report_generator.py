...
class FMEAReportGenerator:
    ...
    def _create_title_page(self) -> List:
        ...
        # Title
        title = Paragraph(
            f"FMEA Analysis Report<br/>{self.project_name}",
    ...
    def _create_executive_summary(self, circuit_data: Dict, failure_modes: List[Dict]) -> List:
        ...
        high_risk_modes = sum(1 for fm in failure_modes if 125 <= fm.get('rpn', 0) < 300)
        avg_rpn = sum(fm.get('rpn', 0) for fm in failure_modes) / total_modes if total_modes > 0 else 0
        ...
        findings_data = [
            ['Metric', 'Value', 'Status'],
            ['Total Failure Modes Analyzed', str(total_modes), self._get_status_indicator(total_modes > 10)],
            ['Critical Risk Modes (RPN ≥ 300)', str(critical_modes), self._get_status_indicator(critical_modes == 0)],
            ['High Risk Modes (125 ≤ RPN < 300)', str(high_risk_modes), self._get_status_indicator(high_risk_modes <= 3)],
            ['Average RPN Score', f"{avg_rpn:.1f}", self._get_status_indicator(avg_rpn < 150)],
        ]
    ...
    def _create_system_overview(self, circuit_data: Dict) -> List:
        ...
        if 'subsystems' in circuit_data:
            ...
            for subsystem in circuit_data['subsystems']:
                bullet = f"• <b>{subsystem['name']}</b>: {subsystem.get('description', '')}"
    ...
    def _create_risk_matrix(self, failure_modes: List[Dict]) -> List:
        ...
        risk_table = Table(risk_data, colWidths=[1.5*inch, 1.5*inch, 1*inch, 2.5*inch])
    ...
    def _create_recommendations(self, failure_modes: List[Dict]) -> List:
        ...
        if high_priority:
            elements.append(Paragraph("Priority Actions", self.styles['CustomHeading2']))
            
            for fm in high_priority:
                recommendation = fm.get('recommendation', 'Review and implement appropriate mitigation')
                bullet = f"• <b>{fm.get('component', '')}</b> - {fm.get('failure_mode', '')}: {recommendation}"
                elements.append(Paragraph(bullet, self.styles['CustomBody']))
                
        # General recommendations
        elements.append(Paragraph("General Recommendations", self.styles['CustomHeading2']))
...
def analyze_circuit_for_fmea(circuit_path: str) -> Tuple[Dict, List[Dict]]:
    ...
    circuit_data = {
        ...
        'subsystem_count': 5,
        'subsystems': [
            {'name': 'USB-C Interface', 'description': 'USB-C connector with ESD protection'},
            {'name': 'Power Supply', 'description': '5V to 3.3V linear regulation'},
            {'name': 'ESP32-C6 MCU', 'description': 'Main microcontroller with WiFi/BLE'},
            {'name': 'Debug Interface', 'description': 'Programming and debugging header'},
