        ...
        class ComponentVisitor(ast.NodeVisitor):
            ...
            def visit_Call(self, node):
                ...
                if isinstance(node.func, ast.Name) and node.func.id == 'Component':
                    ...
                    if component_info:
                        ref = component_info.get('ref', f'U{len(self.parser.components)+1}')
                        self.parser.components[ref] = component_info
                        
                        # Add to current subcircuit if in a function
                        if self.current_function and self.current_function in self.parser.subcircuits:
                            self.parser.subcircuits[self.current_function]['components'].append(ref)
                ...
                # Check for Net() calls
                    ...
                    if net_name:
                        ...
                        # Add to current subcircuit if in a function
                        if self.current_function and self.current_function in self.parser.subcircuits:
                            self.parser.subcircuits[self.current_function]['nets'].append(net_name)
