    ...
    for component_name, description in components_to_check:
        ...
        try:
            ...
            if results:
                ...
                print(f"   ✅ Found: {best_match['lcsc_part']} - ${best_match.get('price', 'N/A')} - Stock: {best_match.get('stock', 'Unknown')}")
            else:
...
def demo_component_alternatives():
    ...
    try:
        ...
        for i, alt in enumerate(alternatives[:3], 1):
            print(f"   {i}. {alt.get('mfr_part', 'Unknown')} - ${alt.get('price', 'N/A')} - {alt.get('stock', 'Unknown')} in stock")
