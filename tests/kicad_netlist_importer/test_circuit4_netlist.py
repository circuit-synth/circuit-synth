    ...
    def test_import_circuit4(self):
        ...
        for component_ref, component_data in data["components"].items():
            self.assertIn(
                "value",
                component_data,
                f"Component {component_ref} missing 'value' property",
            )
            if "footprint" in component_data:
                self.assertIsInstance(
                    component_data["footprint"],
                    str,
        ...
        self.assertGreater(
            len(subcircuits), 0, "No subcircuits found in the hierarchical circuit"
        )
        ...
        for subcircuit in subcircuits:
            ...
            self.assertIn(
                "components", subcircuit, "Subcircuit missing 'components' field"
            )
            self.assertIn("nets", subcircuit, "Subcircuit missing 'nets' field")
            self.assertIn(
                "subcircuits", subcircuit, "Subcircuit missing 'subcircuits' field"
            )
            ...
            if len(subcircuit["subcircuits"]) > 0:
                for nested_subcircuit in subcircuit["subcircuits"]:
                    self.assertIn(
                        "name",
                        nested_subcircuit,
                        "Nested subcircuit missing 'name' field",
                    )
                    self.assertIn(
                        "components",
                        nested_subcircuit,
                        "Nested subcircuit missing 'components' field",
                    )
                    self.assertIn(
                        "nets",
                        nested_subcircuit,
                        "Nested subcircuit missing 'nets' field",
                    )
            ...
            for net_name in ["GND", "+3V3", "+5V"]:
                if net_name in data["nets"]:
                    ...
                    if (
                        len(subcircuit["components"]) > 0
                        and len(subcircuit["nets"]) > 0
                    ):
        ...
        print(
            f"circuit4.net => JSON output written to: {self.OUTPUT_JSON_PATH.resolve()}"
        )
