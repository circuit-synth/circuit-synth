...
class TestCircuit1NetlistImporter(unittest.TestCase):
    ...
    def test_import_circuit1(self):
        ...
        self.assertGreater(
            len(expected_components), 0, "No components found in the circuit"
        )
        ...
        for net_name, net_data in nets.items():
            ...
            self.assertIsInstance(
                net_data, list, f"Net {net_name} data should be a list (Structure A)"
            )
            ...
            for node in net_data:
                self.assertIn(
                    "component",
                    node,
                    f"Node in net {net_name} missing 'component' field",
                )
                self.assertIn(
                    "pin", node, f"Node in net {net_name} missing 'pin' field"
                )
                self.assertIn(
                    "number",
                    node["pin"],
                    f"Pin in net {net_name} missing 'number' field",
                )
        ...
        print(
            f"circuit1.net => JSON output written to: {self.OUTPUT_JSON_PATH.resolve()}"
        )
