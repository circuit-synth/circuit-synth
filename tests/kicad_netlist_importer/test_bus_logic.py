...
class TestKicadNestedHierarchyImporter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.NETLIST_PATH = (
            cls.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist7.net"
        )
    ...
    def test_import_nested_hierarchy_netlist(self):
        ...
        self.assertIsNotNone(
            esp32_3_subckt, "Should have 'esp32_3' subcircuit inside ESP321"
        )
        ...
        self.assertEqual(
            root_components["U1"]["footprint"], "RF_Module:ESP32-C6-MINI-1"
        )
        ...
        self.assertEqual(
            esp321_components["U2"]["footprint"], "RF_Module:ESP32-C6-MINI-1"
        )
        ...
        self.assertEqual(
            esp32_3_components["U3"]["footprint"], "RF_Module:ESP32-C6-MINI-1"
        )
        ...
        for bus_net in bus_nets:
            ...
            self.assertIn(
                bus_net, esp32_3_nets, f"esp32_3 should have {bus_net} bus net"
            )
        ...
        # For U1
        self._verify_unconnected_pins(
            root_nets, "U1", [8, 9, 12, 13, 19, 20, 26, 30, 31]
        )

        # For U2
        self._verify_unconnected_pins(
            esp321_nets, "U2", [8, 9, 12, 13, 19, 20, 26, 30, 31]
        )

        # For U3
        self._verify_unconnected_pins(
            esp32_3_nets, "U3", [8, 9, 12, 13, 17, 18, 19, 20, 30, 31]
        )
        ...
        self.assertGreaterEqual(
            len(esp321_nets), 32, "ESP321 should have at least 38 nets"
        )
        self.assertGreaterEqual(
            len(esp32_3_nets), 32, "esp32_3 should have at least 38 nets"
        )
        ...
        logger.info(
            f"Nested hierarchy test => JSON output written to: {self.OUTPUT_JSON.resolve()}"
        )

    def _verify_pin_connected(
        self, nets, net_name, component, pin_number, pin_function=None
    ):
        ...
        self.assertTrue(
            pin_connected, f"{component} pin {pin_number} should connect to {net_name}"
        )
    ...
    def _verify_pin_count(self, nets, net_name, component, expected_count):
        ...
        nodes = nets[net_name]  # Structure A
        actual_count = sum(
            1 for node in nodes if node["component"] == component
        )  # Already a list
            ...
            for variant in variants:
                if variant in nets:
                    ...
                    self.assertEqual(
                        len(nodes),
                        1,
                        f"Unconnected net {variant} should have exactly 1 node",
                    )
                    ...
                    self.assertEqual(
                        node["component"],
                        component,
                        f"Node should be connected to {component}",
                    )
                    self.assertEqual(
                        node["pin"]["number"],
                        str(pin),
                        f"Node should be connected to pin {pin}",
                    )
                    break

            self.assertTrue(
                found, f"No unconnected net found for {component} pin {pin}"
            )
