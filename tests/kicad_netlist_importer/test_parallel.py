...
class TestKicadParallelNetlistImporter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.NETLIST_PATH = cls.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist5.net"
    ...
    def test_import_parallel_netlist(self):
        ...
        self.assertGreaterEqual(len(subcircuits), 3, 
        ...
        self.assertIn("P1", usb_subckt["components"], "USB subcircuit should contain P1")
        self.assertIn("R1", usb_subckt["components"], "USB subcircuit should contain R1")

        # Check for power_filter subcircuit
        power_filter_subckt = find_subcircuit(subcircuits, "power_filter")
        self.assertIsNotNone(power_filter_subckt, "Should have 'power_filter' subcircuit at root level")
        self.assertIn("C4", power_filter_subckt["components"], "power_filter should contain C4")
        self.assertIn("C5", power_filter_subckt["components"], "power_filter should contain C5")
        self.assertIn("L1", power_filter_subckt["components"], "power_filter should contain L1")

        # Check for uSD_Card subcircuit
        usd_subckt = find_subcircuit(subcircuits, "uSD_Card")
        self.assertIsNotNone(usd_subckt, "Should have 'uSD_Card' subcircuit at root level")
        ...
        self.assertEqual(len(usb_children), 1, "USB should have 1 child subcircuit: 'regulator'")
        ...
        self.assertEqual(len(led_children), 1, "LED should have 1 child: 'light_sensor'")
        ...
        self.assertIn("+5V_USB", power_filter_subckt["nets"], "Power filter should have +5V_USB net")
        self.assertIn("+5V", power_filter_subckt["nets"], "Power filter should have +5V net")
        ...
        for net_name, net_info in usb_nets.items():
            if net_name == "+5V_USB":
                # Structure A: net_info is the list of nodes
                for node in net_info:
                    if node["component"] == "P1" and node["pin"]["number"] in ["A4", "A9", "B4", "B9"]:
                        p1_vbus_connected = True
                        break
        self.assertTrue(p1_vbus_connected, "P1's VBUS pins should be connected to +5V_USB net")
        ...
        for net_name, net_info in led_nets.items():
            ...
            for node in net_info:
                if node["component"] == "D1" and node["pin"]["number"] == "2":
                    ...
                    break
        self.assertEqual(d1_anode_net, "+5V", "D1 anode should be on +5V net in the LED subcircuit")

        # -------- Unconnected Pin Tests --------
        # Check for unconnected pins in various subcircuits
        self.assertTrue(any(n.startswith("unconnected-(U1-") for n in root_nets),
        ...
        logger.info(f"Parallel netlist => JSON output written to: {self.OUTPUT_JSON.resolve()}")
