...
class TestKicadHierarchicalNetlistImporter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.NETLIST3_PATH = cls.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist3.net"
    ...
    def test_import_hierarchical_netlist(self):
        ...
        self.assertEqual(data["name"], "netlist3", "Circuit name should match input file name")

        # -------- Hierarchical Structure Tests --------
        # Verify there's exactly one subcircuit named "usb"
        self.assertEqual(len(data["subcircuits"]), 1, "Should have exactly one subcircuit")
        usb_subcircuit = data["subcircuits"][0]
        self.assertEqual(usb_subcircuit["name"], "usb", "Subcircuit should be named 'usb'")
        
        # Verify subcircuit has correct structure
        subcircuit_fields = ["name", "components", "nets", "subcircuits"]
        for field in subcircuit_fields:
            self.assertIn(field, usb_subcircuit, f"Missing required subcircuit field: {field}")
        
        # Verify USB subcircuit doesn't have nested subcircuits (leaf node)
        self.assertEqual(len(usb_subcircuit["subcircuits"]), 0, "USB subcircuit should not have any child subcircuits")
        
        # -------- Component Placement Tests --------
        # Root level should have exactly U1
        self.assertEqual(len(data["components"]), 1, "Root circuit should have exactly one component")
        self.assertIn("U1", data["components"], "Root circuit should contain component U1")
        
        # U1 should be ESP32-C6-MINI-1
        u1 = data["components"]["U1"]
        self.assertEqual(u1["value"], "ESP32-C6-MINI-1", "U1 should be an ESP32-C6-MINI-1")
        self.assertEqual(u1["footprint"], "RF_Module:ESP32-C6-MINI-1", "U1 should have correct footprint")
        
        # USB subcircuit should have exactly P1 and R2
        usb_components = usb_subcircuit["components"]
        self.assertEqual(len(usb_components), 2, "USB subcircuit should have exactly two components")
        ...
        self.assertEqual(p1["value"], "USB_C_Plug_USB2.0", "P1 should be a USB_C_Plug_USB2.0")
        self.assertEqual(p1["description"], "USB 2.0-only Type-C Plug connector", "P1 should have correct description")
        
        # Verify R2 properties
        r2 = usb_components["R2"]
        self.assertEqual(r2["value"], "10k", "R2 should be a 10k resistor")
        self.assertEqual(r2["footprint"], "Resistor_SMD:R_0603_1608Metric", "R2 should have correct footprint")
        
        # -------- Net Distribution Tests --------
        # Check root circuit nets
        root_nets = data["nets"]
        expected_root_nets = ["+3V3", "D+", "D-", "GND"]  # These are the nets connected to U1
        for net_name in expected_root_nets:
            self.assertIn(net_name, root_nets, f"Root circuit should contain net {net_name}")
        
        # +5V should NOT be in root circuit (no connections there)
        self.assertNotIn("+5V", root_nets, "+5V should not appear in root circuit (no connections)")
        
        # Check USB subcircuit nets
        usb_nets = usb_subcircuit["nets"]
        expected_usb_nets = ["+5V", "D+", "D-", "GND", "Net-(P1-CC)", "unconnected-(P1-VCONN-PadB5)"]
        for net_name in expected_usb_nets:
            self.assertIn(net_name, usb_nets, f"USB subcircuit should contain net {net_name}")
        
        # +3V3 should NOT be in USB subcircuit (no connections there)
        self.assertNotIn("+3V3", usb_nets, "+3V3 should not appear in USB subcircuit (no connections)")
        
        # -------- Net Connectivity Tests --------
        # Test +3V3 net - should only connect to U1 pin 3
        plus3v3_nodes = root_nets["+3V3"] # Structure A
        self.assertEqual(len(plus3v3_nodes), 1, "+3V3 should connect to exactly one pin")
        self.assertEqual(plus3v3_nodes[0]["component"], "U1", "+3V3 should connect to U1")
        self.assertEqual(plus3v3_nodes[0]["pin"]["number"], "3", "+3V3 should connect to U1 pin 3")
        self.assertEqual(plus3v3_nodes[0]["pin"]["name"], "3V3", "+3V3 should connect to U1.3V3")
        self.assertEqual(plus3v3_nodes[0]["pin"]["type"], "power_in", "+3V3 pin should be power_in type")
        
        # Test D+ net in root - should connect to U1.IO20
        dplus_root_nodes = root_nets["D+"] # Structure A
        self.assertEqual(len(dplus_root_nodes), 1, "D+ in root should connect to exactly one pin")
        self.assertEqual(dplus_root_nodes[0]["component"], "U1", "D+ in root should connect to U1")
        self.assertEqual(dplus_root_nodes[0]["pin"]["number"], "26", "D+ in root should connect to U1 pin 26")
        self.assertEqual(dplus_root_nodes[0]["pin"]["name"], "IO20", "D+ in root should connect to U1.IO20")
        
        # Test D+ net in USB subcircuit - should connect to P1.A6
        dplus_usb_nodes = usb_nets["D+"] # Structure A
        self.assertEqual(len(dplus_usb_nodes), 1, "D+ in USB should connect to exactly one pin")
        self.assertEqual(dplus_usb_nodes[0]["component"], "P1", "D+ in USB should connect to P1")
        self.assertEqual(dplus_usb_nodes[0]["pin"]["number"], "A6", "D+ in USB should connect to P1 pin A6")
        self.assertEqual(dplus_usb_nodes[0]["pin"]["name"], "D+", "D+ in USB should connect to P1.D+")
        ...
        for pin_num in u1_gnd_pins:
            ...
            for node in gnd_root_nodes:
                if node["component"] == "U1" and node["pin"]["number"] == pin_num:
                    pin_found = True
                    break
            self.assertTrue(pin_found, f"GND in root should connect to U1 pin {pin_num}")
        ...
        self.assertEqual(len(cc_nodes), 2, "Net-(P1-CC) should have exactly 2 connections")
        ...
        unconnected_io_pins = ["IO0", "IO1", "IO2", "IO3", "IO4", "IO5", "IO6", "IO7", "IO8", "IO9", 
                              "IO12", "IO13", "IO14", "IO15", "IO18", "IO21", "IO22", "IO23"]
        ...
        self.assertIn("unconnected-(P1-VCONN-PadB5)", usb_nets, "Missing unconnected net for P1.VCONN")
        ...
        self.assertEqual(data["properties"]["tool"], "Eeschema 8.0.8", "Incorrect tool version")

        print(f"netlist3.net => JSON output written to: {self.OUTPUT3_JSON_PATH.resolve()}")
    ...
    def test_import_netlist4(self):
        ...
        NETLIST4_PATH = self.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist4.net"
        ...
        # 1) Verify file presence
        self.assertTrue(
            NETLIST4_PATH.is_file(),
            f"Missing test netlist file: {NETLIST4_PATH}"
        )
        ...
        # Circuit name should match input file name
        self.assertEqual(data["name"], "netlist4", "Circuit name should match input file name")

        # -------- Root Level Component Tests --------
        # Root level should have C1 and U1
        self.assertEqual(len(data["components"]), 2, "Root circuit should have exactly two components")
        self.assertIn("C1", data["components"], "Root circuit should contain component C1")
        self.assertIn("U1", data["components"], "Root circuit should contain component U1")
        
        # Verify C1 properties
        c1 = data["components"]["C1"]
        self.assertEqual(c1["value"], "10uF", "C1 should be a 10uF capacitor")
        self.assertEqual(c1["footprint"], "Capacitor_SMD:C_0603_1608Metric", "C1 should have correct footprint")
        ...
        self.assertEqual(len(data["subcircuits"]), 1, "Should have exactly one first-level subcircuit")
        usb_subcircuit = data["subcircuits"][0]
        self.assertEqual(usb_subcircuit["name"], "usb", "First subcircuit should be named 'usb'")
        ...
        self.assertEqual(len(usb_subcircuit["subcircuits"]), 1, "USB should have exactly one subcircuit")
        regulator = usb_subcircuit["subcircuits"][0]
        self.assertEqual(regulator["name"], "regulator", "Second-level subcircuit should be named 'regulator'")
        
        # Verify regulator components
        reg_components = regulator["components"]
        self.assertEqual(len(reg_components), 3, "Regulator subcircuit should have exactly three components")
        ...
        self.assertEqual(u2["value"], "AMS1117-3.3", "U2 should be an AMS1117-3.3 regulator")
        self.assertEqual(u2["footprint"], "Package_TO_SOT_SMD:SOT-223-3_TabPin2", "U2 should have correct footprint")
        
        # -------- Third-Level Hierarchy (LED) Tests --------
        # Regulator should have exactly one subcircuit (LED)
        self.assertEqual(len(regulator["subcircuits"]), 1, "Regulator should have exactly one subcircuit")
        led = regulator["subcircuits"][0]
        self.assertEqual(led["name"], "led", "Third-level subcircuit should be named 'led'")
        
        # Verify LED components
        led_components = led["components"]
        self.assertEqual(len(led_components), 4, "LED subcircuit should have exactly four components")
        ...
        self.assertEqual(led_components["R2"]["value"], "330r", "R2 should be a 330 ohm resistor")
        self.assertEqual(led_components["R3"]["value"], "220r", "R3 should be a 220 ohm resistor")
        
        # -------- Fourth-Level Hierarchy (Light Sensor) Tests --------
        # LED should have exactly one subcircuit (light_sensor)
        self.assertEqual(len(led["subcircuits"]), 1, "LED should have exactly one subcircuit")
        light_sensor = led["subcircuits"][0]
        self.assertEqual(light_sensor["name"], "light_sensor", "Fourth-level subcircuit should be named 'light_sensor'")
        
        # Verify light sensor components
        sensor_components = light_sensor["components"]
        self.assertEqual(len(sensor_components), 1, "Light sensor subcircuit should have exactly one component")
        self.assertIn("U3", sensor_components, "Light sensor subcircuit should contain U3")
        
        # Verify U3 properties
        u3 = sensor_components["U3"]
        self.assertEqual(u3["value"], "APDS-9301", "U3 should be an APDS-9301 light sensor")
        self.assertEqual(u3["footprint"], "OptoDevice:Broadcom_APDS-9301", "U3 should have correct footprint")
        ...
        self.assertEqual(u2_pins.get("1"), "GND", "U2 pin 1 (GND) should connect to GND")
        self.assertEqual(u2_pins.get("2"), "+3V3", "U2 pin 2 (VO) should connect to +3V3")
        ...
        self.assertEqual(d1_pins.get("1"), "Net-(D1-K)", "D1 cathode should connect to current limiting resistor")
        self.assertEqual(r2_pins.get("1"), "Net-(D1-K)", "R2 should connect to D1 cathode")
        ...
        self.assertEqual(d2_pins.get("1"), "Net-(D2-K)", "D2 cathode should connect to current limiting resistor")
        self.assertEqual(r3_pins.get("1"), "Net-(D2-K)", "R3 should connect to D2 cathode")
        ...
        self.assertEqual(u3_pins.get("1"), "+3V3", "U3 pin 1 (VDD) should connect to +3V3")
        self.assertEqual(u3_pins.get("2"), "GND", "U3 pin 2 (GND) should connect to GND")
        ...
        self.assertEqual(u3_pins.get("3"), "GND", "U3 pin 3 (ADR_SEL) should connect to GND")
        
        # -------- Unconnected Pin Tests --------
        # Verify some unconnected pins of U1 are correctly represented
        unconnected_io_pins = ["IO0", "IO1", "IO2", "IO3", "IO4", "IO5", "IO6", "IO7", "IO8", "IO9", 
                            "IO15", "IO18", "IO21", "IO22", "IO23"]
