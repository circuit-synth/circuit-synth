...
class TestKicadNetlistImporter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.NETLIST1_PATH = cls.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist1.net"
        cls.NETLIST2_PATH = cls.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist2.net"
    ...
    def test_import_netlist1(self):
        ...
        print(f"netlist1.net => JSON output written to: {self.OUTPUT1_JSON_PATH.resolve()}")
    ...
    def test_import_netlist2(self):
        ...
        self.assertEqual(data["components"]["D1"]["footprint"], "LED_SMD:LED_0603_1608Metric")
        self.assertEqual(data["components"]["D1"]["description"], "Light emitting diode")
        
        self.assertEqual(data["components"]["R1"]["value"], "10k")
        self.assertEqual(data["components"]["R1"]["footprint"], "Resistor_SMD:R_0603_1608Metric")
        self.assertEqual(data["components"]["R1"]["description"], "Resistor")
        
        self.assertEqual(data["components"]["U1"]["value"], "ESP32-C6-MINI-1")
        self.assertEqual(data["components"]["U1"]["footprint"], "RF_Module:ESP32-C6-MINI-1")
        self.assertTrue("RF Module, ESP32-C6 SoC" in data["components"]["U1"]["description"])
        ...
        self.assertEqual(data["nets"]["+3V3"][0]["pin"]["type"], "power_in") # Structure A
        ...
        expected_pin_types = {"power_in", "input", "bidirectional", "passive", "unspecified"}
        self.assertTrue(expected_pin_types.issubset(pin_types_found), 
        ...
        unconnected_nets = [net for net in data["nets"] if net.startswith("unconnected-")]
        self.assertGreaterEqual(len(unconnected_nets), 5)  # Should have multiple unconnected nets
        ...
        print(f"netlist2.net => JSON output written to: {self.OUTPUT2_JSON_PATH.resolve()}")
    ...
    def test_match_reference_json(self):
        ...
        expected_json_path = self.BASE_DIR / "test_data" / "kicad9" / "expected_outputs" / "netlist2_expected.json"
