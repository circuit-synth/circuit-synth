...
class NetlistParser:
    ...
    def parse(self):
        ...
        if not net_matches:
            ...
            net_blocks = re.split(r'(?=\(net\s+\(code)', nets_content)[1:]  # Skip empty first element
    ...
    def get_component_types(self) -> Dict[str, str]:
        ...
        return {ref: f"{comp.get('value', 'unknown')}" for ref, comp in self.components.items()}
...
def run_kicad_cli_netlist_export(schematic_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        pytest.fail("kicad-cli not found. Please ensure KiCad is installed and kicad-cli is in PATH.")
        return False


def compare_netlists(reference_netlist: NetlistParser, generated_netlist: NetlistParser) -> Tuple[bool, List[str]]:
    """
    ...
    if gen_components != expected_components:
        differences.append(f"Expected components {expected_components}, got {gen_components}")
    ...
    for net_name, expected_conn in expected_connections.items():
        ...
        if actual_conn != expected_conn:
            differences.append(f"Net '{net_name}' connections mismatch: expected {expected_conn}, got {actual_conn}")
    ...
    for net_name, actual_conn in normalized_nets.items():
        ...
        differences.append(f"Unexpected net '{net_name}' with connections {actual_conn}")
...
def test_resistor_divider_netlist_comparison():
    ...
    if not reference_sch_files:
        pytest.fail(f"No .kicad_sch files found in reference directory: {reference_dir}")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Compare netlists
        is_equivalent, differences = compare_netlists(reference_parser, generated_parser)
