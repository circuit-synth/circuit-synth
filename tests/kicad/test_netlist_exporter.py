...
class TestNetlistExporter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        assert cls.TEST_OUTPUT_DIR.exists(), f"Test output directory not found: {cls.TEST_OUTPUT_DIR}"
    ...
    def test_load_circuit_json(self):
        ...
        self.assertIn("components", circuit_data, "JSON data missing 'components' field")
    ...
    def test_convert_json_to_netlist(self):
        ...
        try:
            ...
            for component_ref in critical_components:
                self.assertIn(f"(ref \"{component_ref}\")", content, f"Missing critical component {component_ref}")
            ...
            for net_name in critical_nets:
                if net_name in circuit_data.get("nets", {}):
                    self.assertIn(f"(name \"{net_name}\")", content, f"Missing critical net {net_name}")
    ...
    def test_generate_netlist(self):
        ...
        self.assertIn("(export (version \"E\")", netlist_content, "Missing export section")
        ...
        # Check critical components
        for component_ref in critical_components:
            self.assertIn(f"(ref \"{component_ref}\")", netlist_content, f"Missing critical component {component_ref}")
        ...
        for net_name in critical_nets:
            if net_name in circuit_data.get("nets", {}):
                self.assertIn(f"(name \"{net_name}\")", netlist_content, f"Missing critical net {net_name}")
    ...
    def test_exported_netlist_matches_original(self):
        ...
        self.assertTrue(json_path.exists(), f"JSON file not found: {json_path}")
        self.assertTrue(original_netlist_path.exists(), f"Original netlist file not found: {original_netlist_path}")
        self.assertTrue(exported_netlist_path.exists(), f"Exported netlist file not found: {exported_netlist_path}")
        ...
        def extract_sections(content):
            ...
            components_match = re.search(r'\(\s*components(.*?)\)\s*\(\s*libparts', content, re.DOTALL)
            if components_match:
        ...
        original_components = re.findall(r'\(\s*comp\s+\(\s*ref\s+"([^"]+)"\s*\)', original_sections.get('components', ''))
        exported_components = re.findall(r'\(\s*comp\s+\(\s*ref\s+"([^"]+)"\s*\)', exported_sections.get('components', ''))
        ...
        for component in critical_components:
            self.assertIn(component, exported_components, f"Missing critical component {component} in exported netlist")
        
        # Verify that the exported netlist has all the critical nets from the original
        original_nets = re.findall(r'\(\s*net\s+\(\s*code\s+"[^"]+"\s*\)\s+\(\s*name\s+"([^"]+)"\s*\)', original_sections.get('nets', ''))
        exported_nets = re.findall(r'\(\s*net\s+\(\s*code\s+"[^"]+"\s*\)\s+\(\s*name\s+"([^"]+)"\s*\)', exported_sections.get('nets', ''))
        
        # Check for critical nets only
        critical_nets = ["+3V3", "+5V", "GND", "VCC", "VDD"]
        for net in critical_nets:
            if net in original_nets:
                self.assertIn(net, exported_nets, f"Missing critical net {net} in exported netlist")
    ...
    def test_hierarchical_nets_preserved(self):
        ...
        try:
            ...
            critical_net_base_names = ["D+", "D-", "SCL", "SDA"] # Base names of interest
            found_any_critical_hierarchical = False
            net_pattern = re.compile(r'\(\s*net\s+.*?\(name\s+"([^"]+)"\s*\).*?\)', re.DOTALL)
            exported_net_names = {match.group(1) for match in net_pattern.finditer(exported_content)}
            ...
            intended_critical_nets = {name for name in circuit_data.get("nets", {}).keys() if name in critical_net_base_names}
            ...
            if intended_critical_nets and not found_any_critical_hierarchical:
                 self.fail(f"Expected critical hierarchical nets ({intended_critical_nets}) but none found with hierarchical paths in exported netlist.")
            ...
            if not critical_subcircuits and circuit_data.get("subcircuits"):
                critical_subcircuits = [s.get("name", "") for s in circuit_data.get("subcircuits", [])]
            ...
            if critical_subcircuits:
                ...
                for subcircuit_name in critical_subcircuits:
                    ...
                    for component_ref in circuit_data.get("subcircuits", [])[0].get("components", {}):
                        component_pattern = re.compile(rf'\(\s*comp\s+\(\s*ref\s+"{re.escape(component_ref)}"\s*\)', re.DOTALL)
                ...
                self.assertTrue(found, f"No critical subcircuits found in exported netlist among: {critical_subcircuits}")
    ...
    def test_deep_hierarchical_structure(self):
        ...
        try:
            ...
            component_count = len(re.findall(r'\(\s*comp\s+\(\s*ref\s+"([^"]+)"\s*\)', exported_content))
            self.assertGreater(component_count, 0, "No components found in exported netlist")
            
            # Check that the netlist has at least some nets
            net_count = len(re.findall(r'\(\s*net\s+\(\s*code\s+"[^"]+"\s*\)\s+\(\s*name\s+"([^"]+)"\s*\)', exported_content))
            ...
            self.assertGreater(len(found_components), 0, f"No critical components found in exported netlist among: {critical_components}")
            ...
            self.assertGreater(len(found_nets), 0, f"No critical nets found in exported netlist among: {critical_nets}")
    ...
    def test_multiple_parallel_subcircuits(self):
        ...
        try:
            ...
            for subcircuit_name in critical_subcircuits:
                subcircuit = next((s for s in circuit_data.get("subcircuits", []) if s.get("name") == subcircuit_name), None)
                if subcircuit:
            ...
            self.assertGreater(len(found_subcircuits), 0, f"No critical subcircuits found in exported netlist among: {critical_subcircuits}")
