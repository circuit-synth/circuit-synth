...
from circuit_synth.kicad.netlist_exporter import convert_json_to_netlist, load_circuit_json
...
class TestNetlistExportComparison(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        assert len(cls.netlist_files) > 0, "No netlist files found in test data directory"
    ...
    def extract_components(self, content):
        ...
        comp_pattern = r'\(\s*comp\s+\(\s*ref\s+"([^"]+)"\s*\)(.*?)(?:\(\s*tstamps|\(\s*comp|\)$)'
        for match in re.finditer(comp_pattern, components_section, re.DOTALL):
            ...
            footprint_match = re.search(r'\(\s*footprint\s+"([^"]+)"\s*\)', comp_content)
            if footprint_match:
                properties['footprint'] = footprint_match.group(1)
            
            # Extract libsource with more flexible pattern
            libsource_match = re.search(r'\(\s*libsource\s+\(\s*lib\s+"([^"]+)"\s*\)\s+\(\s*part\s+"([^"]+)"\s*\)', comp_content)
            if libsource_match:
    ...
    def extract_nets(self, content):
        ...
        for match in re.finditer(net_pattern, nets_section, re.DOTALL):
            ...
            nodes = []
            node_pattern = r'\(\s*node\s+\(\s*ref\s+"([^"]+)"\s*\)\s+\(\s*pin\s+"([^"]+)"\s*\)'
    ...
    def compare_components(self, original_components, exported_components):
        ...
        for ref in critical_components:
            if ref not in exported_components:
                differences.append(f"Critical component {ref} missing from exported netlist")
                continue
            
            # Check value for critical components
            if 'value' in original_components[ref] and 'value' in exported_components[ref]:
                if original_components[ref]['value'] != exported_components[ref]['value']:
                    differences.append(f"Component {ref} value mismatch: {original_components[ref]['value']} vs {exported_components[ref]['value']}")
    ...
    def test_netlist2_export_comparison(self):
        ...
        # Compare components
        component_differences = self.compare_components(original_components, exported_components)
        self.assertEqual(len(component_differences), 0, f"Component differences found: {component_differences}")
        
        # Compare nets - filter out non-critical differences
        net_differences = self.compare_nets(original_nets, exported_nets)
        self.assertEqual(len(net_differences), 0, f"Critical net differences found: {net_differences}")
    ...
    def test_export_all_netlists(self):
        ...
        for netlist_file in self.netlist_files:
            ...
            try:
                ...
                # Compare components
                component_differences = self.compare_components(original_components, exported_components)
                self.assertEqual(len(component_differences), 0, 
                                f"Component differences found in {netlist_file.name}: {component_differences}")
                
                # Compare nets - filter out non-critical differences
                net_differences = self.compare_nets(original_nets, exported_nets)
                self.assertEqual(len(net_differences), 0, 
            ...
            try:
                ...
                def check_critical_subcircuits(subcircuits, path=""):
                        ...
                        if subcircuit_name in critical_subcircuit_names:
                            ...
                            for component_ref in subcircuit.get("components", {}):
                                ...
                                if not component_pattern.search(exported_content):
                                    self.fail(f"Missing component {component_ref} from critical subcircuit {subcircuit_name}")
                    ...
                    if critical_subcircuit_names and not critical_subcircuits_found:
                        self.fail(f"No critical subcircuits found among: {critical_subcircuit_names}")
                ...
                def check_critical_hierarchical_nets_in_export(exported_content):
                    ...
                    if intended_critical_nets and not found_any_critical:
                         self.fail(f"Expected critical hierarchical nets ({intended_critical_nets}) but none found in exported netlist content.")

                # Check critical hierarchical nets in the exported content
                check_critical_hierarchical_nets_in_export(exported_content)
            
            except Exception as e:
                self.fail(f"Error checking hierarchical structure for {netlist_name}: {e}")
