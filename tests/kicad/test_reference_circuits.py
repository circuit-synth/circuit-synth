...
class ReferenceCircuitTester:
    ...
    def test_multi_unit_component(self) -> bool:
        ...
        gen_path = self.reference_circuit_dir / 'reference_generated' / 'child2.kicad_sch'
    ...
    def test_hierarchical_sheets(self) -> bool:
        ...
        main_path = self.reference_circuit_dir / 'reference_generated' / 'reference_generated.kicad_sch'
    ...
    def compare_structures(self, gen: Dict, ref: Dict) -> List[str]:
        ...
        if len(gen['components']) != len(ref['components']):
            differences.append(f"Component count: {len(gen['components'])} vs {len(ref['components'])}")
            
        if len(gen['sheets']) != len(ref['sheets']):
            differences.append(f"Sheet count: {len(gen['sheets'])} vs {len(ref['sheets'])}")
            
        if gen['has_lib_symbols'] != ref['has_lib_symbols']:
            differences.append(f"lib_symbols: {gen['has_lib_symbols']} vs {ref['has_lib_symbols']}")
    ...
    def is_functionally_equivalent(self, gen: Dict, ref: Dict) -> bool:
        ...
        return (len(gen['components']) == len(ref['components']) and
                len(gen['sheets']) == len(ref['sheets']))
