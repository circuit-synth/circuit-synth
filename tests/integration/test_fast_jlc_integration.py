...
def test_find_cheapest():
    ...
    if result:
        ...
        assert 'F' in result.description.upper() or 'capacitor' in result.description.lower()
...
def test_basic_part_preference():
    ...
    if results:
        ...
        if basic_count > 0 and extended_count > 0:
            ...
            assert sum(basic_scores) / len(basic_scores) > sum(extended_scores) / len(extended_scores)
