...
from circuit_synth.kicad.netlist_exporter import convert_json_to_netlist, PinType
...
class TestKicadNetlistExporter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ORIGINAL1_NETLIST_PATH = cls.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist1.net"
        cls.ORIGINAL2_NETLIST_PATH = cls.BASE_DIR / "test_data" / "kicad9" / "netlists" / "netlist2.net"
    ...
    def test_export_netlist1(self):
        ...
        # Check net nodes
        self.assertIn("(node (ref \"R1\") (pin \"1\") (pintype \"passive\"))", netlist_content)
        self.assertIn("(node (ref \"C1\") (pin \"1\") (pintype \"passive\"))", netlist_content)
        self.assertIn("(node (ref \"R1\") (pin \"2\") (pintype \"passive\"))", netlist_content)
        self.assertIn("(node (ref \"R2\") (pin \"1\") (pintype \"passive\"))", netlist_content)
        self.assertIn("(node (ref \"C1\") (pin \"2\") (pintype \"passive\"))", netlist_content)
        self.assertIn("(node (ref \"R2\") (pin \"2\") (pintype \"passive\"))", netlist_content)
        ...
        print(f"JSON => netlist conversion output written to: {self.OUTPUT1_NETLIST_PATH.resolve()}")
    ...
    def test_export_netlist2(self):
        ...
        self.assertIn("(node (ref \"U1\") (pin \"1\") (pintype \"power_in\") (pinfunction \"GND\"))", netlist_content)
        self.assertIn("(node (ref \"U1\") (pin \"17\") (pintype \"bidirectional\") (pinfunction \"IO12\"))", netlist_content)
        
        # ---- Check unconnected pins ----
        # Verify unconnected pins are properly handled with correct pin types
        self.assertIn("(net (code \"5\") (name \"unconnected-(U1-EN-Pad8)\")", netlist_content)
        self.assertIn("(node (ref \"(U1\") (pin \"None\") (pintype \"passive\"))", netlist_content)
        
        print(f"JSON => netlist conversion output written to: {self.OUTPUT2_NETLIST_PATH.resolve()}")
    ...
    def test_round_trip_conversion(self):
        ...
        if not self.OUTPUT1_NETLIST_PATH.exists():
            self.skipTest("Exported netlist doesn't exist - run test_export_netlist1 first")
        ...
        for original_net_name in original_nets_raw:
            ...
            generated_net_name = original_net_name if original_net_name in generated_connections else normalized_name

            if original_net_name in original_connections and generated_net_name in generated_connections:
                 self.assertEqual(
                 ...
                 self.fail(f"Net '{original_net_name}' found in original connections but not in generated connections (checked as '{generated_net_name}')")
