...
class TestNetlist3Validation(NetlistValidationBase):
    ...
    def test_component_properties(self):
        ...
        self.assertGreaterEqual(len(self.json_data["subcircuits"]), 1, "Missing subcircuits in JSON")
        usb_subcircuit = self.find_subcircuit(self.json_data["subcircuits"], "usb")
        
        # Check P1 and R2 components exist in USB subcircuit
        self.assertIn("P1", usb_subcircuit["components"], "P1 component missing from USB subcircuit")
        self.assertIn("R2", usb_subcircuit["components"], "R2 component missing from USB subcircuit")
        ...
        self.assert_component_property(usb_subcircuit["components"], "P1", "reference", "P1")
        if p1_value:
            self.assert_component_property(usb_subcircuit["components"], "P1", "value", p1_value)
        if p1_description:
            self.assert_component_property(usb_subcircuit["components"], "P1", "description", p1_description)
        ...
        self.assert_component_property(usb_subcircuit["components"], "R2", "reference", "R2")
        if r2_value:
            self.assert_component_property(usb_subcircuit["components"], "R2", "value", r2_value)
        if r2_footprint:
            self.assert_component_property(usb_subcircuit["components"], "R2", "footprint", r2_footprint)
        if r2_description:
            self.assert_component_property(usb_subcircuit["components"], "R2", "description", r2_description)

    def test_net_count(self):
        """Test that all nets are correctly extracted."""
        # Extract nets from netlist with more flexible pattern
        netlist_nets = re.findall(r'\(net\s+\(code\s+"[^"]+"\)\s+\(name\s+"([^"]+)"\)', self.netlist_content)
        ...
        for net in important_nets:
            ...
            self.assertTrue(any(net in n for n in netlist_nets), f"Important net {net} missing from netlist")
    ...
    def test_hierarchical_nets(self):
        ...
        self.assertIn("D+", usb_subcircuit["nets"], "D+ net missing from USB subcircuit")
    ...
    def test_net_nodes(self):
        ...
        self.assertIsInstance(gnd_net, list, "GND net data should be a list (Structure A)")
        if gnd_net: # Check if the list is not empty
            ...
            self.assertGreater(len(components_connected_to_gnd), 0, "No components connected to GND in JSON")
        ...
        if "+3V3" in self.json_data["nets"]:
            ...
            self.assertIsInstance(v3v3_net_nodes, list, "+3V3 net data should be a list (Structure A)")
            if v3v3_net_nodes:
                u1_pin3v3_node = next((node for node in v3v3_net_nodes
                                     if node.get("component") == "U1" and node.get("pin", {}).get("number") == "3"), None)
                if u1_pin3v3_node:
                    self.assertEqual(u1_pin3v3_node.get("pin", {}).get("name"), "3V3", "U1 pin 3 name mismatch")
                    self.assertEqual(u1_pin3v3_node.get("pin", {}).get("type"), "power_in", "U1 pin 3 type mismatch")
    ...
    def test_pin_types(self):
        ...
        if "D+" in self.json_data["nets"]:
             ...
             if isinstance(dplus_net_nodes, list):
                u1_io20_node = next((node for node in dplus_net_nodes
                                   if node.get("component") == "U1" and node.get("pin", {}).get("number") == "26"), None)
                if u1_io20_node:
                    self.assertEqual(u1_io20_node.get("pin", {}).get("type"), "bidirectional", "U1 pin 26 type mismatch")
        
        # Input pin - look for any unconnected net with EN in the name
        unconnected_en_net_name = next((name for name in self.json_data["nets"] if "unconnected" in name and "EN" in name), None)
        if unconnected_en_net_name:
            en_net_nodes = self.json_data["nets"][unconnected_en_net_name]
            if isinstance(en_net_nodes, list):
                u1_en_node = next((node for node in en_net_nodes
                                 if node.get("component") == "U1" and node.get("pin", {}).get("number") == "8"), None)
                if u1_en_node:
                    self.assertEqual(u1_en_node.get("pin", {}).get("type"), "input", "U1 pin 8 type mismatch")
        
        # No connect pin - look for any unconnected net with NC in the name
        unconnected_nc_net_name = next((name for name in self.json_data["nets"] if "unconnected" in name and "NC" in name), None)
        if unconnected_nc_net_name:
            nc_net_nodes = self.json_data["nets"][unconnected_nc_net_name]
            if isinstance(nc_net_nodes, list):
                u1_nc_node = next((node for node in nc_net_nodes
                                 if node.get("component") == "U1" and "NC" in node.get("pin", {}).get("name", "")), None)
                if u1_nc_node:
                    # Note: no_connect in netlist is mapped to unspecified in JSON
                    self.assertEqual(u1_nc_node.get("pin", {}).get("type"), "unspecified", "U1 NC pin type mismatch")
    ...
    def test_subcircuit_structure(self):
        ...
        self.assertGreater(len(self.json_data.get("subcircuits", [])), 0, "No subcircuits found in JSON")
        
        # Check that the netlist has sheets with more flexible pattern
        sheet_count = len(re.findall(r'\(sheet\s+\(number\s+"[^"]+"\)', self.netlist_content))
        self.assertGreater(sheet_count, 1, "No subcircuit sheets found in netlist")  # At least root + one subcircuit
        
        # Check that at least one subcircuit from JSON is represented in the netlist
        json_subcircuits = [subcircuit["name"] for subcircuit in self.json_data.get("subcircuits", [])]
        self.assertTrue(any(re.search(rf'\(sheet.*?\(name\s+"[^"]*{re.escape(name)}[^"]*"\)', self.netlist_content, re.DOTALL)
        ...
        local_nets = [net for net in usb_subcircuit["nets"] if "Net-" in net or "unconnected-" in net]
    ...
    def test_component_sheetpath(self):
        ...
        self.assertIn("U1", self.json_data["components"], "U1 component missing from JSON")
        ...
        if "Sheetname" in u1["properties"]:
            self.assertEqual(u1["properties"]["Sheetname"], "Root", "U1 Sheetname property mismatch")
        ...
        if "Sheetname" in p1["properties"]:
            self.assertEqual(p1["properties"]["Sheetname"], "usb", "P1 Sheetname property mismatch")
    ...
    def test_net_codes(self):
        ...
        netlist_net_codes = {}
        for match in re.finditer(r'\(net\s+\(code\s+"([^"]+)"\)\s+\(name\s+"([^"]+)"\)', self.netlist_content):
