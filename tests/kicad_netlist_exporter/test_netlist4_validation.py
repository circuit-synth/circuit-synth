...
class TestNetlist4Validation(NetlistValidationBase):
    ...
    def test_file_existence(self):
        ...
        self.assertTrue(self.NETLIST_FILE.exists(), f"Netlist file not found: {self.NETLIST_FILE}")
        self.assertTrue(self.JSON_FILE.exists(), f"JSON file not found: {self.JSON_FILE}")
    ...
    def test_basic_properties(self):
        ...
        if source_match:
            self.assertEqual(self.json_data["properties"]["source"], source_match.group(1), "Source property mismatch")
        if date_match:
            self.assertEqual(self.json_data["properties"]["date"], date_match.group(1), "Date property mismatch")
        if tool_match:
            self.assertEqual(self.json_data["properties"]["tool"], tool_match.group(1), "Tool property mismatch")
    ...
    def test_hierarchical_structure(self):
        ...
        netlist_sheets = re.findall(r'\(sheet\s+\(number\s+"[^"]+"\)\s+\(name\s+"([^"]+)"\)', self.netlist_content)
        ...
        for subcircuit in self.json_data.get("subcircuits", []):
            ...
            for sub_subcircuit in subcircuit.get("subcircuits", []):
                ...
                for sub_sub_subcircuit in sub_subcircuit.get("subcircuits", []):
                    json_sheets.append(f"/{subcircuit['name']}/{sub_subcircuit['name']}/{sub_sub_subcircuit['name']}/")
                    
                    # Add even deeper nested subcircuits
                    for sub_sub_sub_subcircuit in sub_sub_subcircuit.get("subcircuits", []):
                        json_sheets.append(f"/{subcircuit['name']}/{sub_subcircuit['name']}/{sub_sub_subcircuit['name']}/{sub_sub_sub_subcircuit['name']}/")
        ...
        self.assertGreaterEqual(len(json_sheets), min_expected_sheets, 
                             f"Sheet count too low: {len(json_sheets)} in JSON vs at least {min_expected_sheets} expected")
        self.assertLessEqual(len(json_sheets), max_expected_sheets, 
        ...
        self.assertIn("/usb/regulator/", json_sheets, "Missing /usb/regulator/ sheet in JSON")
        self.assertIn("/usb/regulator/led/", json_sheets, "Missing /usb/regulator/led/ sheet in JSON")
        self.assertIn("/usb/regulator/led/light_sensor/", json_sheets, "Missing /usb/regulator/led/light_sensor/ sheet in JSON")

    def test_component_count(self):
        """Test that all components are correctly extracted."""
        # Extract components from netlist with more flexible pattern
        netlist_components = re.findall(r'\(comp\s+\(ref\s+"([^"]+)"\)', self.netlist_content)
        ...
        min_expected_components = max(1, len(netlist_components) - 2)  # At least 1, or netlist count - 2
        max_expected_components = len(netlist_components) + 2  # Allow for up to 2 extra components
        
        self.assertGreaterEqual(len(json_components), min_expected_components, 
                             f"Component count too low: {len(json_components)} in JSON vs at least {min_expected_components} expected")
        self.assertLessEqual(len(json_components), max_expected_components, 
        ...
        for comp_ref in key_components:
            self.assertIn(comp_ref, json_components, f"Key component {comp_ref} missing from JSON")
    ...
    def test_component_properties(self):
        ...
        self.assert_component_property(self.json_data["components"], "U1", "reference", "U1")
        
        # Extract U1 value from netlist using safe method
        u1_value = self.safe_extract_property("U1", "value")
        if u1_value:
            self.assert_component_property(self.json_data["components"], "U1", "value", u1_value)
        ...
        self.assert_component_property(self.json_data["components"], "U1", "description")
        
        # Test C1 component properties
        self.assert_component_property(self.json_data["components"], "C1", "reference", "C1")
        ...
        if c1_value:
            self.assert_component_property(self.json_data["components"], "C1", "value", c1_value)
        if c1_footprint:
            self.assert_component_property(self.json_data["components"], "C1", "footprint", c1_footprint)
        if c1_description:
            self.assert_component_property(self.json_data["components"], "C1", "description", c1_description)
    ...
    def test_deep_hierarchical_components(self):
        ...
        usb_subcircuit = self.find_subcircuit(self.json_data.get("subcircuits", []), "usb")
        regulator_subcircuit = self.find_subcircuit(usb_subcircuit.get("subcircuits", []), "regulator")
        led_subcircuit = self.find_subcircuit(regulator_subcircuit.get("subcircuits", []), "led")
        light_sensor_subcircuit = self.find_subcircuit(led_subcircuit.get("subcircuits", []), "light_sensor")
        
        # Check U3 component in light_sensor subcircuit
        self.assert_component_property(light_sensor_subcircuit["components"], "U3", "reference", "U3")
        ...
        if u3_value:
            self.assert_component_property(light_sensor_subcircuit["components"], "U3", "value", u3_value)
        if u3_footprint:
            self.assert_component_property(light_sensor_subcircuit["components"], "U3", "footprint", u3_footprint)
        if u3_description:
            self.assert_component_property(light_sensor_subcircuit["components"], "U3", "description", u3_description)
        
        # Check sheetpath property
        self.assertIn("properties", light_sensor_subcircuit["components"]["U3"], "U3 properties missing from JSON")
        self.assertIn("Sheetname", light_sensor_subcircuit["components"]["U3"]["properties"], "U3 Sheetname property missing from JSON")
        self.assertEqual(light_sensor_subcircuit["components"]["U3"]["properties"]["Sheetname"], "light_sensor", "U3 Sheetname property mismatch")

    def test_hierarchical_nets(self):
        """Test that hierarchical nets are correctly represented."""
        # Check SDA net in main circuit
        self.assertIn("SDA", self.json_data["nets"], "SDA net missing from main circuit")
        ...
        self.assertIn("SDA", light_sensor_subcircuit["nets"], "SDA net missing from light_sensor subcircuit")
        ...
        for net_name in ["SCL", "INT"]:
            self.assertIn(net_name, self.json_data["nets"], f"{net_name} net missing from main circuit")
            self.assertIn(net_name, light_sensor_subcircuit["nets"], f"{net_name} net missing from light_sensor subcircuit")
            # Optional: Use check_net_nodes for detailed verification if needed.
    ...
    def test_global_nets(self):
        ...
        self.assertIn("GND", self.json_data["nets"], "GND net missing from main circuit")
        ...
        self.assertIn("+3V3", self.json_data["nets"], "+3V3 net missing from main circuit")
        # In Structure A, global nets just appear in multiple scopes; no 'is_hierarchical' flag.
        # Optional: Use check_net_nodes for detailed verification if needed.
    ...
    def test_local_nets(self):
        ...
        for net_name in local_nets:
            ...
            self.assertNotIn(net_name, self.json_data["nets"], 
    ...
    def test_net_nodes(self):
        ...
        if "+3V3" in self.json_data["nets"]:
            ...
            if plus3v3_net: # Check if the net exists and has nodes
                ...
                u1_pin3_node = next((node for node in plus3v3_net
                                   if node["component"] == "U1" and node["pin"]["number"] == "3"), None)
                if u1_pin3_node:
                    self.assertEqual(u1_pin3_node["pin"]["name"], "3V3", "U1 pin 3 name mismatch")
                    self.assertEqual(u1_pin3_node["pin"]["type"], "power_in", "U1 pin 3 type mismatch")

                # Verify C1 pin 1 is connected to +3V3 if it exists
                c1_pin1_node = next((node for node in plus3v3_net
                                   if node["component"] == "C1" and node["pin"]["number"] == "1"), None)
                if c1_pin1_node:
                    self.assertIsNotNone(c1_pin1_node, "C1 pin 1 node missing from +3V3 net")
        ...
        if "+3V3" in light_sensor_subcircuit["nets"]:
            ...
            if light_sensor_plus3v3_net: # Check if the net exists and has nodes
                ...
                u3_pin1_node = next((node for node in light_sensor_plus3v3_net
                                   if node["component"] == "U3" and node["pin"]["number"] == "1"), None)
                if u3_pin1_node:
                    self.assertEqual(u3_pin1_node["pin"]["name"], "VDD", "U3 pin 1 name mismatch")
                    self.assertEqual(u3_pin1_node["pin"]["type"], "power_in", "U3 pin 1 type mismatch")
            # Optional: Use check_net_nodes for more comprehensive checks
    ...
    def test_pin_types(self):
        ...
        if "+3V3" in self.json_data["nets"]:
            ...
            u1_pin3v3_node = next((node for node in plus3v3_nodes
                                 if node["component"] == "U1" and node["pin"]["number"] == "3"), None)
            if u1_pin3v3_node:
                self.assertEqual(u1_pin3v3_node["pin"]["type"], "power_in", "U1 pin 3 type mismatch")
        
        # Bidirectional pin
        if "D+" in self.json_data["nets"]:
            dplus_nodes = self.json_data["nets"]["D+"] # Structure A
            u1_io20_node = next((node for node in dplus_nodes
                               if node["component"] == "U1" and node["pin"]["number"] == "26"), None)
            if u1_io20_node:
                self.assertEqual(u1_io20_node["pin"]["type"], "bidirectional", "U1 pin 26 type mismatch")
        
        # Input pin
        unconnected_en_net_name = next((net for net in self.json_data["nets"] if "unconnected" in net and "EN" in net), None)
        if unconnected_en_net_name:
            unconnected_en_nodes = self.json_data["nets"][unconnected_en_net_name] # Structure A
            u1_en_node = next((node for node in unconnected_en_nodes
                             if node["component"] == "U1" and node["pin"]["number"] == "8"), None)
            if u1_en_node:
                self.assertEqual(u1_en_node["pin"]["type"], "input", "U1 pin 8 type mismatch")
        
        # No connect pin (in netlist) / unspecified pin (in JSON)
        unconnected_nc_net_name = next((net for net in self.json_data["nets"] if "unconnected" in net and "NC" in net), None)
        if unconnected_nc_net_name:
            unconnected_nc_nodes = self.json_data["nets"][unconnected_nc_net_name] # Structure A
            u1_nc_node = next((node for node in unconnected_nc_nodes
                             if node["component"] == "U1" and "NC" in node["pin"].get("name", "")), None)
            if u1_nc_node:
                # Note: no_connect in netlist is mapped to unspecified in JSON
                self.assertEqual(u1_nc_node["pin"]["type"], "unspecified", "U1 NC pin type mismatch")
        ...
        if "+3V3" in light_sensor_subcircuit["nets"]:
            ...
            u3_vdd_node = next((node for node in ls_plus3v3_nodes
                              if node["component"] == "U3" and node["pin"]["number"] == "1"), None)
            if u3_vdd_node:
                self.assertEqual(u3_vdd_node["pin"]["type"], "power_in", "U3 pin 1 type mismatch")
        
        if "SCL" in light_sensor_subcircuit["nets"]:
            ls_scl_nodes = light_sensor_subcircuit["nets"]["SCL"] # Structure A
            u3_scl_node = next((node for node in ls_scl_nodes
                              if node["component"] == "U3" and node["pin"]["number"] == "4"), None)
            if u3_scl_node:
                self.assertEqual(u3_scl_node["pin"]["type"], "input", "U3 pin 4 type mismatch")
        
        if "SDA" in light_sensor_subcircuit["nets"]:
            ls_sda_nodes = light_sensor_subcircuit["nets"]["SDA"] # Structure A
            u3_sda_node = next((node for node in ls_sda_nodes
                              if node["component"] == "U3" and node["pin"]["number"] == "5"), None)
            if u3_sda_node:
                self.assertEqual(u3_sda_node["pin"]["type"], "bidirectional", "U3 pin 5 type mismatch")
