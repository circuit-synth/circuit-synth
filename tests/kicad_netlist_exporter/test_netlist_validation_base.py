...
from typing import List, Dict, Optional, Any # Import necessary types including Any
...
class NetlistValidationBase(unittest.TestCase):
    ...
    def safe_extract_property(self, component_ref, property_name):
        ...
        """
        # Try direct pattern
        pattern = rf'\(comp\s+\(ref\s+"{component_ref}"\).*?\({property_name}\s+"([^"]+)"\)'
    ...
    def assert_component_property(self, components, ref, property_name, expected_value=None):
        """
        ...
        if expected_value is not None:
            ...
            if isinstance(expected_value, str) and expected_value.startswith("extract:"):
                property_to_extract = expected_value.split(":", 1)[1]
                extracted_value = self.safe_extract_property(ref, property_to_extract)
                if extracted_value:
                    self.assertEqual(components[ref][property_name], extracted_value, 
                                    f"{ref} {property_name} mismatch")
            else:
                self.assertEqual(components[ref][property_name], expected_value, 
                                f"{ref} {property_name} mismatch")
        else:
            # Just check property exists
            self.assertIn(property_name, components[ref], 
                         f"{ref} {property_name} missing")
    ...
    def assert_net_count_range(self, nets, min_count, max_count=None, message=None):
        ...
        count = len(nets)
        if message is None:
            message = f"Net count {count} outside acceptable range [{min_count}-{max_count}]"
    ...
    def check_net_nodes(self, node_list: List[Dict[str, Any]], component_ref: str, pin_number: Optional[str] = None):
        """
        ...
        logger.debug(f"check_net_nodes: Checking for Comp='{component_ref}', Pin='{pin_number}' in node_list (len={len(nodes)}): {nodes[:3]}...") # Log first 3 nodes
        if pin_number is None:
            # Just check component exists in any node
            found = any(node.get("component") == component_ref for node in nodes)
            self.assertTrue(found, f"Component {component_ref} missing from net nodes: {nodes}")
        else:
            # Check specific pin
            found = any(node.get("component") == component_ref and
                        node.get("pin", {}).get("number") == str(pin_number) # Ensure pin_number is compared as string
                        for node in nodes)
            self.assertTrue(found, f"Component {component_ref} pin {pin_number} missing from net nodes: {nodes}")