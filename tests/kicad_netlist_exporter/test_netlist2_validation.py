...
class TestNetlist2Validation(NetlistValidationBase):
    ...
    def test_component_count(self):
        ...
        self.assertEqual(len(json_components), len(netlist_components), 
        ...
        for comp_ref in netlist_components:
            self.assertIn(comp_ref, json_components, f"Component {comp_ref} missing from JSON")
    ...
    def test_component_properties(self):
        ...
        self.assert_component_property(self.json_data["components"], "D1", "reference", "D1")
        self.assert_component_property(self.json_data["components"], "D1", "value", "LED")
        self.assert_component_property(self.json_data["components"], "D1", "footprint", "LED_SMD:LED_0603_1608Metric")
        
        # Test R1 component properties
        self.assert_component_property(self.json_data["components"], "R1", "reference", "R1")
        self.assert_component_property(self.json_data["components"], "R1", "value", "10k")
        self.assert_component_property(self.json_data["components"], "R1", "footprint", "Resistor_SMD:R_0603_1608Metric")
        
        # Test U1 component properties
        self.assert_component_property(self.json_data["components"], "U1", "reference", "U1")
        self.assert_component_property(self.json_data["components"], "U1", "value", "ESP32-C6-MINI-1")
        self.assert_component_property(self.json_data["components"], "U1", "footprint", "RF_Module:ESP32-C6-MINI-1")
    ...
    def test_net_count(self):
        ...
        self.assertEqual(len(json_nets), len(netlist_nets), 
    ...
    def test_net_nodes(self):
        ...
        self.assertIsInstance(v3v3_net, list, "+3V3 net data should be a list (Structure A)")
        ...
        self.assertIn("Net-(D1-A)", self.json_data["nets"], "Net-(D1-A) net missing from JSON")
        d1a_net = self.json_data["nets"]["Net-(D1-A)"]
        
        # Check that Net-(D1-A) net has nodes (d1a_net is the list of nodes in Structure A)
        self.assertIsInstance(d1a_net, list, "Net-(D1-A) net data should be a list (Structure A)")
    ...
    def test_unconnected_pins(self):
        ...
        unconnected_nets = [net_name for net_name in self.json_data["nets"] if "unconnected-" in net_name]
        self.assertGreater(len(unconnected_nets), 0, "No unconnected nets found in JSON")
        ...
        for net_name in unconnected_nets:
            ...
            self.assertIsInstance(net, list, f"{net_name} net data should be a list (Structure A)")
    ...
    def test_round_trip_conversion(self):
        ...
        self.assertEqual(set(original_refs), set(exported_refs),
                        "Component references don't match between original and exported netlists")
        
        # Extract net names from both netlists
        original_nets = re.findall(r'\(net \(code "[^"]+"\) \(name "([^"]+)"\)', self.netlist_content)
        exported_nets = re.findall(r'\(net \(code "[^"]+"\) \(name "([^"]+)"\)', exported_content)
        
        # Compare net names
        self.assertEqual(set(original_nets), set(exported_nets),
        ...
        net_blocks_original = re.findall(r'\(net \(code "[^"]+"\) \(name "([^"]+)"\)(.*?)\)', self.netlist_content, re.DOTALL)
        for net_name, net_content in net_blocks_original:
            # Find all node references in this net
            nodes = re.findall(r'\(node \(ref "([^"]+)"\) \(pin "([^"]+)"\)', net_content)
            original_connections[net_name] = [f"{ref}:{pin}" for ref, pin in nodes]
        
        # Find all net blocks in exported netlist
        net_blocks_exported = re.findall(r'\(net \(code "[^"]+"\) \(name "([^"]+)"\)(.*?)\)', exported_content, re.DOTALL)
        ...
        for net_name in original_nets:
            if net_name in original_connections and net_name in exported_connections:
                self.assertEqual(set(original_connections[net_name]),
