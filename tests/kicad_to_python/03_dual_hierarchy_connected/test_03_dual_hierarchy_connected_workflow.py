...
class TestConnectedHierarchicalWorkflow:
    ...
    def test_connected_hierarchical_parsing(self):
        ...
        if child1_file.exists():
            ...
            assert "@circuit(name='child1')" in child1_content, "child1 circuit not found in separate file"
        ...
        assert "child1_circuit = child1(" in python_content, "child1 instantiation not found"
    ...
    def test_connected_hierarchical_execution(self):
        ...
        assert proc_result.returncode == 0, f"Python execution failed: {proc_result.stderr}"
        ...
        # Verify project file
        project_file = generated_project / "03_dual_hierarchy_connected_generated.kicad_pro"
    ...
    def test_connected_net_preservation(self):
        ...
        subprocess.run(["uv", "run", "python", str(python_file)], cwd=str(output_dir), check=True)
        
        # Verify connections in generated schematics
        generated_project = output_dir / "03_dual_hierarchy_connected_generated"
        
        # Check main schematic for hierarchical labels and specific connections
        main_sch_content = (generated_project / "03_dual_hierarchy_connected_generated.kicad_sch").read_text()
        assert "hierarchical_label" in main_sch_content, "Hierarchical labels missing from main schematic"
        
        # Verify specific VIN/GND labels in main circuit (clean notation without slash)
        assert 'VIN' in main_sch_content, "VIN hierarchical label missing from main schematic"
        assert 'GND' in main_sch_content, "GND hierarchical label missing from main schematic"
        ...
        assert "hierarchical_label" in child_sch_content, "Hierarchical labels missing from child schematic"
        
        # Verify specific VIN/GND labels in child circuit (clean notation without slash)
        assert 'VIN' in child_sch_content, "VIN hierarchical label missing from child schematic"
        assert 'GND' in child_sch_content, "GND hierarchical label missing from child schematic"
    ...
    def test_connected_round_trip(self):
        ...
        subprocess.run(["uv", "run", "python", str(python_file_1)], cwd=str(output_dir), check=True)
        ...
        assert "@circuit(name='main')" in python_content_2, "main circuit lost in round-trip"
        assert "from child1 import child1" in python_content_2, "child1 import lost in round-trip"
        ...
        if child1_file_2.exists():
            ...
            assert "@circuit(name='child1')" in child1_content_2, "child1 circuit lost in round-trip"
    ...
    def test_hierarchical_reference_uniqueness(self):
        ...
        main_sch = (generated_project / "03_dual_hierarchy_connected_generated.kicad_sch").read_text()
        child_sch = (generated_project / "child1.kicad_sch").read_text()
        
        # Should have R3 in main and R2 in child
        assert '"R3"' in main_sch or '(reference R3)' in main_sch, "R3 reference missing from main schematic"
        assert '"R2"' in child_sch or '(reference R2)' in child_sch, "R2 reference missing from child schematic"
