...
class TestESP32C6HierarchicalWorkflow(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.reference_generated = cls.test_data_dir / "ESP32_C6_Dev_Board_generated_reference"
    ...
    def test_hierarchical_project_detection(self):
        ...
        # Run the conversion
        success = syncer.sync()
        self.assertTrue(success, "ESP32-C6 hierarchical KiCad-to-Python conversion should succeed")
        ...
        self.assertGreater(len(python_files), 1, 
    ...
    def test_hierarchical_code_structure(self):
        ...
        self.assertIn("import", main_content, "Should contain import statements for subcircuits")
        ...
        self.assertIn("def ", main_content, "Should contain circuit function definitions")
    ...
    def test_import_statement_generation(self):
        ...
        if main_py.exists():
            ...
            self.assertNotIn("import USB_Port", main_content, 
                            "Should not have uppercase function imports (old bug)")
            self.assertNotIn("import Power_Supply", main_content,
                            "Should not have uppercase function imports (old bug)")
    ...
    def test_hierarchical_round_trip_execution(self):
        ...
        generated_dirs = [d for d in self.python_output_dir.iterdir() 
                         if d.is_dir() and d.name.endswith("_generated")]
        
        self.assertGreater(len(generated_dirs), 0, 
        ...
        for ext in expected_extensions:
            ...
            self.assertGreater(len(matching_files), 0, 
                              f"Should generate at least one {ext} file")
    ...
    def test_net_connectivity_preservation(self):
        ...
        try:
            os.chdir(self.python_output_dir)
            result = subprocess.run(["uv", "run", "main.py"], 
        ...
        if netlist_files:
            ...
            expected_nets = ["GND", "VCC_3V3"]  # Basic nets that should always be present
            
            for net_name in expected_nets:
                self.assertTrue(
                    f'"{net_name}"' in netlist_content or f"'{net_name}'" in netlist_content,
    ...
    def test_full_hierarchical_round_trip(self):
        ...
        try:
            ...
            result1 = subprocess.run(["uv", "run", "main.py"], 
                                   capture_output=True, text=True, timeout=120)
            self.assertEqual(result1.returncode, 0, f"First execution failed: {result1.stderr}")
        except subprocess.TimeoutExpired:
        ...
        generated_dirs = [d for d in first_python.iterdir() 
                         if d.is_dir() and d.name.endswith("_generated")]
        
        self.assertGreater(len(generated_dirs), 0, "Should generate KiCad output directory")
        
        generated_kicad_dir = generated_dirs[0]
        generated_project_files = list(generated_kicad_dir.glob("*.kicad_pro"))
        
        self.assertGreater(len(generated_project_files), 0, "Should generate .kicad_pro file")
        ...
        try:
            ...
            result2 = subprocess.run(["uv", "run", "main.py"], 
                                   capture_output=True, text=True, timeout=120)
            self.assertEqual(result2.returncode, 0, f"Second execution failed: {result2.stderr}")
        except subprocess.TimeoutExpired:
        ...
        for python_dir in [first_python, second_python]:
            ...
            self.assertGreater(len(python_files), 0, 
                              f"Should generate Python files in {python_dir}")
    ...
    def test_comparative_output_validation(self):
        ...
        self.assertGreater(len(generated_files), 0, "Should generate at least one Python file")
