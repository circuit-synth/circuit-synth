...
class TestSymbolLibCache:
    ...
    def mock_kicad_env(self, monkeypatch, temp_cache_dir):
        ...
        device_lib.write_text('(kicad_symbol_lib (version 20211014) (generator kicad_symbol_editor))')
        
        power_lib = fake_kicad_dir / "power.kicad_sym"
        power_lib.write_text('(kicad_symbol_lib (version 20211014) (generator kicad_symbol_editor))')
    ...
    def test_find_symbol_library(self, mock_kicad_env):
        ...
        SymbolLibCache._symbol_index = {
            "R": {"lib_name": "Device", "lib_path": mock_kicad_env / "Device.kicad_sym"},
            "C": {"lib_name": "Device", "lib_path": mock_kicad_env / "Device.kicad_sym"},
            "VCC": {"lib_name": "power", "lib_path": mock_kicad_env / "power.kicad_sym"},
            "GND": {"lib_name": "power", "lib_path": mock_kicad_env / "power.kicad_sym"}
        }
        SymbolLibCache._index_built = True
    ...
    def test_cache_expiration(self, mock_parse, mock_kicad_env):
        ...
        assert SymbolLibCache._is_cache_expired(current_time - 3600, 2) is False  # 1 hour old, 2 hour TTL
        assert SymbolLibCache._is_cache_expired(current_time - 7200, 1) is True   # 2 hours old, 1 hour TTL
