...
class TestPythonCodeGeneratorEdgeCases:
    ...
    def test_single_circuit_file_generation(self, generator, temp_output_dir):
        ...
        circuit = Circuit(
            name="main",
            components=[Component(reference="R1", lib_id="Device:R", value="10k")],
            nets=[Net(name="VCC", connections=[("R1", "1")]), Net(name="GND", connections=[("R1", "2")])]
        )
    ...
    def test_dual_hierarchy_no_shared_nets(self, generator, temp_output_dir):
        ...
        main_circuit = Circuit(
            ...
            nets=[Net(name="VCC_MAIN", connections=[]), Net(name="GND_MAIN", connections=[])]
        )
        
        child_circuit = Circuit(
            name="child1",
            components=[Component(reference="R2", lib_id="Device:R", value="2k")],
            nets=[Net(name="VCC_CHILD", connections=[]), Net(name="GND_CHILD", connections=[])]
        )
    ...
    def test_dual_hierarchy_with_shared_nets(self, generator, temp_output_dir):
        ...
        main_circuit = Circuit(
            name="main",
            components=[Component(reference="R1", lib_id="Device:R", value="1k")],
            nets=[Net(name="VCC", connections=[]), Net(name="GND", connections=[]), Net(name="UNIQUE_MAIN", connections=[])]
        )
        
        child_circuit = Circuit(
            name="child1",
            components=[Component(reference="R2", lib_id="Device:R", value="2k")],
            nets=[Net(name="VCC", connections=[]), Net(name="GND", connections=[]), Net(name="UNIQUE_CHILD", connections=[])]
        )
        ...
        instantiation_line = [line for line in main_content.split('\n') if 'child1_instance = child1(' in line][0]
        assert "vcc" in instantiation_line and "gnd" in instantiation_line
        
        # Check child file
        child_content = (temp_output_dir / "child1.py").read_text()
        assert "def child1(" in child_content
        function_line = [line for line in child_content.split('\n') if 'def child1(' in line][0]
    ...
    def test_multiple_subcircuits_different_shared_nets(self, generator, temp_output_dir):
        ...
        child1_circuit = Circuit(
            name="child1",
            components=[],
            nets=[Net(name="VCC", connections=[]), Net(name="GND", connections=[])]  # Shares VCC, GND
        )
        
        child2_circuit = Circuit(
            name="child2",
            components=[],
            nets=[Net(name="SIGNAL", connections=[]), Net(name="VCC", connections=[])]  # Shares SIGNAL, VCC
        )
        
        circuits = {"main": main_circuit, "child1": child1_circuit, "child2": child2_circuit}
        main_file = temp_output_dir / "main.py"
    ...
    def test_hierarchical_detection_logic(self, generator):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result1 = generator.update_python_file(temp_file, single_circuits, preview_only=True)
            assert "from child1 import child1" not in result1
            
            # Multiple circuits should generate hierarchical code
            result2 = generator.update_python_file(temp_file, multi_circuits, preview_only=True)
    ...
    def test_component_reference_sanitization(self, generator):
        ...
        circuit = Circuit(
            name="main",
            components=[
                Component(reference="R+1", lib_id="Device:R", value="10k"),  # Invalid reference
                Component(reference="123C", lib_id="Device:C", value="1uF"),  # Starts with number
            ],
            nets=[]
        )
    ...
    def test_unconnected_nets_filtering(self, generator):
        ...
        circuit = Circuit(
            ...
            nets=[
                Net(name="VCC", connections=[("R1", "1")]),
                Net(name="unconnected-(R1-Pad2)", connections=[("R1", "2")]),  # Should be filtered
            ]
        )
