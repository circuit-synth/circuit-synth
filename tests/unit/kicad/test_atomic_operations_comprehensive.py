...
class TestAtomicOperationsComprehensive:
    ...
    def copy_reference_schematic(self, test_data_dir: Path, ref_name: str, temp_dir: Path) -> Path:
    ...
    def analyze_schematic(self, schematic_path: Path) -> dict:
        ...
        if lib_symbols_start != -1 and lib_symbols_end != -1:
            non_lib_content = content[:lib_symbols_start] + content[lib_symbols_end+1:]
    ...
    def test_blank_to_single_resistor_progression(self, test_data_dir, temp_dir):
        ...
        test_schematic = self.copy_reference_schematic(test_data_dir, 'blank_schematic', temp_dir)
        ...
        assert after_add_analysis['symbols'] > initial_analysis['symbols'], "No symbols added"
        assert after_add_analysis['has_lib_symbols_section'], "Missing lib_symbols section"
        ...
        assert after_add_analysis['has_lib_symbols_section'] == ref_analysis['has_lib_symbols_section']
    ...
    def test_single_to_two_resistor_progression(self, test_data_dir, temp_dir):
        ...
        test_schematic = self.copy_reference_schematic(test_data_dir, 'single_resistor', temp_dir)
        ...
        assert len(after_add_analysis['resistor_refs']) == 2, f"Expected 2 resistors, got {len(after_add_analysis['resistor_refs'])}"
    ...
    def test_two_to_single_resistor_removal(self, test_data_dir, temp_dir):
        ...
        test_schematic = self.copy_reference_schematic(test_data_dir, 'two_resistors', temp_dir)
        ...
        assert len(after_remove_analysis['resistor_refs']) == 1, f"Expected 1 resistor, got {len(after_remove_analysis['resistor_refs'])}"
        assert 'R1' in after_remove_analysis['resistor_refs'], "R1 should remain"
        assert 'R2' not in after_remove_analysis['resistor_refs'], "R2 should be removed"
        
        # Verify fewer symbols than initial
        assert after_remove_analysis['symbols'] < initial_analysis['symbols'], "No symbols removed"
    ...
    def test_single_to_blank_removal(self, test_data_dir, temp_dir):
        ...
        assert len(after_remove_analysis['resistor_refs']) == 0, "All resistors should be removed"
        ...
        logger.info(f"After removing all components: has_symbol_instances={after_remove_analysis['has_symbol_instances']}")
        # This is acceptable - the key thing is no component references remain
    
    def test_complete_cycle_blank_single_two_single_blank(self, test_data_dir, temp_dir):
        ...
        success = remove_component_from_schematic_exact(file_path=test_schematic, reference="R2")
        assert success, "Failed to remove R2"
        ...
        success = remove_component_from_schematic_exact(file_path=test_schematic, reference="R1")
        assert success, "Failed to remove R1"
    ...
    def test_atomic_safety_invalid_operations(self, test_data_dir, temp_dir):
        ...
        success = remove_component_from_schematic_exact(file_path=test_schematic, reference="R999")
        ...
        assert after_failed_remove == initial_analysis, "File should be unchanged after failed operation"
    ...
    def test_mixed_component_types(self, test_data_dir, temp_dir):
        ...
        success4 = remove_component_from_schematic_exact(file_path=test_schematic, reference="C1")
        assert success4, "Failed to remove capacitor"
        
        # Verify capacitor removed but others remain
        final_analysis = self.analyze_schematic(test_schematic)
        assert final_analysis['symbols'] < analysis['symbols'], "Symbols should decrease after removal"
