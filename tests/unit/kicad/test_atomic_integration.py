  ...
  "schematic": {
    ...
    def test_add_sheet_reference(self, sample_kicad_project):
        ...
        success = atomic.add_sheet_reference(
            "test_project", "Power_Supply", "Power_Supply.kicad_sch", (35, 35), (44, 20)
        )
    ...
    def test_migrate_circuit_to_atomic_basic(self, sample_json_netlist, temp_dir):
        ...
        try:
            success = migrate_circuit_to_atomic(
                str(sample_json_netlist), str(output_dir)
            )
            ...
            if success:
                ...
                logger.warning(
                    "migrate_circuit_to_atomic returned False - feature may not be fully implemented yet"
                )

        except Exception as e:
            logger.warning(
                f"migrate_circuit_to_atomic failed: {e} - feature may not be fully implemented yet"
            )
    ...
    def test_atomic_operations_error_handling(self, sample_kicad_project):
        ...
        success = atomic.add_component_atomic(
            "nonexistent_schematic", {"symbol": "Device:R", "ref": "R1", "value": "10k"}
        )
    ...
    def test_atomic_operations_with_backup_restore(self, sample_kicad_project):
        ...
        success1 = atomic.add_component_atomic(
            "test_project",
            {"symbol": "Device:R", "ref": "R1", "value": "10k", "position": (100, 80)},
        )
        ...
        atomic.add_component_atomic(
            "test_project",
            {"symbol": "Invalid:Component", "ref": "U999", "value": "invalid"},
        )
...
class TestReferenceCircuitCompatibility:
    ...
    def test_reference_circuits_analyzable(self, test_data_dir):
        ...
        for ref_name in reference_names:
            ...
            assert (
                "(kicad_sch" in content
            ), f"Reference {ref_name} should be valid KiCad schematic"

            logger.info(
                f"Reference {ref_name}: {len(content)} chars, {content.count('(symbol')} symbols"
            )
