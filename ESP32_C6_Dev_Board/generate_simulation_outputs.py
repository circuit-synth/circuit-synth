...
def generate_usb_differential_outputs(output_dir):
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Time_ns", "USB_DP", "USB_DM", "USB_DP_MCU", "USB_DM_MCU", "Differential"])
        for i in range(0, len(time_ns), 10):  # Subsample for reasonable file size
            diff = usb_dp[i] - usb_dm[i]
            writer.writerow([
                time_ns[i], usb_dp[i], usb_dm[i],
                usb_dp_mcu[i], usb_dm_mcu[i], diff
            ])
    print(f"  ✓ Generated: {csv_path.name}")
...
def generate_led_driver_outputs(output_dir):
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Time_us", "LED_Control_V", "LED_Cathode_V", "LED_Current_mA", "Brightness_%"])
        for i in range(0, len(time_us), 10):
            brightness = (led_current[i] / 1.3) * 100  # Percentage of max current
            writer.writerow([
                time_us[i], led_control[i], led_cathode[i],
                led_current[i], brightness
            ])
    print(f"  ✓ Generated: {csv_path.name}")
