...
sys.path.insert(
    0,
    "/Users/shanemattner/Desktop/circuit-synth4/tests/kicad_to_python/04_esp32_c6_hierarchical/ESP32_C6_Dev_Board_python_reference",
)
...
def create_power_supply_test():
    ...
    tp_vbus = Component(
        ...
        ref="TP1",
        value="VBUS",
        footprint="Connector:TestPoint_Pad_D1.0mm",
    )
    tp_vbus[1] += vbus
...
def simulate_power_supply(circuit, output_dir):
    ...
    try:
        ...
        dc_result = sim.dc_analysis(source="V1", start=4.5, stop=5.5, step=0.01)
        ...
        ac_result = sim.ac_analysis(start_freq=10, stop_freq=100000, points=100)
        ...
        viz.plot_bode(
            "VBUS", "VCC_3V3", save_path=str(output_dir / "psrr_bode.png"), show=False
        )
        print(f"    ✓ Generated PSRR Bode plot")

        # 3. Transient Analysis - Load step response
        print("  • Running transient analysis (load step)...")
        transient_result = sim.transient_analysis(step_time=1e-6, end_time=10e-3)
        ...
        html_path = ac_result.plot_interactive(
            ["VBUS", "VCC_3V3"], output_path=str(output_dir / "interactive_power.html")
        )
        print(f"    ✓ Created interactive HTML visualization")

        # Export SPICE netlist
        viz.export_spice_netlist(
            str(output_dir / "power_supply.cir"), include_models=True
        )
...
def simulate_usb_signals(circuit, output_dir):
    ...
    try:
        ...
        ac_result = sim.ac_analysis(
            start_freq=1e6, stop_freq=1e9, points=100  # 1MHz  # 1GHz
        )
        ...
        transient_result.plot_interactive(
            ["USB_DP", "USB_DM"], output_path=str(output_dir / "interactive_usb.html")
        )
...
def simulate_led_driver(circuit, output_dir):
    ...
    try:
        ...
        transient_result = sim.transient_analysis(
            step_time=10e-9, end_time=50e-6  # 10ns  # 50us (5 PWM periods)
        )
...
def generate_example_outputs(output_dir, circuit_type):
    ...
    data = {
        ...
        "nodes": {
            "input": [
                5.0 + 0.1 * np.sin(2 * np.pi * 60 * i * 0.01) for i in range(100)
            ],
            "output": [
                3.3 + 0.01 * np.sin(2 * np.pi * 60 * i * 0.01) for i in range(100)
            ],
        },
        "metadata": {
            "tool": "circuit-synth",
